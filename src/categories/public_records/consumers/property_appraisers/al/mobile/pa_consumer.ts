...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://esearch.mobilecopropertytax.com/'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    await page.click('a[data-filter="search-owner"]');
                ...
                await Promise.all([
                    buttonSearch[1].click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                
                const search_results = await page.$x('//table[@data-role="selectable"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[1].textContent?.trim());
                            link = "https://esearch.mobilecopropertytax.com/Property/View/" + link?.trim();
                    ...
                    let link = await search_results[0].evaluate(el => el.children[1].textContent?.trim());
                    link = "https://esearch.mobilecopropertytax.com/Property/View/" + link?.trim();
                    datalinks.push(link);
                }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      await page.waitForXPath('//th[text()="Name:"]/parent::tr/td[1]');
      const full_name_xpath = '//th[text()="Name:"]/parent::tr/td[1]';
      ...
      // property address
      const property_address_xpath = '//th[text()="Address:"]/parent::tr/td[1]';
      ...
      const last_sale_amount = await this.getTextByXpathFromPage(page, '//tbody[contains(., "Deed Date")]/tr[2]/td[3]');

      // property type
      const property_type_xpath = '//th[text()="Type:"]/parent::tr/td';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//th[text()="Taxable Value:"]/parent::tr/td[1]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = '';
      let year_built = await this.getTextByXpathFromPage(page, '//th[text()="Year Built"]/ancestor::tbody/tr[2]/td[5]');

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://gis.yavapai.us/v4/search.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="p_search"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#p_search');
                if (this.searchBy == 'name'){
                    await page.type('input#p_search', search_value, {delay: 150});
                } else {
                    await page.type('input#p_search', search_value, {delay: 150});
                }
                await page.keyboard.press('Enter');
                await Promise.race([
                    page.waitForXPath('//span[contains(text(), "Sorry, no records")]'),
                    page.waitForXPath('//table/tbody/tr[@alt="Show this parcel"]'),
                    page.waitForXPath('//h3[contains(., "Information for Parcel")]')
                ])
                let [notFound] = await page.$x('//span[contains(text(), "Sorry, no records")]');
                if (notFound){
                    console.log('Not found!');
                    break;
                }
                let [foundOne] = await page.$x('//h3[contains(., "Information for Parcel")]');
                if(foundOne){
                    let result = await this.getPropertyInfos(page);
                    await this.parseResult(result, document);
                    break;
                }
                const search_results = await page.$x('//tr[@alt="Show this parcel"]/parent::tbody/tr');
                let result_length = search_results.length / 2;
                ...
                if(this.searchBy == 'name'){
                    ...
                    for(let i = 0; i < result_length; i++){
                        try{
                            let name = await search_results[indName].evaluate(el => el.children[0].textContent?.trim());
                            const regexp = new RegExp(owner_name_regexp);
                            if (regexp.exec(name!.toUpperCase())){
                                await search_results[indAddress].click();
                                await page.waitForXPath('//h3[contains(., "Information for Parcel")]');
                                let result = await this.getPropertyInfos(page);
                                await this.parseResult(result, document);
                                break;
                            }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//span[@id="owner_name"]';
      ...
      // property address
      const property_address_xpath = '//span[@id="situs_add"]';
      ...
      // mailing address
      const mailing_address_xpath = '//span[@id="situs_add"]';
      let mailing_address = await this.getTextByXpathFromPage(page, mailing_address_xpath);
      mailing_address = mailing_address.replace(/\s+/g, ' ');
      let mailing_zip = await this.getTextByXpathFromPage(page, '//span[@id="zip"]');
      let mailing_state = await this.getTextByXpathFromPage(page, '//span[@id="state"]');
      let mailing_city = await this.getTextByXpathFromPage(page, '//span[@id="city"]');
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[@class="docdate"]';
      const last_sale_amount_xpath = '//td[@class="docSale"]';
      ...
      // property type
      const property_type_xpath = '//span[@id="currLegClass"]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//span[@id="lpvTotal"]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = await this.getTextByXpathFromPage(page, '//span[@id="fcvTotal"]');
      const year_built = await this.getTextByXpathFromPage(page, '//span[contains(., "Constructed:")]/text()[last()]');

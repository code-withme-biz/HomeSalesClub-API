...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://taxsearch.arapahoegov.com/'
    }

    xpaths = {
        isPAloaded: '//input[@id="ContentPlaceHolder1_txtFrom"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://taxsearch.arapahoegov.com/");
                return false;
            }
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#ContentPlaceHolder1_txtFrom');
                if(parsed_addr.number){
                    await page.type('input#ContentPlaceHolder1_txtFrom', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    await page.type('input#ContentPlaceHolder1_txtStreetName', parsed_addr.street, {delay: 50})
                }
                if(parsed_addr.sec_unit_num){
                  await page.type('input#ContentPlaceHolder1_txtUnit', parsed_addr.sec_unit_num, {delay: 50})
                }
                await Promise.all([
                    page.click('input#ContentPlaceHolder1_btnByAddress'),
                    page.waitForNavigation()
                ]);
                let found = await page.$x('//a[@id="ContentPlaceHolder1_aParcelSearch"]');
                if(found.length < 1){
                    console.log("Not found!");
                    break;
                }
                await Promise.all([
                  page.click('a#ContentPlaceHolder1_aParcelSearch'),
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        // name
        const full_name_xpath = '//span[@id="ucParcelHeader_lblFullOwnerListTxt"]';
        ...
        // property address
        const property_address_xpath = '//span[@id="ucParcelHeader_lblSitusAddressTxt"]';
        ...
        let property_city = await this.getTextByXpathFromPage(page, '//span[@id="ucParcelHeader_lblSitusCityTxt"]');
  
        // mailing address
        const mailing_address_xpath = '//span[@id="ucParcelHeader_lblOwnerAddressTxt"]';
        const mailing_address_2_xpath = '//span[@id="ucParcelHeader_lblOwnerCSZTxt"]';
        ...
        // sales info"
        const last_sale_recording_date_xpath = '//span[@id="ucParcelSale_rptrSale_lblSaleTitle"]/ancestor::tbody/tr[2]/td[3]';
        const last_sale_amount_xpath = '//span[@id="ucParcelSale_rptrSale_lblSaleTitle"]/ancestor::tbody/tr[2]/td[4]';
        ...
        // property type
        const property_type_xpath = '//span[@id="ucParcelHeader_lblLandUseTxt"]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
  
        // assessed value and est. value
        const total_assessed_value_xpath = '//span[@id="ucParcelValue_lblAssdTotal"]';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value_xpath = '//span[@id="ucParcelValue_lblApprTotal"]'
        const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);
  
        // year built
        const year_built_xpath = '//*[text()="Year Built"]/ancestor::td[1]/following-sibling::td[2]';

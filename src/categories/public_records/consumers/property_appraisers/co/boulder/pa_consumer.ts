...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://maps.boco.solutions/propertysearch/'
    }

    xpaths = {
        isPAloaded: '//input[@id="searchField"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#searchField');
                await page.type('input#searchField', search_value, {delay: 150});
                await this.sleep(3000);
                try{
                    await page.waitForXPath('//div[@id="choices"]/div', {timeout: 5000});
                } catch(e){
                ...
                try{
                    await Promise.race([
                        page.waitForXPath('//tbody[@role="rowgroup"]/tr'),
                        page.waitForXPath('//div[contains(@ng-repeat, "propertyInfo.Owners")]')
                    ]);
                } catch(e){
                    console.log('Not found!');
                    break;
                }
                const search_results = await page.$x('//tbody[@role="rowgroup"]/tr');
                if(search_results.length < 1){
                    let [foundOne] = await page.$x('//div[contains(@ng-repeat, "propertyInfo.Owners")]');
                    if(foundOne){
                        if(this.searchBy == 'name'){
                            let full_name = await this.getTextByXpathFromPage(page, '//span[text()="Owner:"]/parent::div/span[2]');
                            const regexp = new RegExp(owner_name_regexp);
                            if(!regexp.exec(full_name.toUpperCase())){
                                console.log('Not found!');
                                break;
                            }
                        }
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        ...
                        if (regexp.exec(name!.toUpperCase())){
                            ...
                            await page.waitForXPath('//div[contains(@ng-repeat, "propertyInfo.Owners")]');
                            let result = await this.getPropertyInfos(page);
                            await this.parseResult(result, document);
                            break;
                        }
                    ...
                    await search_results[0].click({clickCount: 2});
                    await page.waitForXPath('//div[contains(@ng-repeat, "propertyInfo.Owners")]');
                    let result = await this.getPropertyInfos(page);
                    await this.parseResult(result, document);
                }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//span[text()="Owner:"]/parent::div/span[2]';
      ...
      // property address
      const property_address_xpath = '//span[text()="Property Address:"]/parent::div/span[2]';
      ...
      // mailing address
      const mailing_address_xpath = '//span[text()="Mailing Address:"]/parent::div/span[2]';
      let mailing_address = await this.getTextByXpathFromPage(page, mailing_address_xpath);
      let mailing_zip = await this.getTextByXpathFromPage(page, '//span[text()="Zip:"]/parent::div/span[2]');
      const mailing_address_2_xpath = '//span[text()="City:"]/parent::div/span[2]';
      const mailing_address_2 = await this.getTextByXpathFromPage(page, mailing_address_2_xpath);
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//tr[@ng-repeat="deed in deeds"]/td[2]';
      const last_sale_amount_xpath = '//tr[@ng-repeat="deed in deeds"]/td[4]';
      ...
      const total_assessed_value_xpath = '//th[text()="Assessed"]/ancestor::table/tbody/tr[1]/td[3]/span';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//th[text()="Assessed"]/ancestor::table/tbody/tr[1]/td[2]/span';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      const year_built = await this.getTextByXpathFromPage(page, '//span[text()="Built:"]/parent::div/span[2]');

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.douglas.co.us/assessor/'
    }

    xpaths = {
        isPAloaded: '//input[contains(@class, "search-bar")]'
    }
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              ...
              search_value = owner_name;
                ...
                search_value = ((parsev1.number ? parsev1.number : '') + ' ' + (parsev1.street ? parsev1.street + ' ' : '') + (parsev1.type ? parsev1.type : '')).trim();
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    await page.type('input[type="text"]', search_value, {delay: 150});
                } else {
                    await page.type('input[type="text"]', search_value, {delay: 150});
                }
                let buttonSearch = await page.$x('//i[contains(@class, "fa-search")]');
                await buttonSearch[0].click();
                await Promise.race([
                    page.waitForXPath('//div[contains(text(), "There were no results")]'),
                    page.waitForXPath('//a[@class="table-row"]'),
                    page.waitForXPath('//div[@class="owner-info"]/div[2]')
                ])
                let [notFound] = await page.$x('//div[contains(text(), "There were no results")]');
                if(notFound){
                    console.log('Not found!');
                    break;
                }
                let [foundOne] = await page.$x('//div[@class="owner-info"]/div[2]');
                if(foundOne){
                    let result = await this.getPropertyInfos(page);
                    await this.parseResult(result, document);
                    break;
                }
                const search_results = await page.$x('//a[@class="table-row"]');
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            ...
                            if (regexp.exec(name!.toUpperCase())){
                                ...
                                await this.parseResult(result, document);
                                await page.goBack();
                                await page.waitForXPath('//a[@class="table-row"]');
                            }
                        } catch(e){
                            continue;
                        }
                    ...
                    let address = await search_results[0].evaluate(el => el.children[2].children[0].textContent?.trim());
                    address = address?.toLowerCase().replace(/\s+/g, '');
                    let address_search = search_value.toLowerCase().replace(/\s+/g, '');
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//div[@class="owner-info"]/div[2]';
      ...
      // property address
      const property_address_xpath = '//div[contains(@class, "owner-address")]//h3/text()[1]';
      const property_address_2_xpath = '//div[contains(@class, "owner-address")]//h3/text()[last()]';
      ...
      // mailing address
      const mailing_address_xpath = '//div[@class="owner-info"]/div[3]/span/text()[1]';
      const mailing_address_2_xpath = '//div[@class="owner-info"]/div[3]/span/text()[last()]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[contains(., "Deed")]/parent::tr/td[2]';
      const last_sale_amount_xpath = '//td[contains(., "Deed")]/parent::tr/td[3]';
      ...
      // property type
      const property_type_xpath = '//div[text()="Building Use:"]/parent::div/div[2]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//table[@class="value-data-table table"]/tbody[2]/tr[1]/td[3]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//table[@class="value-data-table table"]/tbody[2]/tr[1]/td[2]';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      // year built
      const year_built_xpath = '//div[text()="Year Built:"]/following-sibling::div[1]';

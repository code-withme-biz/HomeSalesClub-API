...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    owner_xpath = '//p[./span[contains(., "Owner")]]/span[2]';
    mailing_rows_xpath = '//p[./span[contains(., "Mailing Address")]]/span[2]';
    last_sale_date_xpath = '//div[@id="sales"]//table/tbody[1]/tr[1]/td[2]';
    last_sale_amount_xpath = '//div[@id="sales"]//table/tbody[1]/tr[1]/td[3]';
    property_type_xpath = '//div[@id="land"]//table/tbody[1]/tr[1]/td[2]';
    ...
    multiple_row_xpath = '//div[@class="data-list-section"]/ul/li[1]/p/span[@class="value  null"][1]';
    ...
    async parseResult(page: puppeteer.Page, link: any, document: IOwnerProductProperty, address: any) {
        ...
        let owner_names, mailing_address, mailing_city, mailing_zip, mailing_state, property_address, property_type, last_sale_date, total_assessed_value, last_sale_amount, year_built;
        try {
            mailing_address = await page.evaluate(el => el.textContent, (await page.$x(this.mailing_rows_xpath))[0]);
            let parsemailing = parseAddress.parseLocation(mailing_address);
            if(parsemailing){
                mailing_city = parsemailing.city  || '';
                mailing_zip = parsemailing.zip  || '';
                mailing_state = parsemailing.state  || '';
            }
        } catch {
            mailing_address = '';
            mailing_city = '';
        ...
        try {
            owner_names = await page.evaluate(el => el.textContent, (await page.$x(this.owner_xpath))[0]);
        } catch {
            owner_names = '';
        }
        try {
        ...
        const property_address_parsed = parserAddress.parseLocation(property_address);
        const mailing_address_parsed = parserAddress.parseLocation(mailing_address);
        let owner_occupied;
        try{
            owner_occupied = this.compareAddress(this.searchBy === 'name' ? property_address_parsed : address['parsed'], mailing_address_parsed);
        } catch(e){
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        const url_search = 'https://property.spatialest.com/co/elpaso/#/';
        ...
        let doc = docsToParse;
            ...
            if (flagMultipleRow) {
                try {
                    ...
                    let nameOwnerRow = await page.$x('//div[@class="data-list-section"]/ul/li[2]/p/span[@class="value  null"]');

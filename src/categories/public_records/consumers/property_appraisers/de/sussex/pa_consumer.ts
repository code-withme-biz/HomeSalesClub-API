...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://property.sussexcountyde.gov/PT/search/commonsearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'https://property.sussexcountyde.gov/PT/search/commonsearch.aspx?mode=address'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (!hasResult) {
                  let [found] = await page.$x('//table[@id="Owners"]/tbody/tr[2]/td[1]');
                  if(found){
                ...
                for(let i = 0; i < rows.length; i++) {
                    const row = rows[i];
                    let name = await row.evaluate(el => el.children[2].children[0].textContent?.trim()) || '';
                    let address = await row.evaluate(el => el.children[6].children[0].textContent?.trim()) || ''; 
                    if(this.searchBy == 'name') {
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//table[@id="Owners"]/tbody/tr[2]/td[1]';
      ...
      // property address
      const property_address_xpath = '//td[text()="Property Location:"]/parent::tr/td[2]';
      ...
      let property_city = await this.getTextByXpathFromPage(page, '//td[text()="City:"]/parent::tr/td[2]');
      let property_zip = await this.getTextByXpathFromPage(page, '//td[text()="Zip:"]/parent::tr/td[2]');
      const property_address_parsed = parser.parseLocation(property_address);
      
      // mailing address
      const mailing_address_xpath = '//table[@id="Owners"]/tbody/tr[2]/td[3]';
      ...
      let mailing_zip = await this.getTextByXpathFromPage(page, '//table[@id="Owners"]/tbody/tr[2]/td[6]');
      let mailing_state = await this.getTextByXpathFromPage(page, '//table[@id="Owners"]/tbody/tr[2]/td[5]');
      let mailing_city = await this.getTextByXpathFromPage(page, '//table[@id="Owners"]/tbody/tr[2]/td[4]');
      // owner occupied
      let owner_occupied;
      try{
      ...
      // property type
      const property_type_xpath = '//td[text()="Use Code (LUC):"]/parent::tr/td[2]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value = '';
      const est_value_xpath = '//td[text()="100% Total Value"]/parent::tr/td[2]';
      ...
      const last_sale_amount_xpath = '//table[@id="Sales"]/tbody/tr[2]/td[3]';

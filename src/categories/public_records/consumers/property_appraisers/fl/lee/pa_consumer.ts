...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.leepa.org/search/propertysearch.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="ctl00_BodyContentPlaceHolder_OwnerNameTextBox"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {                
                await page.waitForSelector('input#ctl00_BodyContentPlaceHolder_OwnerNameTextBox');
                if (this.searchBy == 'name'){
                    await page.type('input#ctl00_BodyContentPlaceHolder_OwnerNameTextBox', search_value, {delay: 150});
                } else {
                    await page.type('input#ctl00_BodyContentPlaceHolder_AddressTextBox', search_value, {delay: 150});
                }
                await page.click('input#ctl00_BodyContentPlaceHolder_SubmitPropertySearch');
                try{
                    await page.waitForXPath('//table[@class="resultsDataGrid"]/tbody/tr/td[@class="boxLeft boxTop boxBottom boxRight"]');
                } catch(e){
                    console.log("Not found!");
                    break;
                }
                const search_results = await page.$x('//table[@class="resultsDataGrid"]/tbody/tr/td[@class="boxLeft boxTop boxBottom boxRight"]');
                const rows = await page.$x('//table[@class="resultsDataGrid"]/tbody/tr/td[@class="boxLeft boxTop boxBottom boxRight"]');
                const linkrows =  await page.$x('//a[text()="Parcel Details"]');
                const datalinks = [];
                if(this.searchBy == 'name'){
                    for(let i = 0; i < rows.length; i++){
                        let name1 = await rows[i].evaluate(el => el.children[0].textContent?.trim());
                        name1 = name1?.replace("&","").replace("+","").trim();
                        let name2 = await rows[i].evaluate(el => el.children[1].textContent?.trim());
                        const regexp = new RegExp(owner_name_regexp);
                        if(regexp.exec(name1!.toUpperCase()) || regexp.exec(name2!.toUpperCase())){
                            let link = await linkrows[i].evaluate(el => el.getAttribute('href'));
                            link = "https://www.leepa.org" + link + "&TaxRollDetails=True&AppraisalDetails=True&SalesDetails=True";
                            datalinks.push(link);
                        }
                    }
                } else {
                    let link = await linkrows[0].evaluate(el => el.getAttribute('href')?.trim());
                    link = "https://www.leepa.org" + link + "&TaxRollDetails=True&AppraisalDetails=True&SalesDetails=True";
                    datalinks.push(link);
                }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//div[@id="divDisplayParcelOwner"]/div[1]/div[1]/div[2]/div/text()[1]';
      let full_name = await this.getTextByXpathFromPage(page, full_name_xpath);
      const owner_names = [];
      full_name = full_name.replace("+", "").replace("&","").trim();
      let parseName = this.parseOwnerName(full_name);
      owner_names.push(parseName);

      // property address
      const property_address_xpath = '//div[@id="divDisplayParcelOwner"]/div[2]/div[2]/text()[1]';
      const property_address_2_xpath = '//div[@id="divDisplayParcelOwner"]/div[2]/div[2]/text()[2]';
      ...
      // mailing address
      const rowowner = await page.$x('//div[@id="divDisplayParcelOwner"]/div[1]/div[1]/div[2]/div/text()');
      const mailing_address_xpath = '//div[@id="divDisplayParcelOwner"]/div[1]/div[1]/div[2]/div/text()['+(rowowner.length - 1)+']';
      const mailing_address_2_xpath = '//div[@id="divDisplayParcelOwner"]/div[1]/div[1]/div[2]/div/text()['+(rowowner.length)+']';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//th[contains(text(), "Sale Price")]/ancestor::tbody/tr[2]/td[2]';
      const last_sale_amount_xpath = '//th[contains(text(), "Sale Price")]/ancestor::tbody/tr[2]/td[1]';
      ...
      // property type
      const property_type_xpath = '//th[text()="Model Type"]/ancestor::tbody/tr[4]/td[2]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
      	
      // assessed value and est. value
      const total_assessed_value_xpath = '//th[text()="Assessed"]/parent::tr/td';

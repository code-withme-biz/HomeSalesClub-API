...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://www.leonpa.org/pt/search/commonsearch.aspx?mode=OWNER',
        propertyAppraiserPageAddress: 'https://www.leonpa.org/pt/search/commonsearch.aspx?mode=ADDRESS'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "Owner(s):")]/following-sibling::td[1]';
      ...
      // property address
      const property_address_xpath = '//td[contains(., "Location:")]/following-sibling::td[1]';
      ...
      // mailing address
      const mailing_address_xpath = '//td[contains(., "Mailing Addr:")]/following-sibling::td[1]';
      ...
      // property type
      const property_type_xpath = '//td[contains(., "Property Use:")]/following-sibling::td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // sales info"
      const last_sale_recording_date_xpath = '//*[@id="Sales Information"]/tbody/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//*[@id="Sales Information"]/tbody/tr[2]/td[2]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      // assessed value and est. value
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//*[@id="Certified Taxable Values"]/tbody/tr[2]/td[5]');
      const est_value_xpath = '//*[@id="Certified Value Detail"]/tbody/tr[2]/td[4]'
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      const year_built_xpath = '//*[text()="Yr Built"]/ancestor::table[1]/tbody/tr[2]/td[6]';

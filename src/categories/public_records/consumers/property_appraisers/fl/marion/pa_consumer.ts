...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.pa.marion.fl.us/PropertySearch.aspx'
    }

    xpaths = {
        isPAloaded: '//*[@id="MCPAMaster_MCPAContent_btnWine"]'
    }
            ...
            while (true){
              ...
              try {
                try {
                  ...
                  await page.goto(`https://www.pa.marion.fl.us/PropertySearch.aspx?SearchBy=${this.searchBy.charAt(0).toUpperCase()+this.searchBy.slice(1)}&Parms=${search_value}`, {waitUntil: 'load'});
                } catch (error) {
                  await page.reload();
                }
                await page.waitForXPath('//text()[contains(., "records found")]', {visible: true});
                const result_text = await this.getTextByXpathFromPage(page, '//text()[contains(., "records found")]');
                const result_number = parseInt(result_text);
                if (result_number === 0) {
                ...
                const links: string[] = [];
                if(this.searchBy == 'name'){
                  const rows = await page.$x('//*[@id="srch"]/table[1]/tbody/tr');
                  ...
                  const rows = await page.$x(`//*[@id="srch"]/table[1]/tbody/tr/td[font[contains(text(), "${search_value}")]]/parent::tr[1]`);
                  for (const row of rows) {
                    let link = await row.evaluate(el => el.children[0].children[0].getAttribute('href')) || '';
                    links.push(link);
                  }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//*[text()="Property Information"]/parent::center[1]/following-sibling::table[1]/tbody/tr[1]/td[1]';
      ...
      let match = name_mailing_addr.split('\n').filter((s:string)=>s.trim()!=='').map((s:string)=>s.trim());
      let length = match.length;
      if (length < 4) return null;
      let full_name = match[length-3];
      let parseName = this.parseOwnerName(full_name);
      const owner_names = [parseName];
      let mailing_address = match[length-2];
      let mailing_zip = '';
      let mailing_state = '';
      let mailing_city = '';
      let mailing_address_parsed = parser.parseLocation(mailing_address + ', ' + match[length-1]);
      ...
      // property address
      const property_address_full_xpath = '//text()[contains(.,"Situs")]';
      let property_address = await this.getTextByXpathFromPage(page, property_address_full_xpath);
      property_address = property_address.slice(6).trim();
      let property_zip = '';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//*[text()="Book/Page"]/ancestor::table[1]/tbody/tr[2]/td[2]';
      const last_sale_amount_xpath = '//*[text()="Book/Page"]/ancestor::table[1]/tbody/tr[2]/td[7]';
      ...
      // property type
      const property_type_xpath = '//b[text()="Improvement"]/ancestor::td[1]/following-sibling::td[1]/text()[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//*[text()="Assessed Val"]/ancestor::table[1]/tbody/tr[2]/td[6]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[text()="Assessed Val"]/ancestor::table[1]/tbody/tr[2]/td[3]'
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      // year built
      const year_built_xpath = '//b[text()="Year Built"]/following-sibling::text()[1]';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.ocpafl.org/searches/parcelsearch.aspx',
    }

    xpaths = {
        isPAloaded: '//*[contains(@id, "PopupPanel1_CloseButton")]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                await page.waitForXPath('//*[@id="popup_ok"]');
                await page.click('#popup_ok');
                await page.waitForXPath('//*[contains(@id, "PopupPanel1_CloseButton")]');
                const [closeButton] = await page.$x('//*[contains(@id, "PopupPanel1_CloseButton")]');
              ...
              try {
                if (this.searchBy === 'name') {
                  await page.type('div[id$="QuickSearches"] input[id$="_OwnerName"]', search_value, {delay: 150});
                  await page.click('div[id$="QuickSearches"] input[id$="_OwnerNameSearch1_ctl00_ActionButton1"]');
                } else {
                  await page.type('div[id$="QuickSearches"] input[id$="_Address"]', search_value, {delay: 150});
                  await page.click('div[id$="QuickSearches"] input[id$="_AddressSearch1_ctl00_ActionButton1"]');
                }

                await page.waitForSelector('div[id$="_UpdateProgress1"]', {visible: true});
                await page.waitForSelector('div[id$="_UpdateProgress1"]', {hidden: true});

                const [isParcel] = await page.$x('//*[contains(text(), "Property Record Card")]')
                ...
                if (isParcel) {
                  ...
                  const [noResult] = await page.$x('//*[contains(text(), "Your search did not return any results")]');
                  if (noResult) {
                    console.log('*** No Results Found!');
                    break;
                  }
                  console.log('~~~~ LIST')
                  const rows = await page.$x('//table[contains(@id, "_ResultsTab_ResultsGrid")]/tbody/tr[position()>1]');
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      await page.waitForXPath('//*[contains(text(), "Property Record Card")]')
      // name
      const full_name_xpath = '//*[contains(text(), "Name(s)")]/following-sibling::div[1]'
      await page.waitForXPath(full_name_xpath);
      ...
      // mailing address
      const mailing_address_xpath = '//*[contains(text(), "Mailing Address")]/following-sibling::text()[1]';
      const mailing_city_zip_xpath = '//*[contains(text(), "Mailing Address")]/following-sibling::text()[2]';
      ...
      // property address
      const property_address_xpath = '//*[contains(text(), "Physical Street Address")]/following-sibling::text()[1]';
      const property_city_zip_xpath = '//*[contains(text(), "Postal City and Zipcode")]/following-sibling::text()[1]';
      ...
      // property type
      const property_type_xpath = '//*[contains(text(), "Property Use")]/following-sibling::text()[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//*[text()="Tax Year Values"]/ancestor::table[1]/tbody/tr[2]/td[6]/table/tbody/tr[1]/td[2]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[text()="Tax Year Values"]/ancestor::table[1]/tbody/tr[2]/td[5]/table/tbody/tr[1]/td[2]';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      // sales info"
      const [salebutton] = await page.$x('//a[contains(@id, "TabContainer1_SaleAnalysis")]');
      await salebutton.click();
      await this.sleep(1000);
      const last_sale_recording_date_xpath = '//table[contains(@id, "SaleAnalysis_SalesAnalysis1_Grid1")]/tbody/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//table[contains(@id, "SaleAnalysis_SalesAnalysis1_Grid1")]/tbody/tr[2]/td[2]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      const [featuresbutton] = await page.$x('//a[contains(@id, "TabContainer1_DetailsTab")]');
      await featuresbutton.click();
      await this.sleep(1000);
      const year_built = await this.getTextByXpathFromPage(page, '//*[contains(text(), "Actual Year Built")]/following-sibling::td[1]/text()');

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://qpublic.schneidercorp.com/Application.aspx?App=WaltonCountyFL&PageType=Search'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      let mailing_address = mailing_full_address[mailing_full_address.length - 2];
      let mailing_zip = '';
      let mailing_state = '';
      let mailing_city = '';
      const mailing_address_parsed = parser.parseLocation(mailing_full_address.slice(mailing_full_address.length - 2).join(' '));
      ...
      const total_assessed_value_xpath = '//th[text()="Assessed Value"]/following-sibling::td[1]';
      ...
      // yearBuilt
      const year_built_xpath = '//*[text()="Year Built"]/ancestor::th[1]/following-sibling::td[1]';

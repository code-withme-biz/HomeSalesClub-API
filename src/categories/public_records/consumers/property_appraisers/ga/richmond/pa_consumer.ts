...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://qpublic.schneidercorp.com/Application.aspx?App=RichmondCountyGA&Layer=Parcels&PageType=Search'
    }
                ...
                for (const row of rows) {
                  ...
                  if (flag) {
                    ...
                    if (this.searchBy === name) {
                      ...
                      let result = await this.getPropertyInfos(detailPage, regexp);
    ...
    async getPropertyInfos(page: puppeteer.Page, nameRegExp: RegExp | null = null): Promise<any> {
      // name and addr
      const owner_names = [];
      const name_xpath = '//*[contains(@id, "_lnkOwnerName")]';
      let owner_name = await this.getTextByXpathFromPage(page, name_xpath);
      const owner_name_arr = owner_name.split('&')[0];
      if (nameRegExp && nameRegExp.exec(owner_name_arr.toUpperCase())){
        return null;
      }
      ...
      // mailing address
      const mailing_address_xpath  = '//*[contains(@id, "_lblAddress")]';
      ...
      const mailing_city_state_xpath  = '//*[contains(@id, "_lblCityStateZip")]';
      let mailing_city_state: any = await this.getTextByXpathFromPage(page, mailing_city_state_xpath);
      let mailing_zip = '';
      let mailing_state = '';
      let mailing_city = '';
      const mailing_address_parsed = parser.parseLocation(mailing_full_address + ' ' + mailing_city_state);
      ...
      const total_assessed_value_xpath = '//th[text()="Accessory Value"]/following-sibling::td';

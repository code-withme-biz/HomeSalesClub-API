...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://beacon.schneidercorp.com/Application.aspx?AppID=909&LayerID=17429&PageTypeID=2&PageID=7823'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {       
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              console.log('Searching by Owner is not enabled!');
              return false;
            }
            else {
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name and addr
      const owner_names = [];
      const name_addr_xpath = '//*[contains(@id, "_lblMailing")]';
      let name_addr: any = await this.getInnerTextByXpathFromPage(page, name_addr_xpath);
      name_addr = name_addr.split('\n').map((s:string)=>s.trim()).filter((s:string)=>s!=='');
      let owner_name = name_addr[0].split('&')[0];
      owner_name = owner_name.replace(/\W|\s+/g, ' ').trim();
      const ownerName = this.parseOwnerName(owner_name.trim());
      owner_names.push(ownerName);

      // mailing address
      let mailing_address = name_addr[1];
      let mailing_zip = '';
      let mailing_state = '';
      let mailing_city = '';
      const mailing_address_parsed = parser.parseLocation(name_addr.slice(1).join(' '));
      ...
      const total_assessed_value_xpath = '//th[text()="Net Assessed Value"]/following-sibling::td';
      const est_value_xpath = '';

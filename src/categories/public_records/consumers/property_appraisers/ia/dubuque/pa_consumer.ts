...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://beacon.schneidercorp.com/Application.aspx?AppID=93&LayerID=929&PageTypeID=2&PageID=589'
    }

    xpaths = {
        isPAloaded: '//a[text()="Agree"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {       
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              console.log('ERROR: Searching by Owner is not allowed!');
              return false;
            ...
            while (true){
              ...
              try {                
                ...
                for (const row of rows) {
                  ...
                  name = name.replace(/\n|\s+/gm, ' ').trim();
                  let address = await page.evaluate(el => el.children[3].textContent, row);
                  ...
                  if (flag) {
                    let link = await row.evaluate((el: any) => el.children[2].children[0].href) || '';
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name and addr
      const owner_names = [];
      const name_xpath = '//*[contains(@id, "_lblDeedName_")]';
      ...
      // mailing address
      const mailing_address_xpath  = '//*[contains(@id, "_lblAddress1")]';
      let mailing_address: any = await this.getTextByXpathFromPage(page, mailing_address_xpath);
      const mailing_citystate_xpath = '//*[contains(@id, "_lblAddress3")]'
      let mailing_citystate: any = await this.getTextByXpathFromPage(page, mailing_citystate_xpath);
      let mailing_zip = '';
      let mailing_state = '';
      let mailing_city = '';
      const mailing_address_parsed = parser.parseLocation(mailing_address + ' ' + mailing_citystate);
      ...
      // property address
      const property_address_xpath = '//*[contains(@id, "_lblPropertyAddress")]';
      ...
      // property type
      const property_type_xpath = '//*[contains(@id, "_lblClass")]';
      ...
      // sales info
      const last_sale_recording_date_xpath = '//table[contains(@id, "_gvwSales")]/tbody/tr[1]/th[1]';
      const last_sale_amount_xpath = '//table[contains(@id, "_gvwSales")]/tbody/tr[1]/td[7]';
      ...
      const total_assessed_value_xpath = '//th[text()="Assessed Land Value"]/following-sibling::td';
      const est_value_xpath = '//th[text()="Assessed Dwelling Value"]/following-sibling::td';
      ...
      // year buit
      const year_built_xpath = '//*[contains(@id, "_lblYearBuilt")]';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://www.adacountyassessor.org/propsys/AddressSearch.jsp'
    }

    xpaths = {
        isPAloaded: '//input[@name="agree"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: http://www.adacountyassessor.org/propsys/AddressSearch.jsp");
                return false;
            }
            ...
            while (true){
              ...
              try {
                ...
                if(tosButton.length > 0){
                    ...
                    let submitButton = await page.$x('//input[@value="Submit"]')
                    await Promise.all([
                ...
                await page.type('input[name="address"]', search_addr);
                await Promise.all([
                    page.click('input[value="Submit"]'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//table[@class="simple"]/tbody/tr');
                if(found.length < 1){
                    console.log("Not found!");
                    break;
                }
                let addressResultHandle = await page.$x('//table[@class="simple"]/tbody/tr[1]/td[3]');
                let addressResult = await addressResultHandle[0].evaluate(el => el.textContent?.trim());
                if(parsed_addr.number){
                    if (!addressResult?.includes(parsed_addr.number)){
                        console.log('Not found!');
                        break;
                    }
                }
                let link = await page.$x('//table[@class="simple"]/tbody/tr[1]/td[1]/a');
                await Promise.all([
                    link[0].click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ])
    ...
    async getPropertyInfos(page: puppeteer.Page, parsed_addr: any): Promise<any> {
        // name
        const full_name_xpath = '//div[@id="detailstab"]/table/tbody/tr[1]/td[1]/text()[8]';
        ...
        const total_assessed_value_xpath = '//td[contains(., "Assessed Value")]/parent::tr/parent::tbody/parent::table/tbody/tr[2]/td[2]';

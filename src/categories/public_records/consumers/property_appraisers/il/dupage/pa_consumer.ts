...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.dupageco.org/PropertyInfo/PropertyLookup.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="ctl00_pageContent_ctl00_txtStreetNumber"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://www.dupageco.org/PropertyInfo/PropertyLookup.aspx");
                return false;
            }
            ...
            while (true){
              ...
              try {
                let checkDialog = await page.$x('//button[@title="Close subscription dialog"]');
                if(checkDialog.length > 0){
                    await checkDialog[0].click();
                }
                await page.waitForSelector('input#ctl00_pageContent_ctl00_txtStreetNumber');
                if(parsed_addr.number){
                    await page.type('input#ctl00_pageContent_ctl00_txtStreetNumber', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    await page.type('input#ctl00_pageContent_ctl00_txtStreet', parsed_addr.street, {delay: 50})
                }
                if(parsed_addr.prefix){
                    try{
                        await page.select('select#ctl00_pageContent_ctl00_ddlDir', parsed_addr.prefix.toUpperCase());
                    } catch(e){
                        //
                    }
                }
                await Promise.all([
                    page.click('input#ctl00_pageContent_ctl00_btnSearch'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//div[contains(., "Parcel Address")]/div[@class="tabdiv11"]');
                if(found.length < 1){
                    let found2 = await page.$x('//a[@id="ctl00_pageContent_ctl00_gvList_ctl02_lnkPin"]');
                    if(found2.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        // name
        const full_name_xpath = '//div[contains(., "Billing Address")]/div[@class="tabdiv11"]/text()[1]';
        ...
        // property address
        const property_address_xpath = '//div[contains(., "Parcel Address")]/div[@class="tabdiv11"]/text()[last()-1]';
        ...
        // mailing address
        const mailing_address_xpath = '//div[contains(., "Billing Address")]/div[@class="tabdiv11"]/text()[last()-1]';
        const mailing_address_2_xpath = '//div[contains(., "Billing Address")]/div[@class="tabdiv11"]/text()[last()]';
        ...
        let valueTab = await page.$x('//a[contains(., "Property Tax History")]');
        await valueTab[0].click();
        await this.sleep(1000);
        // property type
        const property_type_xpath = '//td[contains(., "Property Class")]/parent::tr/td[2]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
  
        // assessed value and est. value
        const total_assessed_value_xpath = '//td[contains(., "Assessed Total")]/parent::tr/td[2]';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value_xpath = '//td[contains(., "Fair Cash Value")]/parent::tr/td[2]'
        const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

        let salesTab = await page.$x('//a[contains(., "Sale History")]');
        await salesTab[0].click();
        await this.sleep(1000);
        // sales info"
        const last_sale_recording_date_xpath = '//table[@id="ctl00_pageContent_ctl00_gvList6"]/tbody/tr[last()]/td[2]';
        const last_sale_amount_xpath = '//table[@id="ctl00_pageContent_ctl00_gvList6"]/tbody/tr[last()]/td[3]';

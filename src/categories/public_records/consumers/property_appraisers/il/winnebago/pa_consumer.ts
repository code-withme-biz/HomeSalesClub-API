...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://assessor.wincoil.us/assessment/search/'
    }

    xpaths = {
        isPAloaded: '//input[@id="txtNumber"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#txtStreet');
                if (this.searchBy == 'name'){
                    await page.type('input#txtLName', last_name, {delay: 150});
                    await page.type('input#txtFName', first_name, {delay: 150});
                } else {
                    await page.type('input#txtNumber', address.number, {delay: 150});
                    await page.type('input#txtStreet', address.street, {delay: 150});
                    if(address.prefix){
                      await page.type('input#txtDirection', address.prefix, {delay: 150});
                    }
                }
                let buttonSearch = await page.$x('//input[@id="btnSubmit"]');
                await Promise.all([
                    buttonSearch[0].click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                
                const search_results = await page.$x('//table[@id="grdList"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            ...
                            link = "http://assessor.wincoil.us/assessment/search/" + link?.trim();
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      let [lastYear] = await page.$x('//select[@id="ddlYears"]/option[2]');
      let lastYearValue: any = await lastYear.evaluate(el => el.getAttribute('value'));
      await Promise.all([
          page.select('select#ddlYears', lastYearValue),
          page.waitForNavigation({waitUntil: 'networkidle0'})
      ])

      // name
      const full_name_xpath = '//i[text()="Owner"]/ancestor::table/tbody/tr[2]/td[2]';
      ...
      // property address
      const property_address_xpath = '//i[text()="Property Location"]/ancestor::table/tbody/tr[3]/td[3]';
      ...
      // mailing address
      const mailing_address_xpath = '//i[text()="Owner"]/ancestor::table/tbody/tr[3]/td[2]';
      const mailing_address_2_xpath = '//i[text()="Owner"]/ancestor::table/tbody/tr[5]/td[2]';
      ...
      // property type
      const property_type_xpath = '//i[contains(., "Class Code")]/ancestor::tr/td[4]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//i[text()="Assessment Level"]/ancestor::table/tbody/tr[2]/td[6]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = '';

      // sales info"
      let last_sale_recording_date, last_sale_amount = '';
      let [checkLink] = await page.$x('//a[@id="lnkRkfdAssessor"]');
      if(checkLink){
        await Promise.all([
          checkLink.click(),
          page.waitForNavigation({waitUntil: 'networkidle0'})
        ]);
        const last_sale_recording_date_xpath = '//table[@id="ctl00_contentPageContent_gridSales"]/tbody/tr[2]/td[1]';
        const last_sale_amount_xpath = '//table[@id="ctl00_contentPageContent_gridSales"]/tbody/tr[2]/td[3]';
        last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
        last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);
      } else {
        const [iframe1] = await page.$x('//*[@id="frame1"]');
        if(iframe1){
          const page1 = await iframe1.contentFrame();
          if(page1){
            try{
              await this.sleep(3000);
              const last_sale_recording_date_xpath = '//td[text()="Sales History"]/ancestor::table/tbody/tr[3]/td[2]';
              const last_sale_amount_xpath = '//td[text()="Sales History"]/ancestor::table/tbody/tr[3]/td[1]';
              last_sale_recording_date = await this.getTextByXpathFromPage(page1, last_sale_recording_date_xpath);
              last_sale_amount = await this.getTextByXpathFromPage(page1, last_sale_amount_xpath);
            } catch(e){
            }
          }
        }

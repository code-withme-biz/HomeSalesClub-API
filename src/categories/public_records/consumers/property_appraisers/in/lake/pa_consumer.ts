...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://engage.xsoftinc.com/lake/DataOnly'
    }

    xpaths = {
        isPAloaded: '//input[@type="search"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                await page.type('input', search_value, {delay: 150});
                await Promise.all([
                  page.keyboard.press('Enter'),
                  page.waitForResponse((response: any) => response.url().includes('DataOnly/GetData') && response.status() === 200)
                ]);
                let [notFound] = await page.$x('//td[text()="No data available in table"]');
                if(notFound){
                  console.log('Not found!');
                  break;
                }
                const search_results = await page.$x('//table[@id="data-table"]/tbody/tr');

                const datalinks: any = [];
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[0].children[0].textContent?.trim());
                            link = "https://engage.xsoftinc.com/lake/Map/GetParcelDetail?parcelId=" + link;
                    ...
                    let address: any = await search_results[0].evaluate(el => el.children[2].textContent?.trim());
                    let link = await search_results[0].evaluate(el => el.children[0].children[0].textContent?.trim());
                    link = "https://engage.xsoftinc.com/lake/Map/GetParcelDetail?parcelId=" + link?.trim();
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//th[text()="CURRENT OWNER"]/ancestor::tbody/tr[2]/td[1]';
      ...
      // property address
      const property_address_xpath = '//th[text()="Property Address: "]/parent::tr/td[1]/text()[1]';
      ...
      let property_zip = await this.getTextByXpathFromPage(page, '//th[text()="Property Address: "]/parent::tr/td[1]/text()[4]');
      let property_state = await this.getTextByXpathFromPage(page, '//th[text()="Property Address: "]/parent::tr/td[1]/text()[3]');
      let property_city = await this.getTextByXpathFromPage(page, '//th[text()="Property Address: "]/parent::tr/td[1]/text()[2]');

      // mailing address
      const mailing_address_xpath = '//th[text()="CURRENT OWNER"]/ancestor::tbody/tr[3]/td[1]';
      let mailing_address = await this.getTextByXpathFromPage(page, mailing_address_xpath);
      let mailing_zip = await this.getTextByXpathFromPage(page, '//th[text()="CURRENT OWNER"]/ancestor::tbody/tr[6]/td[1]');
      let mailing_state = await this.getTextByXpathFromPage(page, '//th[text()="CURRENT OWNER"]/ancestor::tbody/tr[last()-1]/td[1]');
      let mailing_city = await this.getTextByXpathFromPage(page, '//th[text()="CURRENT OWNER"]/ancestor::tbody/tr[last()-2]/td[1]');
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//th[text()="TRANSFER HISTORY"]/ancestor::tbody/tr[2]/td[1]';
      const last_sale_amount_xpath = '//th[text()="Sale Price"]/ancestor::tbody/tr[2]/td[last()]';
      ...
      // property type
      const property_type_xpath = '//th[text()="Property Class:"]/parent::tr/td[2]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value = ''
      const est_value_xpath = '//th[text()="Assessment Year"]/ancestor::tbody/tr[2]/td[last()]';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://beacon.schneidercorp.com/Application.aspx?AppID=402&LayerID=6170&PageTypeID=2&PageID=3301'
    }
            ...
            while (true){
              ...
              try {                
                ...
                for (const row of rows) {
                  ...
                  if (flag) {
                    let link = await row.evaluate((el: any) => el.children[1].children[0].href) || '';
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name and addr
      const owner_names = [];
      const name_xpath = '//*[contains(@id, "ctlBodyPane_ctl01_ctl01_frmOwner_lblName1")]';
      ...
      let mailing_address = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl01_ctl01_frmOwner_lblAddr1"]');
      let mailing_zip = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl01_ctl01_frmOwner_lblZip"]');
      let mailing_state = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl01_ctl01_frmOwner_lblState"]');
      let mailing_city = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl01_ctl01_frmOwner_lblCity"]');
      mailing_city = mailing_city.replace(',','').trim();

      // property address
      let property_address = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl00_ctl01_detSummary_lblPropAddr1"]');
      let property_zip = '';
      let property_state = '';
      let property_city = await this.getTextByXpathFromPage(page, '//span[@id="ctlBodyPane_ctl00_ctl01_detSummary_lblPropCity"]');

      // owner occupied
      let owner_occupied = property_address === mailing_address;

      // property type
      const property_type_xpath = '//span[@id="ctlBodyPane_ctl00_ctl01_detSummary_lblClass"]';
      ...
      // sales info
      const last_sale_recording_date_xpath = '//th[text()="Transfer Date"]/parent::tr/parent::thead/following-sibling::tbody/tr[1]/th[1]';
      ...
      const total_assessed_value_xpath = '//th[text()="Total"]/following-sibling::td[1]';

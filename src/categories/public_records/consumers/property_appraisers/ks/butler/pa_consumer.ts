...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://ww1.bucoks.com/InternetSite/webportal/appraiser/Default.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="MasterContentPlaceHolder_PrimaryUserLogin_PublicAccessButton"]'
    }
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              ...
              console.log(`Looking for owner: ${owner_name}`);
                ...
                address_search = document.propertyId['Property Address'];
                ...
                if(!this.isEmptyOrSpaces(parseaddr.street_address)){
                    ...
                    address_search = parseaddr.street_address;
                }
            ...
            while (true){
              ...
              try {
                let [checkLogin] = await page.$x('//input[@id="MasterContentPlaceHolder_PrimaryUserLogin_PublicAccessButton"]');
                if(checkLogin){
                    await checkLogin.click();
                }
                await page.waitForSelector('input#MasterContentPlaceHolder_ApprSearchOwner_SearchOwnerName');
                if (this.searchBy == 'name'){
                    await page.type('input#MasterContentPlaceHolder_ApprSearchOwner_SearchOwnerName', owner_name, {delay: 150});
                } else {
                    await page.type('input#MasterContentPlaceHolder_ApprSearchAddress_SearchPropertyStreetNum', parsed_addr.number, {delay: 150});
                    await page.type('input#MasterContentPlaceHolder_ApprSearchAddress_SearchPropertyName', parsed_addr.street, {delay: 150});
                    if(parsed_addr.prefix){
                      await page.type('input#MasterContentPlaceHolder_ApprSearchAddress_SearchPropertyDirection', parsed_addr.prefix, {delay: 150});
                    }
                }
                await page.keyboard.press('Enter');
                try{
                    await page.waitForXPath('//table[@class="searchContent"]/tbody/tr', {timeout: 10000});
                } catch(e){
                    console.log('Not found!');
                    break;
                }
                const search_results = await page.$x('//table[@class="searchContent"]/tbody/tr');
                const search_links = await page.$x('//a[contains(@id, "PropertyNumber")]');
                const search_addresses = await page.$x('//span[contains(@id, "PropertyAddress")]/text()[2]');
                let ind = 0;
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        let name = await row.evaluate(el => el.children[1].children[2].children[0].textContent?.trim());
                        const regexp = new RegExp(owner_name_regexp);
                        if (regexp.exec(name!.toUpperCase())){
                            await Promise.all([
                                search_links[ind].click(),
                    ...
                    for(const row of search_results){
                        let address: any = await search_addresses[ind].evaluate(el => el.textContent?.trim());
                        if(this.compareStreetAddress(address, address_search)){
                            await Promise.all([
                                search_links[ind].click(),
                                page.waitForNavigation({waitUntil: 'networkidle0'})
                            ]);
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//span[@id="MasterContentPlaceHolder_ApprOwnersDetails_ResultsRepeater_OwnerName_0"]';
      ...
      // property address
      const property_address_xpath = '//span[@id="MasterContentPlaceHolder_ApprPropertyAddressDetails_ResultsRepeater_PropertyAddress_0"]/text()[1]';
      const property_address_2_xpath = '//span[@id="MasterContentPlaceHolder_ApprPropertyAddressDetails_ResultsRepeater_PropertyAddress_0"]/text()[last()]';
      ...
      // mailing address
      const mailing_address_xpath = '//span[@id="MasterContentPlaceHolder_ApprOwnersDetails_ResultsRepeater_OwnerAddress_0"]/text()[1]';
      const mailing_address_2_xpath = '//span[@id="MasterContentPlaceHolder_ApprOwnersDetails_ResultsRepeater_OwnerAddress_0"]/text()[last()]';
      ...
      // property type
      const property_type_xpath = '//span[@id="MasterContentPlaceHolder_ApprPropertyInfoDetails_ResultsRepeater_PropertyInfoClass_0"]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value = '';
      const est_value_xpath = '//span[@id="MasterContentPlaceHolder_ApprPropertyValuesDetails_ResultsRepeater_ValuesFinalTotal_0"]';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      // sales info"
      let last_sale_recording_date = '';
      let last_sale_amount = '';
      let [salesLink] = await page.$x('//a[@id="MasterContentPlaceHolder_ApprPropertyDeeds_ResultsRepeater_DocumentHyperLink_0"]');
      if(salesLink){
          let link = await page.evaluate(el => el.href, salesLink);
          await page.goto(link);
          try{
            await page.waitForXPath('//table[@class="searchtable"]/tbody/tr[2]/td[5]');
            last_sale_recording_date = await this.getTextByXpathFromPage(page, '//table[@class="searchtable"]/tbody/tr[2]/td[5]'); 
          }catch(e){
              //
          }
      }
      ...
      // year built
      const year_built_xpath = '//*[text()="Year Built:"]/following-sibling::div[1]';

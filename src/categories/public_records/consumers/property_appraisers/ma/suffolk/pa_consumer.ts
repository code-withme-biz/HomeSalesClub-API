...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.cityofboston.gov/assessing/search/'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://www.cityofboston.gov/assessing/search/");
                return false;
            }
            ...
            while (true){
              ...
              try {
                ...
                await page.type('input#q', search_addr, {delay: 150});
                await Promise.all([
                    page.click('input[value="Search parcels"]'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//a[contains(text(), "Details")]');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page, parsed_addr: any): Promise<any> {
        // name
        const full_name_xpath = "//*[contains(text(),'Owner on')]/parent::td/following-sibling::td";
        ...
        // mailing address
        const mailing_address_full_xpath = "//*[contains(text(),'Mailing Address')]/parent::td/following-sibling::td";
        const mailing_address = await this.getTextByXpathFromPage(page, mailing_address_full_xpath);
  
        const mailing_address_parsed = parser.parseLocation(mailing_address);
        let mailing_zip, mailing_city, mailing_state = '';
        ...
        // property type
        const property_type_xpath = "//*[contains(text(),'Property Type')]/parent::td/following-sibling::td";
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
  
        // assessed value and est. value
        const total_assessed_value_xpath = "//*[contains(text(),'Total Assessed Value')]/parent::td/following-sibling::td";
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value = '';
    
        // year built
        const year_built_xpath = '//td[*="Year Built:"]/following-sibling::td[1]';

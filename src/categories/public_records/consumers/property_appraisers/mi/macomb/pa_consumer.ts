...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://bsaonline.com/Home/?uid=259'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    let searchByClick = await page.$x('//a[contains(text(), "By")]');
                    await searchByClick[0].click();
                    await page.click('div#slddi_Name');
                    await this.sleep(1000);
                    await page.type('input#SearchText', search_value, {delay: 150});
                } else {
                    await page.type('input#SearchText', search_value, {delay: 150});
                }
                await Promise.all([
                ...
                let notFoundHandle = await page.$x('//td[text()="No records to display."]');
                if(notFoundHandle.length > 0){
                    console.log("Not found!");
                    break;
                }
                const search_results = await page.$x('//div[@id="DetailGrid"]//table/tbody/tr');
                const datalinks: any = [];
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[0].textContent?.trim());
                            link = "https://bsaonline.com/" + link?.trim();
                            let name = await row.evaluate(el => el.children[8].textContent?.trim());
                            console.log(name);
                ...
                for (let datalink of datalinks) {
                    try{
                        ...
                        let linkdetailhandle = await page.$x('//a[text()="Property Information"]');
                        await Promise.all([
                            linkdetailhandle[0].click(),
                            page.waitForXPath('//th[text()="Property Class"]/parent::tr/td[1]', {visible: true})
                        ]);
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//span[@class="sresult-name"]/text()[last()]';
      ...
      // property address
      const property_address_xpath = '//span[@class="sresult-header1-address"]';
      ...
      // mailing address
      const mailing_address_xpath = '//th[text()="Owner"]/parent::tr/td[1]/text()[2]';
      const mailing_address_2_xpath = '//th[text()="Owner"]/parent::tr/td[1]/text()[last()]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//table[contains(., "Sale Date")]/tbody/tr[1]/td[1]';
      const last_sale_amount_xpath = '//table[contains(., "Sale Date")]/tbody/tr[1]/td[2]';
      ...
      // property type
      const property_type_xpath = '//th[text()="Property Class"]/parent::tr/td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//th[text()="Assessed Value"]/parent::tr/td[last()]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = '';

      // year built
      const year_built_xpath = '//*[contains(@class, "sresult-summary-item")]//td[contains(text(), "Year Built:")]';
      let year_built = await this.getTextByXpathFromPage(page, year_built_xpath);

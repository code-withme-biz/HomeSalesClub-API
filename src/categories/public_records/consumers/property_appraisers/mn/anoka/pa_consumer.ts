...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://prtpublicweb.co.anoka.mn.us/search/commonsearch.aspx?mode=realprop'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://prtpublicweb.co.anoka.mn.us/search/commonsearch.aspx?mode=realprop");
                return false;
            }
            ...
            while (true){
              ...
              try {
                ...
                await page.waitForSelector('input#inpNo');
                if(parsed_addr.number){
                    await page.type('input#inpNo', parsed_addr.number, {delay: 50})
                }
                ...
                let found = await page.$x('//table[@id="Parcel"]');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        ...
        // mailing address
        const mailing_address_xpath = '//td[contains(., "Owner Address:")]/parent::tr/td[2]';
        const mailing_address_2_xpath = '//table[@id="Current Owner"]//td[contains(., "City State Zip:")]/parent::tr/td[2]';
        ...
        // property type
        const property_type_xpath = '//td[contains(., "Property Use Code:")]/parent::tr/td[2]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

        let salesTab = await page.$x('//a[contains(., "Property Transfer History")]');
        await Promise.all([
            salesTab[0].click(),
            page.waitForNavigation({waitUntil: 'networkidle0'})
        ])
        ...
        let valueTab = await page.$x('//a[contains(., "Property Values")]');
        await Promise.all([
            valueTab[0].click(),
            page.waitForNavigation({waitUntil: 'networkidle0'})
        ])
        // assessed value and est. value
        const total_assessed_value_xpath = '//table[@id="Property Values"]/tbody/tr[2]/td[13]';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value = '';
  
        // year built
        const year_built_xpath = '//td[contains(text(), "Year Built")]/following-sibling::td[1]';

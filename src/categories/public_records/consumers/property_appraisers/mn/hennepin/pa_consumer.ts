...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://www16.co.hennepin.mn.us/pins/addrsrch.jsp'
    }

    xpaths = {
        isPAloaded: '//input[@id="INPUT3"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#INPUT3');
                if(parsed_addr.number){
                    await page.type('input#INPUT1', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    await page.type('input#INPUT3', parsed_addr.street, {delay: 50})
                }
                await Promise.all([
                    page.click('input#INPUT4'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//table[@id="TABLE_DETAILS"]');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        // name
        const full_name_xpath = '//td[contains(., "Owner name:")]/parent::tr/td[2]/p';
        ...
        // property address
        const property_address_xpath = '//td[contains(., "Address:")]/parent::tr/td[2]/p';
        ...
        // mailing address
        const mailing_address_xpath = '//td[contains(., "Taxpayer")]/parent::tr/td[2]/table/tbody/tr[2]/td/p';
        const mailing_address_2_xpath = '//td[contains(., "Taxpayer")]/parent::tr/td[2]/table/tbody/tr[last()-1]/td/p';
        ...
        const total_assessed_value_xpath = '//td[contains(., "Taxable market value:")]/parent::tr/td[2]/p';
        ...
        // year built
        const year_built_xpath = '//*[contains(text(), "Construction year:")]/ancestor::td[1]/following-sibling::td[1]';

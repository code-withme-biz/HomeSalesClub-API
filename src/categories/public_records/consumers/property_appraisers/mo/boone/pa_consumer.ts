...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://report.boonecountymo.org/mrcjava/servlet/BC03_MP.mrcMenu'
    }

    xpaths = {
        isPAloaded: '//input[@id="userLogin"]'
    }

    userID = 'devaccess123';
    passwordID = 'devaccess123';
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              ...
              console.log(`Looking for owner: ${owner_name}`);
                ...
                search_address = document.propertyId['Property Address'];
                ...
                if(!this.isEmptyOrSpaces(parseaddr.street_address)){
                    ...
                    search_address = parseaddr.street_address;
                }
            ...
            while (true){
              ...
              try {
                let [checkLogin] = await page.$x('//input[@id="userLogin"]');
                if(checkLogin){
                    await page.type('input#userLogin', this.userID, {delay: 150});
                    await page.type('input[name="mrcpswd"]', this.passwordID, {delay: 150});
                    await page.keyboard.press('Enter');
                }
                await page.waitForXPath('//a[@title="Search Real Property"]');
                let [searchButton] = await page.$x('//a[@title="Search Real Property"]');
                await searchButton.click();
                await page.waitForSelector('input#val_OWNER1');

                if (this.searchBy == 'name'){
                    await page.type('input#val_OWNER1', owner_name, {delay: 150});
                } else {
                    await page.click('a[title="SEARCH BY ADDRESS"]');
                    await page.waitForSelector('input#val_RA_PROPST');
                    await page.type('input#val_RA_PROPST', parsed_addr.street, {delay: 150});
                    await page.type('input#val_RA_PROPNUM', parsed_addr.number, {delay: 150});
                    if(parsed_addr.prefix){
                      await page.type('input#val_RA_PROPDIR', parsed_addr.prefix, {delay: 150});
                    }
                }
                await Promise.all([
                ...
                const search_results = await page.$x('//tbody[@id="mrc_main_table"]/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        ...
                        link = "https://report.boonecountymo.org/mrcjava/servlet/" + link?.trim();
                    ...
                    for(const row of search_results){
                        let link = await row.evaluate(el => el.children[0].children[0].getAttribute('href'));
                        let address: any = await row.evaluate(el => el.children[0].children[0].textContent?.trim());
                        link = "https://report.boonecountymo.org/mrcjava/servlet/" + link?.trim();
                        if(this.compareStreetAddress(address, search_address)){
                            datalinks.push(link);
                            break;
                        }
                    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//strong[text()="Owner"]/ancestor::tr/td[2]';
      ...
      // property address
      const property_address_xpath = '//strong[text()="Property Location"]/ancestor::tr/td[2]';
      ...
      // mailing address
      const mailing_address_xpath = '//strong[text()="Address"]/ancestor::tr/td[2]';
      const mailing_address_2_xpath = '//strong[text()="City, State, Zip"]/ancestor::tr/td[2]/text()[1]';
      ...
      // property type
      const property_type_xpath = '//strong[text()="Use"]/ancestor::tr/td[2]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//th[text()="Current Assessed"]/ancestor::table/tbody/tr[last()]/td[last()]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//th[text()="Current Appraised"]/ancestor::table/tbody/tr[last()]/td[last()]';

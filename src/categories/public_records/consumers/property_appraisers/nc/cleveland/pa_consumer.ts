...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.webgis.net/nc/cleveland/'
    }
              ...
              try {
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        ...
                        link = "https://www.webgis.net/nc/cleveland/PropertyCard.php?pid=" + link?.trim();
                    ...
                    for(const row of search_results){
                        ...
                        if(source == 'Parcel' && this.compareStreetAddress(address, search_value)){
                            await row.click();
                            await page.waitForXPath('//span[text()="Parcel No.:"]/following-sibling::span[1]');
                            let link = await this.getTextByXpathFromPage(page, '//span[text()="Parcel No.:"]/following-sibling::span[1]');
                            if(link != ''){
                                link = "https://www.webgis.net/nc/cleveland/PropertyCard.php?pid=" + link?.trim();
                                datalinks.push(link);
                                break;

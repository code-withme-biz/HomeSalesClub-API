...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://icare.moorecountync.gov/careprd/search/commonsearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'https://icare.moorecountync.gov/careprd/search/commonsearch.aspx?mode=address'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "Name:")]/following-sibling::td[1]';
      ...
      // property type
      const property_type_xpath = '//td[contains(., "Land Use:")]/following-sibling::td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // sales info"
      const last_sale_recording_date_xpath = '//*[@id="Recorded Transaction"]/tbody/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//*[@id="Recorded Transaction"]/tbody/tr[2]/td[4]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      // assessed value and est. value
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//td[contains(., "Total Assessed:")]/following-sibling::td[1]');
      const est_value_xpath = '//td[contains(., "Appraised Total:")]/following-sibling::td[1]'

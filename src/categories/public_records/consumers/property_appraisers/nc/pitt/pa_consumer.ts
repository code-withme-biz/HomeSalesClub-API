...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    footer_selector = '.disclaimer'; // Footer to ensure all element loaded

    mailing_rows_xpath = '//div[contains(@class, "mailing")]/div/text()';
    last_sale_date_xpath = '//div[@id="deed_book_tab"]//table/tbody[1]/tr[1]/td[2]';
    last_sale_amount_xpath = '//div[@id="deed_book_tab"]//table/tbody[1]/tr[1]/td[1]';
    property_type_xpath = '//p[./span[contains(., "Land Class")]]/span[2]';
    ...
    urls = {
        propertyAppraiserPage: 'https://property.spatialest.com/nc/pitt/#/'
    }

    xpaths = {
        isPAloaded: '//input[@id="primary_search"]'
    }
    ...
    async parseResult(page: puppeteer.Page, link: any, document: IOwnerProductProperty, address: any) {
        ...
        let mailing_address = '';
        let mailing_city, mailing_zip, mailing_state;
        ...
        for (let i = 1; i < mailing_rows.length + 1; i++) {
            ...
            try {
                if (parse_addr && parse_addr.number) {
                    mailing_address = ((parse_addr['number'] ? parse_addr['number'] + ' ' : '') +
                        (parse_addr['prefix'] ? parse_addr['prefix'] + ' ' : '') +
                        (parse_addr['street'] ? parse_addr['street'] + ' ' : '') +
                        (parse_addr['type'] ? parse_addr['type'] + ' ' : '') + (parse_addr['sec_unit_num'] ? '#'+parse_addr['sec_unit_num'] : '')).trim();
                    mailing_city = parse_addr.city || '';
                    mailing_zip = parse_addr.zip || '';
                    mailing_state = parse_addr.state || '';
                    index_address = i - 1;
                }
            } catch {
                // pass
        ...
        let property_address, property_city, property_type, last_sale_date, total_assessed_value, last_sale_amount, year_built;
        ...
        try {
            property_city = await page.evaluate(el => el.textContent, (await page.$x('//span[text()="Township"]/parent::p/span[2]'))[0]);
        } catch {
            property_city = '';
        }
        ...
        property_address =
            ((property_address_parsed['number'] ? property_address_parsed['number'] + ' ' : '') +
                ...
                (property_address_parsed['type'] ? property_address_parsed['type'] + ' ' : '') + (property_address_parsed['sec_unit_num'] ? '#'+property_address_parsed['sec_unit_num'] : '')).trim();
        try {
            last_sale_date = '';
        } catch {
            last_sale_date = '';
        ...
        // if (name_and_type.type == 'person') {
        //     let owner_array = name_and_type.name.trim().split(/\s+/g);
        //     owner_last = owner_array ? owner_array.shift() : '';
        //     owner_first = owner_array ? owner_array.shift() : '';
        //     owner_middle = owner_array ? owner_array.shift() : '';
        // } else {
        //     owner_suffix = '';
        // }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        const url_search = 'https://property.spatialest.com/nc/pitt/#/';
        ...
        let doc = docsToParse;
            ...
            await page.waitForSelector(this.footer_selector, { visible: true });

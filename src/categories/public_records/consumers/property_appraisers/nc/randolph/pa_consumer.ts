...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://txpwa.co.randolph.nc.us/publicwebaccess/PersonalPropertySearch.aspx',
    }

    xpaths = {
        isPAloaded: '//*[@name="btnGo"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                await page.waitForXPath('//*[@name="btnGo"]');
              } catch (error) {
                await page.reload();
              }
              try {
                if (this.searchBy === 'name') {
                  await page.select('#lookupCriterion', 'Owner Name');
                  await page.waitForSelector('#txtSearchString', {visible: true});
                  await page.click('input#txtSearchString', {clickCount: 3, delay: 150});
                  await page.type('input#txtSearchString', search_value, {delay: 150});
                } else {
                  await page.select('#lookupCriterion', 'Street Name');
                  await page.waitForSelector('#txtStreetNum', {visible: true});
                  await page.type('input#txtStreetNum', address_house, {delay: 150});
                  await page.type('input#txtSearchString', address_street, {delay: 150});
                }

                let [buttonSearch] = await page.$x('//*[@name="btnGo"]');
                await Promise.all([
                  buttonSearch.click(),
                  page.waitForNavigation()
                ]);

                const [noResult] = await page.$x('//*[contains(text(), "NO DATA FOUND")]');
                if (noResult) {
                  console.log('*** No Results Found!');
                  break;
                }
                                
                const rows = await page.$x('//table[@id="G_dgResults"]/tbody/tr');
                for(const row of rows) {
                  let name = await row.evaluate((el:any) => el.children[2].innerText) || ''; 
                  let address = await row.evaluate((el:any) => el.children[3].innerText) || ''; 
                  let flag = false;
                  if(this.searchBy == 'name') {
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//*[@id="txtName"]';
      ...
      // property address
      const property_address_xpath = '//*[contains(text(), "Property Address:")]/ancestor::td[1]/following-sibling::td[1]';
      ...
      // property type
      const property_type_xpath = '//*[contains(text(), "Abstract Type:")]/ancestor::td[1]/following-sibling::td[1]';
      ...
      const total_assessed_value_xpath = '//*[contains(text(), "Total Assessed Value:")]/ancestor::td[1]/following-sibling::td[1]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[contains(text(), "Total Real Value:")]/ancestor::td[1]/following-sibling::td[1]'

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://sampsonlandrecords.com/PT/search/commonsearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'https://sampsonlandrecords.com/PT/search/commonsearch.aspx?mode=address'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      const mailing_address_2_xpath = '//tr[contains(., "Mailing Address:")]/following-sibling::tr[2]/td[2]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[text()="Sale Price"]/ancestor::tbody[1]/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//td[text()="Sale Price"]/ancestor::tbody[1]/tr[2]/td[4]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      // assessed value and est. value
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//td[text()="Taxable Real Estate:"]/following-sibling::td[1]');
      const est_value_xpath = '//td[text()="Appraised Real Estate Total:"]/following-sibling::td[1]';

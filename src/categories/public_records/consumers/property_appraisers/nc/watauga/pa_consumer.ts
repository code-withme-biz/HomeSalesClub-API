...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://tax.watgov.org/WataugaNC/search/CommonSearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'https://tax.watgov.org/WataugaNC/search/CommonSearch.aspx?mode=address'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // property address
      const property_address_xpath = '//td[contains(text(), "PARID")]/ancestor::tbody[1]/tr[3]/td[2]';
      ...
      let property_zip = property_address_2_arr.pop()?.trim();
      let property_state = property_address_2_arr.pop()?.trim();
      let property_city = property_address_2_arr.pop()?.trim();
      ...
      const mailing_address_2_xpath = '//table[@id="Owner Mailing"]//td[text()="City, State, Zip"]/parent::tr/td[2]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[text()="Sale Date"]/ancestor::tbody[1]/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//td[text()="Sale Date"]/ancestor::tbody[1]/tr[2]/td[4]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      // assessed value and est. value
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//td[text()="Taxable Total"]/following-sibling::td[1]');
      const est_value_xpath = '//td[text()="Appraisal Total"]/following-sibling::td[1]';

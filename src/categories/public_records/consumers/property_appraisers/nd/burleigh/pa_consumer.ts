    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: http://burleigh.northdakotaassessors.com/search.php");
                return false;
            ...
            while (true){
              ...
              try {
                ...
                if(found.length < 1){
                    let [linkOne] = await page.$x('//a[contains(@href, "parcel.php")]');
                    if(linkOne){
                        await Promise.all([
                            linkOne.click(),
                            page.waitForNavigation({waitUntil: 'networkidle0'})

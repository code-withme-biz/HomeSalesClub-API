...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://williams.northdakotaassessors.com/search.php'
    }
            ...
            while (true){
              ...
              try {
                ...
                for(const row of search_results){
                  ...
                  link = "https://williams.northdakotaassessors.com/" + link?.trim();
                  let name = await row.evaluate(el => el.children[2].textContent?.trim());
                  let address: any = await row.evaluate(el => el.children[4].textContent?.trim());

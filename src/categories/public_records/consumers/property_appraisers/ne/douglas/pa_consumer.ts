...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://douglascone.wgxtreme.com/'
    }

    xpaths = {
        isPAloaded: '//button[text()="I agree to the terms of use"]'
    }
            ...
            while (true){
              ...
              try {        
                ...
                if(this.searchBy == 'address'){
                    const body = 'start=0&limit=25&address_searchA='+search_value+'&database=douglascone&useParcelPoints=true&sort=1&dir=ASC';
                    api_response = await axios.post("https://douglascone.wgxtreme.com/java/wgx_taxsearch/taxlist.action", body);

                } else {
                    const body = 'start=0&limit=25&owner_name_searchA='+search_value+'&database=douglascone&useParcelPoints=true&sort=1&dir=ASC';
                    api_response = await axios.post("https://douglascone.wgxtreme.com/java/wgx_taxsearch/taxlist.action", body);
                }
                const datalinks = [];
                if(api_response.status === 200){
                    const resultList = api_response.data.resultList;
                    ...
                    if(this.searchBy == 'name'){
                        for(const searchResult of resultList){
                            const parcelNumber = searchResult.parcelid;
                            const name = searchResult.owner_name;
                            const detail_link = `https://douglascone.wgxtreme.com/java/wgx_douglasne/static/accountinfo.jsp?ext=false&accountno=R${parcelNumber}`;
                            const regexp = new RegExp(owner_name_regexp);
                            if (!regexp.exec(name.toUpperCase())) continue;
                            datalinks.push(detail_link);
                        }
                    } else {
                        const searchResult = resultList[0];
                        const parcelNumber = searchResult.parcelid;
                        const detail_link = `https://douglascone.wgxtreme.com/java/wgx_douglasne/static/accountinfo.jsp?ext=false&accountno=R${parcelNumber}`;
                        datalinks.push(detail_link);
                    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//th[contains(., "Owner")]/ancestor::table/tbody/tr[1]/td[1]/text()[1]';
      ...
      // property address
      const property_address_xpath = '//th[contains(., "Parcel Address")]/parent::tr/td[1]/text()[1]';
      const property_address_2_xpath = '//th[contains(., "Parcel Address")]/parent::tr/td[1]/text()[last()]';
      ...
      // mailing address
      const mailing_address_xpath = '//th[contains(., "Owner")]/ancestor::table/tbody/tr[1]/td[1]/text()[last()-1]';
      const mailing_address_2_xpath = '//th[contains(., "Owner")]/ancestor::table/tbody/tr[1]/td[1]/text()[last()]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//th[contains(., "Sales Date:")]/parent::tr/td[1]';
      const last_sale_amount_xpath = '//th[contains(., "Price:")]/parent::tr/td[1]';
      ...
      // property type
      const property_type_xpath = '//th[contains(., "Account Type:")]/parent::tr/td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//th[contains(., "Total")]/ancestor::table/tbody/tr[1]/td[4]';

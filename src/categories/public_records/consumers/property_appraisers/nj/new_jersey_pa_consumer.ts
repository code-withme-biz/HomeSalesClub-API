...
export default abstract class NewJerseyPAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://tax1.co.monmouth.nj.us/cgi-bin/prc6.cgi?&ms_user=monm&passwd=data&srch_type=0&adv=0&out_type=0&district=0400'
    }

    xpaths = {
        isPAloaded: '//input[@name="p_loc"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForXPath('//input[@name="p_loc"]');
                if(this.searchBy == 'name'){
                    await page.type('input[name="owner"]', search_value, {delay: 150});
                } else {
                    await page.type('input[name="p_loc"]', search_value, {delay: 150});
                }
                let [submitButton] = await page.$x('//input[@value="Submit Search"]');
                await Promise.all([
                    submitButton.click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let searchResults = await page.$x('//table/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of searchResults){
                        ...
                        link = "https://tax1.co.monmouth.nj.us/cgi-bin/" + link;
                        let name = await row.evaluate(el => el.children[6].children[0].textContent?.trim());
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "Owner:")]/ancestor::tr/td[6]/font';
      ...
      // mailing address
      const mailing_address_xpath = '//font[contains(text(), "Owner Information")]/ancestor::tbody/tr[4]/td[2]/font';
      const mailing_address_2_xpath = '//font[contains(text(), "Owner Information")]/ancestor::tbody/tr[5]/td[2]/font';
      ...
      const last_sale_amount_xpath = '//font[contains(text(), "Sale Date")]/ancestor::tr/td[6]/font[1]';
      ...
      // property type
      const property_type_xpath = '//td[contains(., "Tax Codes")]/ancestor::tr/td[8]/font';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.tax.co.ocean.nj.us/frmTaxBoardTaxListSearch'
    }

    xpaths = {
        isPAloaded: '//input[@id="MainContent_txtStreet"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#MainContent_txtStreet');
                if (this.searchBy == 'name'){
                    await page.type('input#MainContent_txtOwner', search_value, {delay: 150});
                } else {
                    await page.type('input#MainContent_txtStreet', search_value, {delay: 150});
                }
                let buttonSearch = await page.$x('//input[@id="MainContent_btnSearch"]');
                await Promise.all([
                    buttonSearch[0].click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                
                const search_results = await page.$x('//table[@id="MainContent_m_DataTable"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            ...
                            link = "https://www.tax.co.ocean.nj.us/" + link?.trim();
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // property address
      const property_address_xpath = '//td[text()="Location:"]/parent::tr/td[2]';
      ...
      const mailing_address_2_xpath = '//td[text()="City/State:"]/parent::tr/td[2]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[text()="Deed date:"]/parent::tr/td[4]';
      ...
      const total_assessed_value_xpath = '//table[@id="MainContent_AssmtHistTable"]/tbody/tr[3]/td[5]';

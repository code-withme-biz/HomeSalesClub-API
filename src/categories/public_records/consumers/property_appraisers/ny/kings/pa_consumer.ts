...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://a836-pts-access.nyc.gov/care/search/commonsearch.aspx?mode=address'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://a836-pts-access.nyc.gov/care/search/commonsearch.aspx?mode=address");
                return false;
            }
            ...
            while (true){
              ...
              try {
                ...
                await page.waitForSelector('input#inpStreet');
                if(parsed_addr.number){
                    await page.type('input#inpNumber', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    let addr_search = (parsed_addr.street + ' ' + (parsed_addr.type ? parsed_addr.type : '')).trim();
                    await page.type('input#inpStreet', addr_search, {delay: 50})
                }
                await Promise.all([
                    page.click('button#btSearch'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//table[@id="Property Owner(s)"]');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        let assessmentTab = await page.$x('//div[@id="sidemenu"]/li[contains(., "Final")]');
        await Promise.all([
          assessmentTab[0].click(),
          page.waitForNavigation({waitUntil: 'networkidle0'})
        ])

        // name
        const full_name_xpath = '//td[contains(., "Owner Name")]/parent::tr/td[2]';
        ...
        // property address
        const property_address_xpath = '//td[contains(., "Property Address")]/parent::tr/td[2]';
        ...
        // property type
        const property_type_xpath = '//td[contains(., "Building Class")]/parent::tr/td[2]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
  
        // mailing address
        const mailing_address_xpath = '//table[contains(@id, "Final Assessment")]/tbody/tr[8]/td[2]';
        const mailing_address_2_xpath = '//table[contains(@id, "Final Assessment")]/tbody/tr[9]/td[2]';
        ...
        const est_value_xpath = '//td[contains(., "ESTIMATED MARKET VALUE")]/parent::tr/td[4]'

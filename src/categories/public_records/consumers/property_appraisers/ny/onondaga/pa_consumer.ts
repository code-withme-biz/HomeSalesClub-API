...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://ocfintax.ongov.net/Imate/index.aspx',
    }

    xpaths = {
        isPAloaded: '//*[@id="btnPublicAccess"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                // public access
                await page.waitForXPath('//*[@id="btnPublicAccess"]');
                const [publicAccess] = await page.$x('//*[@id="btnPublicAccess"]');
                if (publicAccess) {
                  await Promise.all([
                    publicAccess.click(),
                ...
                await page.waitForXPath('//*[@id="chkAgree"]');
                const [agreeCheckbox] = await page.$x('//*[@id="chkAgree"]');
                await agreeCheckbox.click();
                await this.sleep(100);
                const [agreeButton] = await page.$x('//*[@id="btnSubmit"]');
                await Promise.all([
                  agreeButton.click(),
                  page.waitForNavigation()
                ]);
                await page.waitForXPath('//*[@id="btnSearch"]');
              } catch (error) {
                await page.reload();
              }
              try {
                if (this.searchBy === 'name') {
                  if (last_name)
                    await page.type('input#txtLastOwner', last_name, {delay: 150});
                  if (first_name)
                    await page.type('input#txtFirstOwner', first_name, {delay: 150});
                } else {
                  if (address_house && address_street) {
                    await page.type('input[name="txtStreetNum"]', address_house, {delay: 150});
                    await page.type('input[name="txtStreetName"]', address_street, {delay: 150});
                  } else {
                    break;
                  }
                }
                ...
                await Promise.all([
                  page.waitForXPath('//*[@id="zerocount"]'),
                  page.waitForXPath('//*[@id="tblList"]')
                ]);
                const [noResult] = await page.$x('//*[@id="zerocount"]');
                if (noResult) {
                ...
                for(const row of rows) {
                  ...
                  let addrnumber = await row.evaluate(el => el.children[4].textContent?.trim()) || ''; 
                  let addrstreet = await row.evaluate(el => el.children[5].textContent?.trim()) || ''; 
                  let address = addrnumber + addrstreet;
                  let flag = false;
                  if(this.searchBy == 'name') {
                  ...
                  if (flag) {
                    ...
                    await detailPage.goto(`https://ocfintax.ongov.net/Imate/${link}`, {waitUntil: 'load'});
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // property type
      const property_type_xpath = '//*[contains(text(), "Property Class:")]/following-sibling::td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//*[contains(text(), "Total Assessment:")]/following-sibling::td[1]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[contains(text(), "Full Market Value:")]/following-sibling::td[1]'
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);


      // owner/sales info
      await Promise.all([
        page.click('input[value="Owner/Sales"]'),
        page.waitForNavigation()
      ]);
      // name
      const full_name_xpath = '//*[@id="tblOwnerShip"]/tbody/tr[2]/td[1]';
      ...
      // mailing address
      const mailing_address_xpath = '//*[@id="tblOwnerShip"]/tbody/tr[2]/td[3]';
      ...
      // last saling info
      const last_sale_recording_date_xpath = '//*[@id="tblSales"]/tbody/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//*[@id="tblSales"]/tbody/tr[2]/td[2]'

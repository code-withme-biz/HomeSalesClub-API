...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://auditor.ashtabulacounty.us/PT/search/commonsearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'https://auditor.ashtabulacounty.us/PT/search/commonsearch.aspx?mode=address'
    }
            ...
            while (true){
              ...
              try {
                ...
                for(let i = 0; i < rows.length; i++) {
                    ...
                    let address = await row.evaluate(el => el.children[3].children[0].textContent?.trim()) || ''; 
                    if(this.searchBy == 'name') {
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      let [assessTab] = await page.$x('//a[contains(., "Values")]');
      if(assessTab){
        await assessTab.click();
        await page.waitForXPath('//tr[@id="datalet_header_row"]');
      }
      // assessed value and est. value
      const total_assessed_value_xpath = '//td[text()="Assessed Total"]/following-sibling::td[1]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//td[text()="Appraised Total"]/following-sibling::td[1]';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      let [salesTab] = await page.$x('//a[./span[text()="Sales"]]');
      if(salesTab){

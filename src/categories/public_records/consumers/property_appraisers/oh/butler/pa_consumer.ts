...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://propertysearch.butlercountyauditor.org/search/commonsearch.aspx?mode=OWNER',
        propertyAppraiserPageAddress: 'https://propertysearch.butlercountyauditor.org/search/commonsearch.aspx?mode=ADDRESS'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      let salesTab = await page.$x('//a[contains(., "Transfers")]');
      await Promise.all([
        salesTab[0].click(),
        page.waitForNavigation()
      ])
      // sales info"
      const last_sale_recording_date_xpath = '//table[contains(@id, "Transfers")]/tbody/tr[2]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//table[contains(@id, "Transfers")]/tbody/tr[2]/td[3]';
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      let valueTab = await page.$x('//a[contains(., "Value History")]');
      await Promise.all([
        valueTab[0].click(),
        page.waitForNavigation()
      ])
      // assessed value and est. value
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//td[text()="Total Appraised Value"]/ancestor::tbody/tr[2]/td[7]');
      const est_value_xpath = '//td[text()="Total Appraised Value"]/ancestor::tbody/tr[2]/td[4]';

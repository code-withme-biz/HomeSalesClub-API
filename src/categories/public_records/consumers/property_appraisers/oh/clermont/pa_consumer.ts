...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://www.clermontauditorrealestate.org/_web/search/commonsearch.aspx?mode=OWNER',
        propertyAppraiserPageAddress: 'https://www.clermontauditorrealestate.org/_web/search/commonsearch.aspx?mode=ADDRESS'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (!hasResult) {
                  let [found] = await page.$x('//td[contains(., "Owner(s):")]/following-sibling::td[1]');
                  if(found){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "Mailing Name 1")]/following-sibling::td[1]';
      ...
      // property address
      const property_address_xpath = '//td[contains(., "Address")]/following-sibling::td[1]';
      ...
      // mailing address
      const mailing_address_xpath = '//td[contains(., "Address 1")]/following-sibling::td[1]';
      const mailing_address_2_xpath = '//td[contains(., "Address 3")]/following-sibling::td[1]';
      ...
      let salesTab = await page.$x('//a[contains(., "Sales/Transfers")]');
      await Promise.all([
        salesTab[0].click(),
        page.waitForNavigation({waitUntil: 'networkidle0'})
      ])
      // sales info"
      const last_sale_recording_date_xpath = '//td[contains(., "Sale Date")]/following-sibling::td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//td[contains(., "Sale Amount")]/following-sibling::td[1]';
      ...
      const total_assessed_value = await this.getTextByXpathFromPage(page, '//table[contains(@id, "Assessed Value")]/tbody/tr[3]/td[2]');
      const est_value_xpath = '//table[contains(@id, "Appraised Value")]/tbody/tr[3]/td[2]'

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'https://delaware-auditor-ohio.manatron.com/OwnerSearch.aspx',
        propertyAppraiserPageAddress: 'http://delaware-auditor-ohio.manatron.com/AddressSearch.aspx'
    }

    xpaths = {
        isPAloaded: '//*[@name="btnSearch"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                await page.waitForXPath('//*[@name="btnSearch"]');
              } catch (error) {
                await page.reload();
              }
              try {
                if (this.searchBy === 'name') {
                  ...
                  await page.type('input#HouseNoLow', address_house, {delay: 150});
                  await page.type('input#Street', address_street, {delay: 150});
                }

                let [buttonSearch] = await page.$x('//*[@name="btnSearch"]');
                await Promise.all([
                  buttonSearch.click(),
                ...
                if (url.indexOf('mpropertynumber') > -1) {
                  let result = await this.getPropertyInfos(page);
                  if (result) await this.parseResult(result, document);
                  break;
                }

                const [noResult] = await page.$x('//*[contains(text(), "No Results Found")]');
                if (noResult) {
                  console.log('*** No Results Found!');
                  break;
                }
                                
                const rows = await page.$x('//*[contains(text(), "Property Location")]/ancestor::table[1]/tbody/tr[position()>1]');
                for(const row of rows) {
                  let content = await row.evaluate((el:any) => el.innerText) || ''; 
                  let flag = false;
                  if(this.searchBy == 'name') {
                    const regexp = new RegExp(owner_name_regexp);
                    if (regexp.exec(content.toUpperCase())){
                      flag = true;
                    }
                  } else {
                    if(this.compareStreetAddress(content, search_value)){
                      flag = true;
                    }
                  }
                  if (flag) {
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//*[contains(text(), "Owner Name")]/following-sibling::td[1]';
      ...
      // property address
      const property_address_xpath = '//*[contains(text(), "Property Address:")]/ancestor::tr[1]/following-sibling::tr[1]/td[1]';
      ...
      // mailing address
      const mailing_address_xpath = '//*[contains(text(), "Owner Address")]/following-sibling::td[1]';
      ...
      // property type
      const property_type_xpath = '//*[contains(text(), "Use Code")]/following-sibling::td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      const year_built = await this.getTextByXpathFromPage(page, '//*[contains(text(), "Year Built")]/following-sibling::td[1]');

      // sales info"
      await Promise.all([
        page.click('a[href*="ValueandTransfer"]]'),
        page.waitForNavigation()
      ]);
      await page.waitForXPath('//*[text()="Sale Amount"]/ancestor::table[1]');
      const last_sale_recording_date_xpath = '//*[text()="Sale Amount"]/ancestor::table[1]/tbody/tr[1]/td[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//*[text()="Sale Amount"]/ancestor::table[1]/tbody/tr[1]/td[2]'
      const last_sale_amount = await this.getTextByXpathFromPage(page, last_sale_amount_xpath);

      // assessed value and est. value
      await Promise.all([
        page.click('a[href*="ValueHistory"]'),
        page.waitForNavigation()
      ]);
      await page.waitForXPath('//*[text()="Reason"]/ancestor::table[1]');
      const total_assessed_value_xpath = '//*[text()="Reason"]/ancestor::table[1]/tbody/tr[1]/td[5]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[text()="Reason"]/ancestor::table[1]/tbody/tr[1]/td[3]'

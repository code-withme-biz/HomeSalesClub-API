...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.mcohio.org/government/elected_officials/treasurer/mctreas/search.cfm'
    }

    xpaths = {
        isPAloaded: '//input[@name="streetname"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input[name="streetname"]');
                if (this.searchBy == 'name'){
                    await page.type('input[name="own1"]', owner_name, {delay: 150});
                } else {
                    if(parsed_addr.number){
                        await page.type('input[name="streetnbr"]', parsed_addr.number, {delay: 150});
                    }
                    if(parsed_addr.street){
                        await page.type('input[name="streetname"]', parsed_addr.street, {delay: 150});
                    }
                ...
                let buttonSearch = await page.$x('//input[@name="SEARCH"]');
                await buttonSearch[0].click();
                // await page.waitForXPath('//table[@width="96%"]', {visible: true})
                try{
                    await Promise.race([
                        page.waitForXPath('//table[@width="96%"]'),
                        page.waitForXPath('//td[text()="Owner 1"]')
                    ])
                } catch(e){
                    console.log('Not found!');
                    break;
                }
                const search_results = await page.$x('//th[text()="Parcel ID"]/ancestor::tbody/tr');
                if(search_results.length < 1){
                    let foundOne = await page.$x('//td[text()="Owner 1"]');
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // mailing address
      const mailing_address_xpath = '//td[text()="Address 1"]/parent::tr/td[2]';
      const mailing_address_2_xpath = '//td[text()="Address 3"]/parent::tr/td[2]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//tbody[contains(., "Sales")]/tr[2]/td[2]';
      const last_sale_amount_xpath = '//tbody[contains(., "Sales")]/tr[3]/td[2]';
      ...
      // property type
      const property_type = await this.getTextByXpathFromPage(page, '//td[text()="Class"]/parent::tr/td[2]');

      // assessed value and est. value
      const total_assessed_value_xpath = '//tbody[contains(., "Current Values")]/tr[contains(., "Total")]/td[3]';

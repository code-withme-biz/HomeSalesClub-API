...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://fiscaloffice.summitoh.net/PropertyTaxValues/ThothRefintg.htm'
    }

    xpaths = {
        isPAloaded: '//input[@name="addr"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input[name="addr"]');
                if (this.searchBy == 'name'){
                    await page.type('input[name="own"]', search_value, {delay: 150});
                } else {
                    await page.type('input[name="addr"]', search_value, {delay: 150});
                }
                ...
                const search_results = await page.$x('//table[contains(., "PARCEL")]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[1].children[0].getAttribute('value'));
                            link = "http://fiscaloffice.summitoh.net/pls/apex/thothrefstrt.do_one?parcel=" + link?.trim() + "&print=1&lienyear=" + currentYear + "&doland=ON&docard=ON&cardnum=ALL&dosales=ON&dopermits=ON&dotaxes=ON&doassess=ON&docauv=OFF&donotes=ON&dogeninf=OFF";
                    ...
                    let link = await search_results[0].evaluate(el => el.children[1].children[0].getAttribute('value'));
                    link = "http://fiscaloffice.summitoh.net/pls/apex/thothrefstrt.do_one?parcel=" + link?.trim() + "&print=1&lienyear=" + currentYear + "&doland=ON&docard=ON&cardnum=ALL&dosales=ON&dopermits=ON&dotaxes=ON&doassess=ON&docauv=OFF&donotes=ON&dogeninf=OFF";
                    datalinks.push(link);
                }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "OWNER")]/parent::tr/td[2]';
      ...
      // property address
      const property_address_full_xpath = '//td[contains(., "ADDR.")]/parent::tr/td[2]';
      const property_address_full = await this.getTextByXpathFromPage(page, property_address_full_xpath);
      const property_address_arr = property_address_full.split(",");
      let property_address = property_address_arr[0].trim();
      let property_address_parsed = parser.parseLocation(property_address);
      let property_address_2 = property_address_arr[1].trim();
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[contains(., "DATE")]/parent::tr/td[1]/font/text()[1]';
      const last_sale_amount_xpath = '//td[contains(., "DATE")]/parent::tr/td[4]/font/text()[1]';
      ...
      // property type
      const property_type_xpath = '//tr[contains(., "DESC.")]//font[@size="-2"]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//td[contains(., "TOTAL:")]/parent::tr/td[last()-1]/font';

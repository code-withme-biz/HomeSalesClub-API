...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.actdatascout.com/RealProperty/Oklahoma/Pottawatomie'
    }

    xpaths = {
        isPAloaded: '//input[@id="FirstName"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    await page.click('input#FirstName', {clickCount: 3})
                    await page.type('input#FirstName', first_name, {delay: 150});
                    await page.click('input#LastName', {clickCount: 3})
                    await page.type('input#LastName', last_name, {delay: 150});
                } else {
                    ...
                    if(parsed_addr.prefix){
                      try{
                        let option = (await page.$x('//select[@id="StreetDirection"]/option[text()="'+ parsed_addr.prefix + '"]'))[0];
                        let optionVal: any = await (await option.getProperty('value')).jsonValue();
                        await page.select('#StreetDirection', optionVal);
                      } catch(e){
                        //
                      }
                    }
                ...
                await Promise.race([
                    page.waitForXPath('//div[contains(@class, "bg-danger") and contains(., "No results found")]'),
                    page.waitForXPath('//table[@id="RealPropertyResultsTable"]/tbody/tr')
                ]);
                const search_results = await page.$x('//table[@id="RealPropertyResultsTable"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        ...
                        if (regexp.exec(name!.toUpperCase())){
                            let link = await page.$x('//table[@id="RealPropertyResultsTable"]/tbody/tr['+ind+']/td[1]/form/button');
                            await link[0].click();
                            await page.waitForXPath('//td[text()="Name:"]/parent::tr/td[2]');
                            break;
                        }
                        ind++;
                    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // property address
      const property_address_xpath = '//td[text()="Physical Address:"]/parent::tr/td[2]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//tr[@id="DocumentLinks"]/td[1]';
      const last_sale_amount_xpath = '//tr[@id="DocumentLinks"]/td[6]';

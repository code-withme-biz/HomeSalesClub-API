...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://dial.deschutes.org/Search/General'
    }

    xpaths = {
        isPAloaded: '//input[@id="value"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    await page.type('input#value', search_value, {delay: 150});
                } else {
                    await page.type('input#value', search_value, {delay: 150});
                }
                ...
                const search_results = await page.$x('//table[@id="uxResults"]/tbody/tr');
                if(search_results.length < 1){
                    let [checkFoundOne] = await page.$x('//strong[text()="Mailing To:"]/following-sibling::text()[1]');
                    if (checkFoundOne){
                ...
                if(this.searchBy == 'name'){
                    ...
                    let address = await search_results[0].evaluate(el => el.children[0].children[0].getAttribute('href'));
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//strong[text()="Mailing To:"]/following-sibling::text()[1]';
      ...
      // property address
      const property_address_full_xpath = '//span[@id="uxSitusAddress"]';
      ...
      let property_zip = property_address_full_arr[2].trim().split(/\s+/)[1].trim();
      let property_state = property_address_full_arr[2].trim().split(/\s+/)[0].trim();
      let property_address_parsed = parser.parseLocation(property_address);

      // mailing address
      const mailing_address_xpath = '//strong[text()="Mailing To:"]/following-sibling::text()[last() - 2]';
      const mailing_address_2_xpath = '//strong[text()="Mailing To:"]/following-sibling::text()[last() - 1]';
      ...
      // property type
      const property_type_xpath = '//strong[text()="Property Class:"]/following-sibling::text()';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//td[contains(., "Assessed Value")]/following-sibling::td';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//tr[@class="totals"]/td[2]';
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      let [salesButton] = await page.$x('//a[text()="Sales"]');
      await Promise.all([
          salesButton.click(),
          page.waitForNavigation({waitUntil: 'networkidle0'})
      ])
      // sales info"
      const last_sale_recording_date_xpath = '//th[text()="Sale Amount"]/ancestor::table/tbody/tr[2]/td[1]';
      const last_sale_amount_xpath = '//th[text()="Sale Amount"]/ancestor::table/tbody/tr[2]/td[4]';

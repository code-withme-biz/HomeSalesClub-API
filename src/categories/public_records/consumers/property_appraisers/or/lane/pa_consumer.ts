...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://apps.lanecounty.org/TaxStatement/Search.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="SearchAccount_Account"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://apps.lanecounty.org/TaxStatement/Search.aspx");
                return false;
            }
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#SearchAccount_Account');
                await page.click('input#SearchOptions_2');
                await page.waitForSelector('input#SearchAddress_StreetAddress');
                await page.type('input#SearchAddress_StreetAddress', search_addr, {delay: 150});
                await Promise.all([
                    page.click('input#SearchAddress_Search'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//table[@id="ResultList"]/tbody/tr[2]/td[1]/a');
                if(found.length < 1){
                    console.log("Not found!");
                    break;
                }
                let jsDetail: any = await found[0].evaluate(el => el.getAttribute('href'));
                const regex = /"(.*?)","(.*?)"/gm;
                let m, id, year;
                while ((m = regex.exec(jsDetail?.trim())) !== null) {
                ...
                let link = "https://apps.lanecounty.org/TaxStatement/Statement.aspx?a="+id+"&y="+year;
    ...
    async getPropertyInfos(page: puppeteer.Page, parsed_addr: any): Promise<any> {
        // name
        const full_name_xpath = '//span[@id="MailAddress"]/text()[1]';
        ...
        // mailing address
        const mailing_address_xpath = '//span[@id="MailAddress"]/text()[last()-1]';
        const mailing_address_2_xpath = '//span[@id="MailAddress"]/text()[last()]';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPageOwner: 'http://tyler.washcopa.org/pt/search/commonsearch.aspx?mode=owner',
        propertyAppraiserPageAddress: 'http://tyler.washcopa.org/pt/search/commonsearch.aspx?mode=address'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (!hasResult) {
                  let [found] = await page.$x('//td[contains(., "Name")]/following-sibling::td[1]');
                  if(found){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      // property address
      const property_address_xpath = '//td[text()="Property Address"]/parent::tr/td[2]';
      ...
      let property_city = await this.getTextByXpathFromPage(page, '//td[text()="Property City/State"]/parent::tr/td[2]');
      let property_zip = await this.getTextByXpathFromPage(page, '//td[text()="Property Zip Code"]/parent::tr/td[2]');
      
      // mailing address
      const mailing_address_xpath = '//td[text()="Address"]/parent::tr/td[2]/text()[1]';
      ...
      const mailing_address_2_xpath = '//td[text()="Address"]/parent::tr/td[2]/text()[last()]';
      ...
      let [assessTab] = await page.$x('//a[contains(., "Assessment/Sales")]');
      if(assessTab){
          await Promise.all([
              assessTab.click(),
              page.waitForNavigation({waitUntil: 'networkidle0'})

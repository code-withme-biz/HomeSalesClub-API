...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://berkeleycountysc.gov/propcards/prop_card_search.php'
    }

    xpaths = {
        isPAloaded: '//input[@name="streetnum"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy == 'name'){
                    await page.type('input[name="ownername"]', owner_name, {delay: 150});
                } else {
                    let street =((address.street ? address.street : '') + ' ' + (address.type ? address.type : '')).trim();
                    if(address.number){
                        await page.type('input[name="streetnum"]', address.number, {delay: 150});
                    }
                    if(address.street){
                        await page.type('input[name="streetname"]', street, {delay: 150});
                    }
                }
                ...
                const search_results = await page.$x('//table/tbody/tr');
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//b[text()="Owner Information:"]/ancestor::td/table/tbody/tr[1]/td[1]/text()[1]';
      ...
      // property address
      const property_address_xpath = '//b[text()="Site addresses:"]/ancestor::table/tbody/tr[1]/td[1]/text()[1]';
      const property_address_2_xpath = '//b[text()="Site addresses:"]/ancestor::table/tbody/tr[1]/td[1]/text()[last()]';
      ...
      property_zip = property_zip?.replace(',','').trim();
      property_city = property_city.replace(',','').trim();

      // mailing address
      const mailing_address_xpath = '//b[text()="Owner Information:"]/ancestor::td/table/tbody/tr[1]/td[1]/text()[2]';
      const mailing_address_2_xpath = '//b[text()="Owner Information:"]/ancestor::td/table/tbody/tr[1]/td[1]/text()[last()]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//b[text()="Last Sale Date:"]/parent::td/text()[1]';
      const last_sale_amount_xpath = '//b[text()="Last Sale Date:"]/parent::td/text()[5]';
      ...
      const total_assessed_value_xpath = '//b[text()="Total Taxable Value:"]/parent::td/text()[1]';

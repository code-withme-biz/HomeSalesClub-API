...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://as400.dorchestercounty.net/webapps/CAMA001A.pgm'
    }

    xpaths = {
        isPAloaded: '//input[@name="f_STREETNO"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input[name="f_STREETNO"]');
                if (this.searchBy == 'name'){
                    await page.click('input[name="f_NAME1"]', {clickCount: 3});
                    await page.type('input[name="f_NAME1"]', owner_name, {delay: 150});
                } else {
                    if(address.number){
                        await page.click('input[name="f_STREETNO"]', {clickCount: 3});
                        await page.type('input[name="f_STREETNO"]', address.number, {delay: 150});
                    }
                    if(address.street){
                        await page.click('input[name="f_STREETNAME"]', {clickCount: 3});
                        await page.type('input[name="f_STREETNAME"]', address.street, {delay: 150});
                    }
                }
                await Promise.all([
                ...
                const search_results = await page.$x('//table[@id="list-table"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[0].children[0].children[0].getAttribute('href'));
                            link = "https://as400.dorchestercounty.net/webapps/" + link?.trim();
                    ...
                    let link = await search_results[0].evaluate(el => el.children[0].children[0].children[0].getAttribute('href'));
                    link = "https://as400.dorchestercounty.net/webapps/" + link?.trim();
                    datalinks.push(link);
                }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//td[contains(., "Owner") and @bgcolor="lightgrey"]/parent::tr/td[2]/font';
      ...
      // property address
      const property_address_xpath = '//td[contains(., "Parcel Address") and @bgcolor="lightgrey"]/parent::tr/td[2]/font';
      ...
      // mailing address
      const mailing_address_xpath = '//td[contains(., "Owner Address") and @bgcolor="lightgrey"]/parent::tr/td[2]/font/text()[1]';
      const mailing_address_2_xpath = '//td[contains(., "Owner Address") and @bgcolor="lightgrey"]/parent::tr/td[2]/font/text()[last()]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//td[contains(., "Sale Date") and @bgcolor="lightgrey"]/parent::tr/td[2]/font';
      const last_sale_amount_xpath = '//td[contains(., "Sale Price") and @bgcolor="lightgrey"]/parent::tr/td[2]/font';
      ...
      const total_assessed_value_xpath = '//table[contains(., "Total Land & Improvements")]/tbody/tr[2]/td[4]/font/span';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = '';

      // property type
      await Promise.all([
          page.click('button#detailbutton'),
          page.waitForNavigation({waitUntil:'networkidle0'})
      ])

      const property_type_xpath = '//table[@id="list-table"]/tbody/tr[2]/td[3]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      const year_built = await this.getTextByXpathFromPage(page, '//table[@id="list-table"]/tbody/tr[2]/td[last()]');

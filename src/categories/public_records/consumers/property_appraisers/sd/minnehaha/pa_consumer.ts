...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.govtechtaxpro.com/parceldetail.php?idCounty=42099'
    }

    xpaths = {
        isPAloaded: '//input[@id="ownername"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                const search_results = await page.$x('//div[@id="searchresults"]//table/tbody/tr[@style="background-color: #ddd;"]');
                const search_results_id = await page.$x('//div[@id="searchresults"]//table/tbody/tr[contains(@id, "groupdetails")]');
                ...
                const datalinks: any = [];
                let ind = 0;
                const regex_id = /groupdetails(.*)/gm;
                let m;
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let name = await row.evaluate(el => el.children[0].textContent?.trim());
                            let groupid = await search_results_id[ind].evaluate(el => el.id);
                            let id = '';
                            while ((m = regex_id.exec(groupid)) !== null) {
                            ...
                            let link = 'https://www.govtechtaxpro.com/modules/parceldetailpopup.php?id='+id;
                            console.log(link);
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//tr[contains(., "Owner Address")]/parent::tbody/tr[2]/td[1]';
      ...
      // property address
      const property_address_xpath = '//tr[contains(., "Owner Address")]/parent::tbody/tr[2]/td[3]/text()[1]';
      ...
      // mailing address
      const mailing_address_xpath = '//tr[contains(., "Owner Address")]/parent::tbody/tr[2]/td[2]/text()[1]';
      const mailing_address_2_xpath = '//tr[contains(., "Owner Address")]/parent::tbody/tr[2]/td[2]/text()[last()]';
      ...
      // property type
      const property_type_xpath = '//tr[contains(., "Parcel Number")]/parent::tbody/tr[2]/td[5]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//tr[contains(., "Assessed Value")]/parent::tbody/tr[4]/td[1]';

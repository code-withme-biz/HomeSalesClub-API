...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://hickman.capturecama.com/CAMA/CAPortal/Custom/CZ_RealPropertySearch42.aspx'
    }
            ...
            while (true){
              ...
              try {
                ...
                if (this.searchBy === 'name') {
                  ...
                  const [option_handle] = await page.$x('//*[@id="SearchByPropAddr"]');
                  await page.evaluate(el => el.click(), option_handle);
                }
                ...
                const rows = await page.$x('//*[@id="GridView1"]/tbody/tr//table/tbody');
                const legends = await page.$x('//*[@id="GridView1"]/tbody/tr/td/fieldset/legend/span')
                let index = 0;
                rows.shift();
                for (const row of rows) {
                  ...
                  if (flag) {
                    let parcel_no = await legends[index].evaluate(el => el.getAttribute('onclick')) || '';
                    const regex = /'(.*?)'/gm;
                    let r = regex.exec(parcel_no);
                    if(r){
                        parcel_no = r[1];
                    } else {
                        continue;
                    }
                    let property_type = await page.evaluate(el => el.children[5].children[2].textContent, row) || '';
                    property_type = property_type.replace('\n','').replace(/\s+/g,' ').trim();
                    let detailPage = await this.browser?.newPage()!;
                    await detailPage.goto(`https://hickman.capturecama.com/CAMA/CAPortal/CZ_RealPropertyInfo.aspx?Item1=${parcel_no}`, {waitUntil: 'load'});
    ...
    async getPropertyInfos(page: puppeteer.Page, property_type: string): Promise<any> {
      ...
      // property address
      const property_address_xpath = '//td[*[contains(text(), "LOCATION:")]]/following-sibling::td[1]';
      let property_address = await this.getTextByXpathFromPage(page1, property_address_xpath);
      ...
      const total_assessed_value_xpath = '//td[contains(text(), "Assd. Val:")]/b';
      let total_assessed_value = await this.getTextByXpathFromPage(page1, total_assessed_value_xpath);
      const est_value_xpath = '//td[contains(text(), "Total:")][2]/b';
      const est_value = await this.getTextByXpathFromPage(page1, est_value_xpath);

      // sales
      const last_sale_recording_date_xpath = '//td[contains(text(), "Sales Info:")]/b[1]';
      const last_sale_recording_date = await this.getTextByXpathFromPage(page1, last_sale_recording_date_xpath);
      const last_sale_amount_xpath = '//td[contains(text(), "Sales Info:")]/b[2]'
      const last_sale_amount = await this.getTextByXpathFromPage(page1, last_sale_amount_xpath);

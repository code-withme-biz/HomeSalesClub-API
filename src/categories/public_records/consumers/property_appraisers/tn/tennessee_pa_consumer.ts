...
export default abstract class NewJerseyPAConsumer extends AbstractPAConsumer {
    abstract county: string;
    abstract countyValue: string;
    publicRecordProducer: IPublicRecordProducer;
    ownerProductProperties: IOwnerProductProperty;

    urls = {
        propertyAppraiserPage: 'https://www.assessment.cot.tn.gov/RE_Assessment/'
    }

    xpaths = {
        isPAloaded: '//select[@name="SelectCounty"]'
    }
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                ...
                if(!parsev1 || !parsev1.number || !parsev1.street){
                    console.log('The street number or street name is missing!');
                    return false;
                }
                search_value = ((parsev1.street ? parsev1.street : '') + ' ' + (parsev1.type ? parsev1.type + ' ' : '') + (parsev1.number ? parsev1.number : '')).trim();
            ...
            while (true){
              ...
              try {
                await page.goto('https://www.assessment.cot.tn.gov/RE_Assessment/SelectCounty.aspx?map=true&SelectCounty='+this.countyValue, { waitUntil: 'networkidle0'});
              } catch (error) {
                await page.reload();
              }
              try {
                await page.waitForXPath('//input[@id="MainContent_txtOwnerName"]');
                if(this.searchBy == 'name'){
                    await page.type('input[name="ctl00$MainContent$txtOwnerName"]', search_value, {delay: 150});
                } else {
                    await page.type('input[name="ctl00$MainContent$txtPropertyAddress"]', search_value, {delay: 150});
                }
                let [submitButton] = await page.$x('//input[@id="MainContent_btnSearch"]');
                await Promise.all([
                    submitButton.click(),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let searchResults = await page.$x('//table[@id="MainContent_GridView1"]/tbody/tr[position()>1]');
                ...
                if(this.searchBy == 'name'){
                    for(let i = 0; i < searchResults.length; i++){
                        let indXpath = i + 2;
                        searchResults = await page.$x('//table[@id="MainContent_GridView1"]/tbody/tr[position()>1]');
                        let name = await searchResults[i].evaluate(el => el.children[1].textContent?.trim());
                        const regexp = new RegExp(owner_name_regexp);
                        if (!regexp.exec(name!.toUpperCase())){
                            continue;
                        }
                        let [button] = await page.$x('//table[@id="MainContent_GridView1"]/tbody/tr['+indXpath+']/td[1]/input');
                        await Promise.all([
                            button.click(),
                            page.waitForNavigation({waitUntil: 'networkidle0'})
                        ]);
                    ...
                    let [button] = await page.$x('//table[@id="MainContent_GridView1"]/tbody/tr[2]/td[1]/input');
                    await Promise.all([
                        button.click(),
                        page.waitForNavigation({waitUntil: 'networkidle0'})
                    ]);
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//b[contains(text(), "Owner:")]/ancestor::td/font[2]/span[1]';
      ...
      // property address
      const property_address_xpath = '//b[contains(text(), "Address:")]/ancestor::tr/td[2]/font';
      let property_address = await this.getTextByXpathFromPage(page, property_address_xpath);
      let property_address_arr = property_address.split(/\s+/g);
      let property_street_number = property_address_arr.pop();
      property_address = (property_street_number + ' ' + property_address_arr.join(' ')).trim();

      console.log('Property Address from web:', property_address);
      let property_address_parsed = parser.parseLocation(property_address);
      // mailing address
      const mailing_address_xpath = '//b[contains(text(), "Owner:")]/ancestor::td/font[2]/span[contains(text(), ",")]/preceding::span[1]';
      const mailing_address_2_xpath = '//b[contains(text(), "Owner:")]/ancestor::td/font[2]/span[contains(text(), ",")]';
      ...
      // sales info"
      const last_sale_recording_date_xpath = '//th[text()="Sale Date"]/parent::tr/parent::tbody/tr[2]/td[1]';
      const last_sale_amount_xpath = '//th[text()="Sale Date"]/parent::tr/parent::tbody/tr[2]/td[2]';
      ...
      // property type
      const property_type_xpath = '//b[contains(text(), "Land Type:")]/ancestor::tr/td[2]/font';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
      	
      // assessed value and est. value
      const total_assessed_value = '';
      const est_value_xpath = '//b[contains(text(), "Total Market Appraisal:")]/ancestor::tr/td[2]/font/span';

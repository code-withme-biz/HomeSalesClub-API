...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://esearch.fbcad.org/'
    }

    xpaths = {
        isPAloaded: '//*[@id="keywords"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input#keywords');
                await page.click('input#keywords', {clickCount: 3, delay: 150});
                await page.type('input#keywords', search_value, {delay: 150});
                ...
                await page.waitForXPath('//*[@id="page-header"]', {visible: true});
                const total = await this.getTextByXpathFromPage(page, '//*[@id="page-header"]');
                if (total.indexOf('Total: 0') > -1) {
                ...
                if(this.searchBy == 'name'){
                  while (true) {
                    const rows = await page.$x('//*[contains(@class, "k-grid-content")]/table/tbody/tr');
                    for(const row of rows){
                      try{
                        ...
                        if (regexp.exec(name.toUpperCase())){
                            let refId = await row.evaluate(el => el.children[1].textContent?.trim()) || '';
                            refIDs.push(refId);
                        }
                      } catch(e){
                          console.log(e);
                    ...
                    const rows = await page.$x(`//*[contains(@class, "k-grid-content")]/table/tbody/tr/td[contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), "${search_value}")]/parent::tr[1]`);
                    for (const row of rows) {
                      let refId = await row.evaluate(el => el.children[1].textContent?.trim()) || '';
                      refIDs.push(refId);
                ...
                for (const refID of refIDs) {
                  await page.goto(`https://esearch.fbcad.org/Property/View/${refID}`, {waitUntil: 'load'});
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//*[text()="Name:"]/following-sibling::td[1]';
      ...
      full_name = full_name.filter((s:string) => s.trim() !== '').map((s:string)=>s.trim());
      full_name = full_name[0].trim();
      full_name = full_name.replace(/\W/g, '');
      ...
      // sales info"
      const last_sale_recording_date = await this.getTextByXpathFromPage(page, '//tbody[contains(., "Deed Date")]/tr[2]/td[1]');;
      const last_sale_amount = '';

      // property type
      const property_type_xpath = '//*[text()="Type:"]/following-sibling::td[1]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//*[text()="Assessed Value:"]/following-sibling::td[1]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//*[text()="Appraised Value:"]/following-sibling::td[1]'
      const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

      let year_built = await this.getTextByXpathFromPage(page, '//th[text()="Year Built"]/ancestor::tbody/tr[2]/td[4]');
      year_built = year_built == '0' ? '' : year_built;

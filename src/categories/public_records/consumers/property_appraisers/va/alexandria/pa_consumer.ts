...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://realestate.alexandriava.gov/index.php?action=address'
    }

    xpaths = {
        isPAloaded: '//input[@name="StreetNumber"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://realestate.alexandriava.gov/index.php?action=address");
                return false;
            }
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input[name="StreetNumber"]');
                if(parsed_addr.number){
                    await page.type('input[name="StreetNumber"]', parsed_addr.number, {delay: 50});
                }
                if(parsed_addr.street){
                    let street = ((parsed_addr.prefix ? parsed_addr.prefix : '') + ' ' + (parsed_addr.street ? parsed_addr.street : '')).trim();
                    await page.type('input[name="text_street"]', street, {delay: 50});
                }
                if(parsed_addr.sec_unit_num){
                    await page.type('input[name="UnitNo"]', parsed_addr.sec_unit_num, {delay: 50});
                }
                await Promise.all([
                    page.click('input[name="Search"]'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//div[@id="coa_rea_main"]/table/tbody/tr//a');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page, parsed_addr: any): Promise<any> {
        // name
        const full_name_xpath = '//td[contains(., "Owner Name:")]/div[2]';
        let full_name = await this.getTextByXpathFromPage(page, full_name_xpath);
        const owner_names = [];
        full_name = full_name.split("OR")[0].trim();
        let parseName = this.parseOwnerName(full_name);
        owner_names.push(parseName);
  
        // mailing address
        const mailing_address_xpath = '//td[contains(., "Mailing Address:")]/div[2]';
        const mailing_address_2_xpath = '//td[contains(., "Mailing Address:")]/div[last()]';
        ...
        // sales info"
        const last_sale_recording_date_xpath = '//td[contains(., "Sale Date")]/ancestor::tbody/tr[2]/td[1]/div';
        const last_sale_amount_xpath = '//td[contains(., "Sale Date")]/ancestor::tbody/tr[2]/td[2]/div';
        ...
        const total_assessed_value_xpath = '//td[contains(., "Assessment Date")]/ancestor::tbody/tr[2]/td[2]/div';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value = '';

        let year_built = await this.getTextByXpathFromPage(page, '//div[@id="coa_rea_main"]/text()[contains(., "Year Built")]');
        year_built = year_built.split(':')[1].trim();

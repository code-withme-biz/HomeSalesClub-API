...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://icare.fairfaxcounty.gov/ffxcare/search/commonsearch.aspx?mode=address'
    }

    xpaths = {
        isPAloaded: '//input[@id="inpNumber"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: https://icare.fairfaxcounty.gov/ffxcare/search/commonsearch.aspx?mode=address");
                return false;
            ...
            while (true){
              ...
              try {
                ...
                if(parsed_addr.sec_unit_num){
                  await page.type('input#inpUnit', parsed_addr.sec_unit_num, {delay: 50})
                }
                await Promise.all([
                    page.click('button#btSearch'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//td[contains(., "Name")]/parent::tr/td[2]');
                if(found.length < 1){
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        ...
        let property_address = (await this.getTextByXpathFromPage(page, property_address_xpath)).split('   ')[0].trim();
        console.log(property_address);
        let property_address_parsed = parser.parseLocation(property_address);
  
        // mailing address
        const mailing_address_xpath = '//td[contains(., "Mailing Address")]/parent::tr/td[2]';
        ...
        // property type
        const property_type_xpath = '//td[contains(., "Land Use Code")]/parent::tr/td[2]';
        ...
        const total_assessed_value_xpath = '//td[contains(., "Current Assessed Total")]/parent::tr/td[2]';

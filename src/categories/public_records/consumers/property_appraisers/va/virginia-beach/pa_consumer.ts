...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://cvb.manatron.com/Tabs/PropertySearch.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@id="fldInput"]'
    }
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                ...
                // }
                await page.waitForSelector('input#fldInput');
                if (this.searchBy == 'name'){
                    await page.type('input#fldInput', search_value, {delay: 150});
                } else {
                    await page.select('select#selSearchBy', 'Column4*');
                    await page.type('input#fldInput', search_value, {delay: 150});
                }
                ...
                const search_results = await page.$x('//table[@id="grm-search"]/tbody/tr');
                ...
                if(this.searchBy == 'name'){
                    for(const row of search_results){
                        try{
                            let link = await row.evaluate(el => el.children[2].children[0].textContent?.trim());
                            link = "https://www.vbgov.com/property-search#DetailView=" + link?.trim();
                    ...
                    let link = await search_results[0].evaluate(el => el.children[2].children[0].textContent?.trim());
                    link = "https://www.vbgov.com/property-search#DetailView=" + link?.trim();
                    let name = await search_results[0].evaluate(el => el.children[0].textContent?.trim());
                    namelists.push(name);
                    datalinks.push(link);
                }
                ...
                console.log(datalinks, namelists);
                let indName = 0;
                for (let datalink of datalinks) {
                    try{
                        ...
                        let result = await this.getPropertyInfos(page, namelists[indName]);
    ...
    async getPropertyInfos(page: puppeteer.Page, full_name: string): Promise<any> {
      // name
      const owner_names = [];
      ...
      // property address
      const property_address_xpath = '//h1[@id="address-details"]';
      ...
      // there is no mailing address in this site

      // owner occupied
      let owner_occupied = false;

      // property type
      const property_type_xpath = '//td[@id="land-property-code"]';
      const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);

      // assessed value and est. value
      const total_assessed_value_xpath = '//th[text()="Total Value"]/parent::tr/td';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value = '';

      // sales info"
      await page.click('a#AssessCharts');
      await this.sleep(500);
      const last_sale_recording_date_xpath = '//table[@id="sales-history-table"]/tbody/tr[1]/td[2]';
      const last_sale_amount_xpath = '//table[@id="sales-history-table"]/tbody/tr[1]/td[3]';

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://65.183.210.212/CowlitzPropertyApp/CowlitzPropertyApp/Zoner/index'
    }

    xpaths = {
        isPAloaded: '//input[@id="address_input_street_number"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: http://65.183.210.212/CowlitzPropertyApp/CowlitzPropertyApp/Zoner/index");
                return false;
            }
            ...
            let address_regex = parsed_addr.number + '\\s+';
            let retry_count = 0;
            while (true){
              if (retry_count > 3){
              ...
              try {
                await page.waitForSelector('input#address_input_street_number');
                if(parsed_addr.number){
                    await page.click('input#address_input_street_number', {clickCount: 3});
                    await page.type('input#address_input_street_number', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    await page.click('input#address_input_street_name', {clickCount: 3});
                    await page.type('input#address_input_street_name', parsed_addr.street, {delay: 50})
                }
                await Promise.all([
                    page.click('input#submit_input'),
                    page.waitForNavigation({waitUntil: 'networkidle0'})
                ]);
                let found = await page.$x('//section[@id="properties_section"]/div/a');
                if(found.length < 1){
                    console.log("Not found!");
                    break;
                }
                let link = await found[0].evaluate(el => el.getAttribute('href'));
                link = 'http://65.183.210.212/CowlitzPropertyApp/CowlitzPropertyApp/Zoner/' + link;

                await page.goto(link, {waitUntil: 'networkidle0'});
                let address_from_web = await this.getTextByXpathFromPage(page, '//header[@class="property-title"]/h1');
                console.log(address_from_web);
                const regex = new RegExp(address_regex);
                if(address_from_web.match(regex)){
    ...
    async getPropertyInfos(page: puppeteer.Page, parsed_addr: any): Promise<any> {
        // name
        const full_name_xpath = '//dt[text()="Primary Owner"]/parent::dl/dd[1]';
        ...
        // mailing address
        const mailing_address_xpath = '//dt[text()="Primary Owner"]/parent::dl/dd[2]';
        const mailing_address_2_xpath = '//dt[text()="City, State, Zip"]/following-sibling::dd[1]';
        ...
        // property type
        const property_type_xpath = '//dt[text()="Property Use"]/following-sibling::dd[1]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
  
        // assessed value and est. value
        const total_assessed_value_xpath = '//section[@id="assess_value_table"]/table/tbody/tr[1]/td[last()-1]';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value_xpath = '//td[contains(., "Total market:")]/parent::tr/td[2]/p'
        const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

        const year_built = await this.getTextByXpathFromPage(page, '//dt[text()="Year Built - SFR"]/following-sibling::dd[1]');

...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://www.snoco.org/proptax/(S(mpj2p3sbo4lvalhg0w4lhgie))/default.aspx'
    }
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        // name
        const full_name_xpath = '//td[text()="Owner"]/parent::tr/td[3]';
        ...
        // mailing address
        const mailing_address_full_xpath = '//td[text()="Owner"]/parent::tr/td[last()]';
        ...
        let mailing_city = mailing_address_full_arr[1].trim();
        let mailing_zip = mailing_address_full_arr[2].trim().split(/\s+/g)[1];
        let mailing_state = mailing_address_full_arr[2].trim().split(/\s+/g)[0];
        ...
        // property type
        const property_type_xpath = '//td[contains(., "Use Code")]/parent::tr/td[2]';
        ...
        const est_value_xpath = '//td[text()="Market Total"]/parent::tr/td[2]'
        const est_value = await this.getTextByXpathFromPage(page, est_value_xpath);

        const year_built = await this.getTextByXpathFromPage(page, '//table[@id="mRealPropertyStructures"]/tbody/tr[2]/td[3]');

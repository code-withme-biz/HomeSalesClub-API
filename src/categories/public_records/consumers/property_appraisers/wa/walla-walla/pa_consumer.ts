...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://propertysearch.co.walla-walla.wa.us/'
    }

    xpaths = {
        isPAloaded: '//input[@id="propertySearchOptions_propertyid"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
              console.log('This site is not supported search by name!');
              return false;
            }
            else {
            ...
            while (true){
              ...
              try {
                // input address
                await page.select('#propertySearchOptions_searchType', 'address');
                await page.type('#propertySearchOptions_streetNumber', parsed_addr.number, {delay: 150});
                await page.type('#propertySearchOptions_streetName', parsed_addr.street, {delay: 150});
                ...
                while (true) {
                  ...
                  for(const row of rows) {
                    ...
                    if (flag) {
                      let link = await row.evaluate(el => el.children[10].children[0].getAttribute('href'));
                      console.log(link);
                      let detailPage = await this.browser?.newPage()!;
                      await detailPage.goto(`http://propertysearch.co.walla-walla.wa.us/PropertyAccess/${link}`, {waitUntil: 'load'});
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      ...
      const year_built = await this.getTextByXpathFromPage(page, '//table[@class="improvementDetails"]/tbody/tr[contains(., "Main")]/td[6]');

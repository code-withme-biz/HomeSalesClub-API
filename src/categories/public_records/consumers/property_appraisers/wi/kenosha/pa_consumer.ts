...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'https://propertyinfo.kenoshacounty.org/'
    }

    xpaths = {
        isPAloaded: '//input[@id="ctl00_cphMainApp_btnEntryPageAccept"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean> {
        ...
        let document = docsToParse;
            ...
            if (this.searchBy === 'name') {
                console.log("By name detected! The site is only supported searched by property address: http://www16.co.hennepin.mn.us/pins/addrsrch.jsp");
                return false;
            }
            ...
            while (true){
              ...
              try {
                let [tosButton] = await page.$x('//input[@id="ctl00_cphMainApp_btnEntryPageAccept"]');
                if(tosButton){
                    await Promise.all([
                        tosButton.click(),
                ...
                await page.waitForSelector('input#ctl00_cphMainApp_ParcelSearchCriteria1_txtHouseNumber');
                if(parsed_addr.number){
                    await page.type('input#ctl00_cphMainApp_ParcelSearchCriteria1_txtHouseNumber', parsed_addr.number, {delay: 50})
                }
                if(parsed_addr.street){
                    await page.type('input#ctl00_cphMainApp_ParcelSearchCriteria1_txtStreetName', parsed_addr.street, {delay: 50})
                }
                await Promise.all([
                    page.click('input#ButtonParcelSearch'),
                    page.waitForXPath('//table[@id="ctl00_cphMainApp_GridViewParcelResults"]/tbody/tr')
                ]);
                let found = await page.$x('//table[@id="ctl00_cphMainApp_GridViewParcelResults"]/tbody/tr');
                if(found.length < 2){
                ...
                await Promise.all([
                    found[1].click(),
                    page.waitForXPath('//span[@id="lblOwnerName"]')
                ])
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
        // name
        const full_name_xpath = '//span[@id="lblOwnerName"]';
        ...
        // property address
        const property_address_xpath = '//span[@id="LabelTitlePropAddr"]';
        ...
        // mailing address
        const mailing_address_xpath = '//span[@id="lblBillingAddress"]/text()[last()-1]';
        const mailing_address_2_xpath = '//span[@id="lblBillingAddress"]/text()[last()]';
        ...
        await Promise.all([
            page.click('a#LinkButtonDocuments'),
            page.waitForNavigation({waitUntil: 'networkidle0'})
        ])
        // sales info"
        const last_sale_recording_date_xpath = '//a[@id="LinkButton_DH_TransferDate"]';
        const last_sale_amount_xpath = '//a[@id="LinkButton_DH_SaleAmount"]';
        ...
        await Promise.all([
            page.click('a#LinkButtonBuildingData'),
            page.waitForNavigation({waitUntil: 'networkidle0'})
        ])
        // property type
        const property_type_xpath = '//span[@id="ctl00_cphMainApp_SearchDetailsParcel_BuildingDetails1_radPanelBar_i0_ctl00_FormViewFeatures_gvFeatures_ctl03_LabelFeatureValue"]';
        const property_type = await this.getTextByXpathFromPage(page, property_type_xpath);
        const year_built_xpath = '//span[@id="ctl00_cphMainApp_SearchDetailsParcel_BuildingDetails1_radPanelBar_i0_ctl00_FormViewFeatures_gvFeatures_ctl02_LabelFeatureValue"]';
        const year_built = await this.getTextByXpathFromPage(page, year_built_xpath);

        await Promise.all([
            page.click('a#LinkButtonAssessments'),
            page.waitForNavigation({waitUntil: 'networkidle0'})
        ])
        // assessed value and est. value
        const total_assessed_value_xpath = '//span[@id="lblTotal"]';
        const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
        const est_value_xpath = '//span[@id="lblEstimatedFMV"]'

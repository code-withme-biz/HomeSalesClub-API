...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    urls = {
        propertyAppraiserPage: 'http://lrs.co.sauk.wi.us/AscentLandRecords/PropertyListing/RealEstateTaxParcel#/Search/'
    }

    xpaths = {
        isPAloaded: '//input[@data-ng-model="data.Search.StreetNum"]'
    }
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty): Promise<boolean>   {
        ...
        let document = docsToParse;
            ...
            while (true){
              ...
              try {
                await page.waitForSelector('input[data-ng-model="data.Search.StreetNum"]');
                if (this.searchBy == 'name'){
                    await page.click('input[data-ng-model="data.Search.LastName"]', {clickCount: 3});
                    await page.type('input[data-ng-model="data.Search.LastName"]', last_name, {delay: 150});
                    await page.click('input[data-ng-model="data.Search.FirstName"]', {clickCount: 3});
                    await page.type('input[data-ng-model="data.Search.FirstName"]', first_name, {delay: 150});
                } else {
                    await page.click('input[data-ng-model="data.Search.StreetNum"]', {clickCount: 3});
                    await page.type('input[data-ng-model="data.Search.StreetNum"]', parsed_addr.number, {delay: 150});
                    await page.click('input[data-ng-model="data.Search.StreetName"]', {clickCount: 3});
                    await page.type('input[data-ng-model="data.Search.StreetName"]', parsed_addr.street, {delay: 150});
                }
                let buttonSearch = await page.$x('//input[@id="btnFindNow"]');
                await buttonSearch[0].click();
                await page.waitForXPath('//span[text()="Search Complete."]');
                const search_results = await page.$x('//tr[@data-ng-repeat="result in Parcels"]');
                ...
                if(this.searchBy == 'name'){
                    ...
                    for(const row of search_results){
                        let link = await page.$x('//tr[@data-ng-repeat="result in Parcels"]['+ind+']/td[1]/a');
                        let name = await row.evaluate(el => el.children[1].children[1].textContent?.trim());
                        const regexp = new RegExp(owner_name_regexp);
                        if (regexp.exec(name!.toUpperCase())){
                            await link[0].click();
                            await page.waitForXPath('//span[text()="Load Complete."]');
                    ...
                    let link = await page.$x('//tr[@data-ng-repeat="result in Parcels"][1]/td[1]/a');
                    let address = await search_results[0].evaluate(el => el.children[2].children[0].children[0].textContent?.trim());
                    console.log(address);
                    let parsed_res = parser.parseLocation(address);
                    if(!parsed_res || !parsed_res.number){
                        break;
                    }
                    if(parsed_res.number == parsed_addr.number){
                        await link[0].click();
                        await page.waitForXPath('//span[text()="Load Complete."]');
                        let result = await this.getPropertyInfos(page);
                        await this.parseResult(result, document);
    ...
    async getPropertyInfos(page: puppeteer.Page): Promise<any> {
      // name
      const full_name_xpath = '//div[@data-ng-repeat="person in topTwo(Owners)"]/strong/text()';
      ...
      // property address
      const property_address_full_xpath = '//div[@data-ng-repeat="site in topTwo(data.TaxParcel.SiteAddresses)"]/strong';
      const property_address_full = await this.getTextByXpathFromPage(page, property_address_full_xpath);
      let property_address, property_city, property_zip, property_state = '';
      let property_address_parsed = parser.parseLocation(property_address_full);
      if(property_address_parsed){
        property_address = ((property_address_parsed.number ? property_address_parsed.number : '') + ' ' + (property_address_parsed.street ? property_address_parsed.street + ' ' : '') +  (property_address_parsed.type ? property_address_parsed.type : '')).trim();
      ...
      // mailing address
      const mailing_address_xpath = '//div[@data-ng-hide="data.TaxParcel.Private && !ViewPrivateParcels"]/strong[2]';
      const mailing_address_2_xpath = '//div[@data-ng-hide="data.TaxParcel.Private && !ViewPrivateParcels"]/strong[last()]';
      ...
      await page.select('select[data-ng-model="data.SelectedUrl"]','5');
      await page.waitForXPath('//span[text()="Load Complete."]');
      await this.sleep(3000);
      // sales info"
      const last_sale_recording_date_xpath = '//th[text()="Document Date"]/ancestor::table/tbody/tr[1]/td[4]';
      ...
      await page.select('select[data-ng-model="data.SelectedUrl"]','1');
      await page.waitForXPath('//span[text()="Load Complete."]');
      await this.sleep(3000);
      const total_assessed_value_xpath = '//th[text()="Land Value"]/ancestor::table/tfoot/tr[1]/td[8]';
      const total_assessed_value = await this.getTextByXpathFromPage(page, total_assessed_value_xpath);
      const est_value_xpath = '//label[@data-ng-show="data.year.EstimatedFairMarketValue != 0"]';

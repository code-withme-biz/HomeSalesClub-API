...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'http://recorder.co.imperial.ca.us/RecorderWorks/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Califonia', 'Imperial');
            ...
            const officialRecordSelector = 'a#MainContent_SearchParent1_SearchMenu1_officialRecords';
            const officialRecord = await page.$(officialRecordSelector);
            await officialRecord?.click();            
            ...
            if (countResults  === 0) {
                ...
                await AbstractProducer.sendMessage('Imperial', 'California', countRecords, 'Civil & Lien');
                return false;
            }
            else {
            ...
            while (!isLast) {
                ...
                if (last_class == 'boldLinkColor') {
                    ...
                    await Promise.all([
                        ...
                        page.waitFor(this.getRandomInt(3000, 10000))
                    ]);
                    isLast = false;
                } else {
                    isLast = true;
    ...
    async getData(page: puppeteer.Page, number: any, type: any, date: any, grantee: any): Promise<any> {
        let grantee_name: any = this.newParseName(grantee);        

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://egov.broomfield.org/recorder/eagleweb/docSearch.jsp'
    };
            ...
            do {
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    if (nameHandle.length > 0) {
                        ...
                        try {
                            await page.waitForXPath('//th[contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"grantor") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"owner") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"party")]/parent::tr/parent::tbody/tr/td/span', { visible: true, timeout: 3000 });
                        } catch (err) {
                        ...
                        const Grantors = await page.$x('//th[contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"grantor") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"owner") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"party")]/parent::tr/parent::tbody/tr/td/span');
                        const Grantes = await page.$x('//th[contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"grantee") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"buyer") or contains(translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"),"borrower")]/parent::tr/parent::tbody/tr/td/span');
                        let names = [];
                        try {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            let dateRange = await this.getDateRange('Colorado', 'Broomfield');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    ...
                    await page.goto('https://egov.broomfield.org/recorder/eagleweb/docSearch.jsp');
            ...
            await AbstractProducer.sendMessage('Broomfield', 'Colorado', countRecords, 'Civil & Lien');
            return false
        }

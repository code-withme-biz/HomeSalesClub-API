...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://countyfusion3.kofiletech.us/countyweb/login.do?countyname=Denver'
    };
    ...
    async read(): Promise<boolean> {
        try {
            await this.browserPages.generalInfoPage?.waitForXPath('//*[@type="button" and contains(@value, "Civil Union Application")]');
            return true;
        } catch (err) {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const [clickCivilUnionBtn] = await page.$x('//*[@type="button" and contains(@value, "Civil Union Application")]');
            await Promise.all([
            ...
            let dateRange = await this.getDateRange('Colorado', 'Denver');
            ...
            await AbstractProducer.sendMessage('Denver', 'Colorado', countRecords, 'Civil & Lien');
            return false
        }
        await AbstractProducer.sendMessage('Denver', 'Colorado', countRecords, 'Civil & Lien');
        return true;

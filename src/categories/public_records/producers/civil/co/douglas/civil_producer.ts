...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://apps.douglas.co.us/LandmarkWeb'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try {
            const civilUrl: string = 'https://apps.douglas.co.us/LandmarkWeb';
            let dateRange = await this.getDateRange('Colorado', 'Douglas');
            ...
            while (fromDate <= toDate) {
                ...
                let documentTypes = ['Deeds', 'Liens', 'Mortgage', 'Release of Liens'];
            ...
            await AbstractProducer.sendMessage('Douglas', 'Colorado', countRecords, 'Civil & Lien');
            return true;
        } catch (err) {

...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/CTNHA/Login.aspx'
    }

    xpaths = {
        isPageLoaded: '//button[@id="btnGuestLogin"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('Connecticut', 'New Haven', 60);
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);

            try {
                const [docBtnHandle] = await page.$x('//button[@id="btnGuestLogin"]');
                await docBtnHandle.click();
                await page.waitForNavigation();
            } catch (error) {
                console.log(error);
                await AbstractProducer.sendMessage('New Haven', 'Connecticut', countRecords, 'Civil & Lien');
                return false;
            }
            ...
            for (const docTypeSelect of docTypeSelects) {
                // setting doc type
                await page.select('select[id="ContentPlaceHolder1_cboDocGroup"]', docTypeSelect);
                await page.waitForNavigation();
                ...
                if (results.length > 0) {
                    ...
                    while (!isLast) {
                        ...
                        for (let i = 1; i < rows.length; i++) {
                            const namesHTML = await rows[i].evaluate(el => el.children[5].children[0].innerHTML);
                ...
                await page.waitForNavigation();
                await page.waitFor(1000);
                await this.randomSleepInOneMinute();
            }
            ...
            await AbstractProducer.sendMessage('New Haven', 'Connecticut', countRecords, 'Civil & Lien', errorImage);
        }

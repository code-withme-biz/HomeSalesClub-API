...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://deeds.sussexcountyde.gov/LandmarkWeb'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const civilUrl: string = 'https://deeds.sussexcountyde.gov/LandmarkWeb';
            let dateRange = await this.getDateRange('Delaware', 'Sussex');
            ...
            while (fromDate <= toDate) {
                ...
                let options = await page.$x('//select[@id="documentCategory-DocumentType"]/option');
                for (const option of options) {
                    // console.log(docTypeSelect);
                    let optionVal: any = await (await option.getProperty('value')).jsonValue();
                    if (optionVal !== '' && optionVal !== 'custom') {
            ...
            await AbstractProducer.sendMessage('Sussex', 'Delaware', countRecords, 'Civil & Lien');
            return true;
        } catch (err) {

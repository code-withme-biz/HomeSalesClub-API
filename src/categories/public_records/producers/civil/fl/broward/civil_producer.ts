...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://officialrecords.broward.org/AcclaimWeb/search/SearchTypeDocType'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'https://officialrecords.broward.org/AcclaimWeb/search/SearchTypeDocType';

        let dateRange = await this.getDateRange('Florida', 'Broward');
        ...
        for (let i = days < 1 ? 1 : days; i >= 0; i--) {
            try {
                ...
                await page.waitForSelector('div#RsltsGrid');
                ...
                while (nextPage) {
                    ...
                    for (const row of resultRows) {
                        ...
                        let recordDate = await page.evaluate(el => el.children[5].textContent.trim(), row);
        ...
        await AbstractProducer.sendMessage('Broward', 'Florida', countRecords, 'Civil & Lien');
        return true;
    }
}
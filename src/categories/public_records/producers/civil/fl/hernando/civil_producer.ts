...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try{
            let dateRange = await this.getDateRange('Florida', 'Hernando');
            ...
            let docTypeSelects = ['D', 'LN', 'MAR', 'MTG', 'LP'];
            await page.goto(civilUrl, { waitUntil: 'load' });
            // choose county
            await page.select('#or_county', '27');
            // type date range
            ...
            // choose doc type
            await page.select('select[name="documentTypes"]', ...docTypeSelects);
            ...
            await AbstractProducer.sendMessage('Hernando', 'Florida', countRecords, 'Civil & Lien');
            return true;
        } catch (e){
            console.log(e);
            await AbstractProducer.sendMessage('Hernando', 'Florida', countRecords, 'Civil & Lien');

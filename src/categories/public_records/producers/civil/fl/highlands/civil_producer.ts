...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://acclaim.hcclerk.org/search/Disclaimer?st=/search/SearchTypeDocType'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        // get date range
        try{
            let dateRange = await this.getDateRange('Florida', 'Highlands');
                ...
                try {
                    ...
                    while (nextPage) {
                        ...
                        for (const row of resultRows) {
                            let names = [];
                            names.push(await page.evaluate(el => el.children[2].textContent.trim(), row));
                            names = names.filter(name => name !== '');
                            let recordDate = await page.evaluate(el => el.children[4].textContent.trim(), row);
                            let docType = await page.evaluate(el => el.children[1].textContent.trim(), row);
            ...
            await AbstractProducer.sendMessage('Highlands', 'Florida', countRecords, 'Civil & Lien');
            return true;
        } catch(e){
            console.log(e);
            await AbstractProducer.sendMessage('Highlands', 'Florida', countRecords, 'Civil & Lien');

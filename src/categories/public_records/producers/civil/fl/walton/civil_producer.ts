...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://orsearch.clerkofcourts.co.walton.fl.us/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try{
            let dateRange = await this.getDateRange('Florida', 'Walton');
            ...
            while (fromDate <= toDate) {
                ...
                let documentTypes = ['Deed', 'Judgment', 'Lien', 'Lis Pendens', 'Marriage License', 'Marriage LicenseCopy', 'Mortgage', 'Probate'];
                ...
                await page.waitForXPath('//li[contains(@class, "disabled")]/a[contains(text(), "Next")]', { visible: true, timeout: 200000 });
                let docTypeHandles;
                while (true) {
            ...
            await AbstractProducer.sendMessage('Walton', 'Florida', countRecords, 'Civil & Lien');
            return true;
        } catch(e){
            console.log(e);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Walton', 'Florida', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
    }
}
...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://resolution.forsythco.com/External/LandRecords/protected/SrchDateRange.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'https://resolution.forsythco.com/External/LandRecords/protected/SrchDateRange.aspx';
        let countRecords = 0;

        try {
            let dateRange = await this.getDateRange('Georgia', 'Forsyth');
            ...
            for (let i = 1; i <= numberOfPages; i++) {
                ...
                for (let i = 1; i < totalRowHandle!.length; i++) {
                    ...
                    if (docType == 'SD') {
                        docType = 'Security Deed'
                    } else if (docType == 'CANC') {
                        docType = 'Cancellation of Security Deed'
                    } else if (docType == 'AFF') {
                        docType = 'AFFIDAVIT'
                    } else if (docType == 'AGRE') {
                        docType = 'AGREEMENT'
                    } else if (docType == 'ASGN') {
                        docType = 'Assignment of Security Deed'
                    } else if (docType == 'CERT') {
                        docType = 'Certificate of Trust'
                    } else if (docType == 'EASE') {
                        docType = 'Easement'
                    } else if (docType == 'ESTD') {
                        docType = "Administrator's Deed"
                    } else if (docType == 'FCD') {
                        docType = "Executor's Deed Under Power"
                    } else if (docType == 'FTL') {
                        ...
                        docType = 'Hospital Lien'
                    } else if (docType == 'RWD') {
                        docType = 'Right Of Way Deed'
                    } else if (docType == 'SHFD') {
                        docType = 'Sherrif Deed'
                    } else if (docType == 'TRSD') {
                        docType = "Trustee's Deed"
                    } else if (docType == 'WD') {
            ...
            await AbstractProducer.sendMessage('Forsyth', 'Georgia', countRecords, 'Civil & Lien');
            return true;
        } catch (e) {
            console.log(e);
            await AbstractProducer.sendMessage('Forsyth', 'Georgia', countRecords, 'Civil & Lien');

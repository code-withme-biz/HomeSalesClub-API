...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://recorder.johnsoncountyiowa.gov/External/LandRecords/protected/SrchDateRange.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'https://recorder.johnsoncountyiowa.gov/External/LandRecords/protected/SrchDateRange.aspx';
        let countRecords = 0;

        try {
            let dateRange = await this.getDateRange('Iowa', 'Johnson');
            ...
            for (let i = 1; i <= numberOfPages; i++) {
                ...
                for (let i = 1; i < totalRowHandle!.length; i++) {
                    ...
                    let docTypeHandle = await page.$x('//table[@id="ctl00_cphMain_lrrgResults_cgvResults"]/tbody/tr[' + i + ']/td[5]');
                    ...
                    const Grantors = await page.$x('//table[@id="ctl00_cphMain_lrrgResults_cgvResults"]/tbody/tr[' + i + ']/td[6]/div/table/tbody/tr/td');
                    const Grantes = await page.$x('//table[@id="ctl00_cphMain_lrrgResults_cgvResults"]/tbody/tr[' + i + ']/td[7]/div/table/tbody/tr/td');
                    let names = [];
                    try {
            ...
            await AbstractProducer.sendMessage('Johnson', 'Iowa', countRecords, 'Civil & Lien');
            return true;
        } catch (e) {
            console.log(e);
            await AbstractProducer.sendMessage('Johnson', 'Iowa', countRecords, 'Civil & Lien');

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://landrecords.polkcountyiowa.gov/LandRecords/protected/SrchDateRange.aspx'
    };
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Iowa', 'Polk');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('https://landrecords.polkcountyiowa.gov/LandRecords/protected/SrchDateRange.aspx', {waitUntil: 'load'});
            ...
            await AbstractProducer.sendMessage('Polk', 'Iowa', countRecords, 'Civil');
            return false;
        }
        await AbstractProducer.sendMessage('Polk', 'Iowa', countRecords, 'Civil');
        return true;

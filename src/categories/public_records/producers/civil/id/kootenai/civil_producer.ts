...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://recorder.kcgov.us/RealEstate/SearchEntry.aspx'
    };
            ...
            do {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    for (let j = 0; j < nameRows.length; j++) {
                        const [checkNomineeElement] = await page.$x(`//*[@id="Table1"]/tbody/tr[6]/td/table/tbody/tr[${j + 1}]/td/table/tbody/tr/td[3]`);
                        const checkNominee = (await page.evaluate(e => e.textContent, checkNomineeElement)).trim();
                        if (checkNominee == 'NOMINEE') continue;
                        const [nameElement] = await page.$x(`//*[@id="Table1"]/tbody/tr[6]/td/table/tbody/tr[${j + 1}]/td/table/tbody/tr/td[2]`);
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Idaho', 'Kootenai');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('http://recorder.kcgov.us/RealEstate/SearchEntry.aspx', {waitUntil: 'load'});
            ...
            await AbstractProducer.sendMessage('Kootenai', 'Idaho', countRecords, 'Civil & Lien');
            return false;
        }

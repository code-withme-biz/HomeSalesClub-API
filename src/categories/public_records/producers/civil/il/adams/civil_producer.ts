...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://il4laredo.fidlar.com/ILAdams/DirectSearch/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://il4laredo.fidlar.com/ILAdams/DirectSearch/#!/search';
            let dateRange = await this.getDateRange('Illinois', 'Adams');
            ...
            await AbstractProducer.sendMessage('Adams', 'Illinois', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Adams', 'Illinois', countRecords, 'Civil & Lien');

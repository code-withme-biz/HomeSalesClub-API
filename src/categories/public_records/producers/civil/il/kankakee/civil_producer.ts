...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://ilkankakee.fidlar.com/ILKankakee/DirectSearch/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://ilkankakee.fidlar.com/ILKankakee/DirectSearch/#!/search';
            let dateRange = await this.getDateRange('Illinois', 'Kankakee');
            ...
            await AbstractProducer.sendMessage('Kankakee', 'Illinois', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Kankakee', 'Illinois', countRecords, 'Civil & Lien');

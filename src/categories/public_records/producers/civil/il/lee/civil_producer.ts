...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://illee.fidlar.com/ILLee/DirectSearch/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://illee.fidlar.com/ILLee/DirectSearch/#!/search';
            let dateRange = await this.getDateRange('Illinois', 'Lee');
            ...
            await AbstractProducer.sendMessage('Lee', 'Illinois', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Lee', 'Illinois', countRecords, 'Civil & Lien');

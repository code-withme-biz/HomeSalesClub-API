...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://inallen.fidlar.com/INAllen/DirectSearch/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://inallen.fidlar.com/INAllen/DirectSearch/#!/search';
            let dateRange = await this.getDateRange('Indiana', 'Allen');
            ...
            await AbstractProducer.sendMessage('Allen', 'Indiana', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Allen', 'Indiana', countRecords, 'Civil & Lien');

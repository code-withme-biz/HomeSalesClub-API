...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://mi2laredo.fidlar.com/MIIngham/AvaWeb/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://mi2laredo.fidlar.com/MIIngham/AvaWeb/#!/search';
            let dateRange = await this.getDateRange('Michigan', 'Ingham');
            ...
            await AbstractProducer.sendMessage('Ingham', 'Michigan', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Ingham', 'Michigan', countRecords, 'Civil & Lien');

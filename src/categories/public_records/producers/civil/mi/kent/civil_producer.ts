...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://deedsselfservice.kentcountymi.gov/web/user/disclaimer'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'https://deedsselfservice.kentcountymi.gov/web/user/disclaimer';
        let page = this.browserPages.generalInfoPage!;
        let countRecords = 0;
        try{
            let dateRange = await this.getDateRange('Michigan', 'Kent');
            ...
            await AbstractProducer.sendMessage('Kent', 'Michigan', countRecords, 'Civil & Lien');
            return true;
        } catch(e){
            console.log(e);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Kent', 'Michigan', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
    }
}
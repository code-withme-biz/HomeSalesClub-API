...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://mofranklin.fidlar.com/mofranklin/avaweb/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try{
            const civilUrl: string = 'https://mofranklin.fidlar.com/mofranklin/avaweb/#!/search';
            let dateRange = await this.getDateRange('Missouri', 'Franklin');
            ...
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Franklin', 'Missouri', countRecords, 'Civil & Lien', errorImage);
            return true;
        } catch (error){
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://burke.courtcompsys.com/BurkeNCNW/application.asp?resize=true'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Burke');
            ...
            while (true) {
                ...
                if (rows.length > 0) {
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        let nameHandles = await frame.$x(`//div[@id="resultspane"]/table//tr[contains(@style, "cursor")][${i + 1}]/td[contains(@class, "c6")]//span`);
                        let name = ''
                        for (const nameHandle of nameHandles) {
                            name = name + ' ' + await nameHandle.evaluate(el => el.textContent?.trim());
                        }
            ...
            await AbstractProducer.sendMessage('Burke', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Burke', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
        return true;
    }

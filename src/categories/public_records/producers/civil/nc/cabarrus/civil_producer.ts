...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.cabarrusncrod.org/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {            
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Cabarrus');
            ...
            while (true) {
                if (pageNum > 1) {
                    await page.waitForXPath(`//div[@class="DataGridView-PagingStatusBox" and contains(text(), "${pageNum} /")]`, {visible: true});
                } else {
                    await this.sleep(3000);
                }
                await this.sleep(2000);
                await page.waitForXPath('//div[contains(@id, "VWGROW2_152_R")]', {timeout: 3000});
                const rows = await page.$x('//div[contains(@id, "VWGROW2_152_R")]');
                if (rows.length > 0) {
                    for (let i = 0; i < rows.length; i++) {
                        await this.sleep(2000);
                        const dateHandle = await page.$x(`//div[contains(@id, "VWGROW2_152_R")][${i + 1}]/div[3]//span`);
                        let date = await dateHandle[0].evaluate(el => el.textContent?.trim());
                        const caseHandle = await page.$x(`//div[contains(@id, "VWGROW2_152_R")][${i + 1}]/div[4]//span`);
                        let caseID = await caseHandle[0].evaluate(el => el.textContent?.trim());
                        const typeHandle = await page.$x(`//div[contains(@id, "VWGROW2_152_R")][${i + 1}]/div[7]//span`);
                        let type = await typeHandle[0].evaluate(el => el.textContent?.trim()); 
                        const clickHandle = await page.$x(`//div[contains(@id, "VWGROW2_152_R")][${i + 1}]/div[1]`)
                        await Promise.all([
                            clickHandle[0].click(),
                            page.waitForXPath(`//div[@id="VWGROW2_161_R0"]//span[contains(text(), "${caseID}")]`)
                        ])
                        
                        const nameXpath = `//div[contains(@id, "VWGROW2_174_R")]/div[1]//span[contains(text(), "2")]/parent::div/parent::div/parent::div/parent::div/parent::div/parent::div/parent::div/div[2]//span`;
                    ...
                    const nextEL = await page.$x(`//a[contains(@onclick, "NavigateNext")]`);
                    if (nextEL.length > 0) {
                        pageNum++;
            ...
            await AbstractProducer.sendMessage('Cabarrus', 'North Carolina', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Cabarrus', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
        }

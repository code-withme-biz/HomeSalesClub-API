...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://www.courthousecomputersystems.com/DareNCNW/application.asp'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Dare');
            ...
            await AbstractProducer.sendMessage('Dare', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Dare', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }

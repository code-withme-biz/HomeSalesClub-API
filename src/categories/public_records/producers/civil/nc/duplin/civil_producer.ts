...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://72.15.246.183/duplinncnw/application.asp'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Duplin');
            ...
            await AbstractProducer.sendMessage('Duplin', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Duplin', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://forsythdeeds.com/forsythNameSearch.php?Accept=Accept'
    }

    xpaths = {
        isPAloaded: '//input[@id="party_type"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try{
            ...
            let dateRange = await this.getDateRange('North Carolina', 'Forsyth');
            ...
            while (fromDate <= toDate) {
                ...
                try{
                    ...
                    await page.click('#all_instcodes');
            ...
            await AbstractProducer.sendMessage('North Carolina', 'Forsyth', countRecords, 'Civil');
            return true;
        } catch (error){
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('North Carolina', 'Forsyth', countRecords, 'Civil', errorImage);
            return false;
        }
    }
}
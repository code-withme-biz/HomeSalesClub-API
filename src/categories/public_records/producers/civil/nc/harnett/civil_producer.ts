...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://rod.harnett.org/RealEstate/SearchEntry.aspx?e=newSession'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            let dateRange = await this.getDateRange('North Carolina', 'Harnett');
            ...
            await AbstractProducer.sendMessage('Harnett', 'North Carolina', countRecords, 'Civil & Lien');
            return true;
        } catch (err) {
            console.log(err);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Harnett', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false
        }
    }
}
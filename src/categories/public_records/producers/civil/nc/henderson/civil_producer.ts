...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://henderson.courtcompsys.com/HendersonNCNW/application.asp?resize=true'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Handerson');
            ...
            await AbstractProducer.sendMessage('Handerson', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Handerson', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }

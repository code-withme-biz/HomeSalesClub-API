...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'http://www.hokencrod.org/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Hoke');
            ...
            while (true) {
                ...
                try {
                    ...
                    console.log('retrying loading data --- ', retry_count1);
                    retry_count1++
                }
            }
            ...
            while (true) {
                ...
                if (rows.length > 0) {
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        const clickHandle = await page.$x(`//div[contains(@id, "VWGROW2_140_R")][${i + 1}]//table[contains(@id, "TRG_140_D")]/parent::div/parent::div`)
                        await Promise.all([
                            clickHandle[0].click(),
                    ...
                    const nextEL = await page.$x(`//a[contains(@onclick, "s237.sb68('140')")]`);
                    if (nextEL.length > 0) {
                        pageNum++;
            ...
            await AbstractProducer.sendMessage('Hoke', 'North Carolina', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Hoke', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
        }

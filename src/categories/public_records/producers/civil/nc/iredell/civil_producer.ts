...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://cotthosting.com/NCIredellExternal/User/Login.aspx'
    }

    xpaths = {
        isPAloaded: '//input[@name="ctl00$cphMain$blkLogin$btnGuestLogin"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        const civilUrl: string = 'https://cotthosting.com/NCIredellExternal/User/Login.aspx';
        let countRecords = 0;

        try {
            let dateRange = await this.getDateRange('North Carolina', 'Iredell');

            await page.goto(civilUrl, { timeout: 60000 });
            await page.waitForXPath('//input[@name="ctl00$cphMain$blkLogin$btnGuestLogin"]', { visible: true, timeout: 200000 });
            let signinAsGuest = await page.$x('//input[@name="ctl00$cphMain$blkLogin$btnGuestLogin"]');
            ...
            await AbstractProducer.sendMessage('Iredell', 'North Carolina', countRecords, 'Civil & Lien');
            return true;
        } catch (e) {
            console.log(e);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Iredell', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
    }
}
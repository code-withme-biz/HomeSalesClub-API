...
const removeRowArray = [
    ...
    'ACADEMY', 'VENTURE', 'REVOCABLE', 'CONSTRUCTION', 'HOMETOWN', 'Person', 'CALIFORNIA', 'X',
...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://72.15.246.183/personncnw/application.asp?resize=true'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {                        
            const dateRange = await this.getDateRange('North Carolina', 'Person');
            ...
            await AbstractProducer.sendMessage('Person', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Person', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
        return true;
    }

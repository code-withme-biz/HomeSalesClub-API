...
const removeRowArray = [
    ...
    'ACADEMY', 'VENTURE', 'REVOCABLE', 'CONSTRUCTION', 'HOMETOWN', 'Randolph', 'CALIFORNIA', 'X',
...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://72.15.246.186/RandolphNCNW/application.asp?resize=true'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {                        
            const dateRange = await this.getDateRange('North Carolina', 'Randolph');
            ...
            await AbstractProducer.sendMessage('Randolph', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Randolph', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
        return true;
    }

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://cottweb.rutherfordcountync.gov/External/LandRecords/protected/v4/SrchName.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'http://cottweb.rutherfordcountync.gov/External/LandRecords/protected/v4/SrchName.aspx';
        let page = this.browserPages.generalInfoPage!;
        let countRecords = 0;

        try {
            let dateRange = await this.getDateRange('North Carolina', 'Rutherford');
            ...
            for (let i = 1; i <= parseInt(totalPagesUse!); i++) {
                ...
                for (let k = 2; k < totalRowHandle!.length; k++) {
                    ...
                    if (docType == 'SAT D/T') {
                        docType = 'SATISFACTION OF DEED OF TRUST'
                    }
            ...
            await AbstractProducer.sendMessage('Rutherford', 'North Carolina', countRecords, 'Civil & Lien');
            return true;
        } catch (e) {
            console.log(e);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Rutherford', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
    }
}
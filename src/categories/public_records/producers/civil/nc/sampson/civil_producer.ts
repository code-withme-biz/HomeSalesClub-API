...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'http://www.sampsonrod.org/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {            
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Sampson');
            ...
            await AbstractProducer.sendMessage('Sampson', 'North Carolina', countRecords, 'Civil & Lien');
        }
        catch (error1) {
            console.log('Error: ', error1);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Sampson', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
        }

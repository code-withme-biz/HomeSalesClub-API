...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'http://www.stokescorod.org/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {            
            ...
            const dateRange = await this.getDateRange('North Carolina', 'Stokes');
            ...
            while (true) {
                ...
                const rows = await page.$x('//div[contains(@id, "VWGROW2_154_R")]');
                if (rows.length > 0) {
                    for (let i = 0; i < rows.length; i++) {
                        await this.sleep(2000);
                        const dateHandle = await page.$x(`//div[contains(@id, "VWGROW2_154_R")][${i + 1}]/div[3]//span`);
                        let date = await dateHandle[0].evaluate(el => el.textContent?.trim());
                        const caseHandle = await page.$x(`//div[contains(@id, "VWGROW2_154_R")][${i + 1}]/div[4]//span`);
                        let caseID = await caseHandle[0].evaluate(el => el.textContent?.trim());
                        const typeHandle = await page.$x(`//div[contains(@id, "VWGROW2_154_R")][${i + 1}]/div[7]//span`);
                        let type = await typeHandle[0].evaluate(el => el.textContent?.trim()); 
                        const clickHandle = await page.$x(`//div[contains(@id, "VWGROW2_154_R")][${i + 1}]/div[1]`)
                        await Promise.all([
                            clickHandle[0].click(),
                            page.waitForXPath(`//div[@id="VWGROW2_163_R0"]//span[contains(text(), "${caseID}")]`)
                        ])
                        
                        const nameXpath = `//div[contains(@id, "VWGROW2_176_R")]/div[1]//span[contains(text(), "2")]/parent::div/parent::div/parent::div/parent::div/parent::div/parent::div/parent::div/div[2]//span`;
            ...
            await AbstractProducer.sendMessage('Stokes', 'North Carolina', countRecords, 'Civil & Lien');
            return true;
        }
        catch (error1) {
            console.log('Error: ', error1);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Stokes', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
        }

...
const removeRowArray = [
    ...
    'ACADEMY', 'VENTURE', 'REVOCABLE', 'CONSTRUCTION', 'HOMETOWN', 'Surry', 'CALIFORNIA', 'X',
...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://rod.surryinfo.net/surryncnwpub/application.asp?resize=true'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {                        
            const dateRange = await this.getDateRange('North Carolina', 'Surry');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) { 
                ...
                const eitherHandle = await frame.$x('//input[@id="searchtype" and @value="3"]');
                ...
                while (true) {
                    ...
                    if (rows.length > 0) {
                        for (let j = 0; j < rows.length; j++) {
                            let dateHandle = await frame.$x(`//div[@id="resultspane"]/table//tr[contains(@style, "cursor")][${j + 1}]/td[contains(@class, "c4")]`);
                            let date = await dateHandle[0].evaluate(el => el.textContent?.trim());
                            let caseHandle = await frame.$x(`//div[@id="resultspane"]/table//tr[contains(@style, "cursor")][${j + 1}]/td[contains(@class, "c12")]/div`);
                            let caseID = await caseHandle[0].evaluate(el => el.textContent?.trim());
                            let nameHandle = await frame.$x(`//div[@id="resultspane"]/table//tr[contains(@style, "cursor")][${j + 1}]/td[contains(@class, "c7")]/div`);
                            let name = await nameHandle[0].evaluate(el => el.textContent?.trim());
                            let typeHandle = await frame.$x(`//div[@id="resultspane"]/table//tr[contains(@style, "cursor")][${j + 1}]/td[contains(@class, "c8")]/div`);
            ...
            await AbstractProducer.sendMessage('Surry', 'North Carolina', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Surry', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
        return true;
    }

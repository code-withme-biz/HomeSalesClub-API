...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://rod.wilson-co.com/External/LandRecords/protected/v4/SrchName.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        const civilUrl: string = 'http://rod.wilson-co.com/External/LandRecords/protected/v4/SrchName.aspx';
        let page = this.browserPages.generalInfoPage!;
        let countRecords = 0;

        try {
            let dateRange = await this.getDateRange('North Carolina', 'Wilson');
            ...
            await AbstractProducer.sendMessage('Wilson', 'North Carolina', countRecords, 'Civil & Lien');
            return true;
        } catch (e) {
            console.log(e);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Wilson', 'North Carolina', countRecords, 'Civil & Lien', errorImage);
            return false;
        }
    }
}
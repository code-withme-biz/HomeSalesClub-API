...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://nh5laredo.fidlar.com/nhrockingham/AvaWeb/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://nh5laredo.fidlar.com/nhrockingham/AvaWeb/#!/search';
            let dateRange = await this.getDateRange('New Hampshire', 'Rockingham');
            ...
            await AbstractProducer.sendMessage('Rockingham', 'New Hampshire', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Rockingham', 'New Hampshire', countRecords, 'Civil & Lien');

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://ava.fidlar.com/NHStrafford/AvaWeb/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://ava.fidlar.com/NHStrafford/AvaWeb/#!/search';
            let dateRange = await this.getDateRange('New Hampshire', 'Strafford');
            ...
            while (fromDate <= toDate) {
                ...
                for (let i = 0; i < resultRows.length; i++) {
                    let names = [];
                    let indXpath = i + 1;
                    await resultRows[i].click();
                    await page.waitForXPath('//div[@id="resultsContainer"]/ul/li['+indXpath+']/div/div/div[2]/div[2]/div/div[1]/label[2]');
                    let directNameHandle = await page.$x('//div[@id="resultsContainer"]/ul/li['+indXpath+']/div/div/div[2]/div[2]/div/div[1]/label[2]');
                    let directName = await directNameHandle[0].evaluate(el => el.textContent?.trim());
                    let indirectNameHandle = await page.$x('//div[@id="resultsContainer"]/ul/li['+indXpath+']/div/div/div[2]/div[2]/div/div[2]/label[2]');
            ...
            await AbstractProducer.sendMessage('Strafford', 'New Hampshire', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Strafford', 'New Hampshire', countRecords, 'Civil & Lien');

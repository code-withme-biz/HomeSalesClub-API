...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://press.co.burlington.nj.us/Press/clerk/ClerkHome.aspx?op=basic'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            await page.select('select[name="ctl00$ContentPlaceHolder1$ddlTotalRecTab2"]', '2000');

            let docTypeSelects = [
                'AFF', 'ANE', 'AGR', 'AL', 'AMA', 'AMB', 'AMC', 'ACL', 'ACLN', 'AMDR', 'AMDT', 
                'AMJ', 'AML', 'AMMD', 'AORD', 'AMD', 'AMEN', 'AJUD', 'ASSN', 'ASA', 'ASC', 'ASL', 
                'ASLR', 'ASLI', 'ASM', 'ASR', 'ASTS', 'ATS', 'ASGN', 'ASMP', 'BOS', 'BCP', 'BCS', 
                'CNB', 'BLAW', 'CNM', 'CTSL', 'CCJ', 'CERT', 'CER', 'COR', 'CON', 'CHECKS', 'CLL', 
                'COMP', 'CLN', 'CLD', 'CONS', 'COJ', 'COO', 'COW', 'CMT', 'CL', 'CNLD', 'CLDB', 
                'CNTR', 'CDR', 'CAG', 'CAS', 'CRD', 'CMTG', 'CPOA', 'CRW', 'CSD', 'CSA', 'CCJR', 
                'CASLR', 'CWETL', 'CRIM', 'DAGR', 'DER', 'DECR', 'DRPA', 'DCRB', 'DCRC', 'DCT', 
                'DET', 'DEED', 'DOD', 'DEEDNC', 'DDCT', 'DSA', 'DAL', 'DASN', 'DJG', 'DIS', 'DORD', 
                'DSC', 'DYFS', 'EAS', 'ESC', 'EXA', 'EXL', 'EXT', 'FTL', 'FTLD', 'FTLR', 'FTLV', 
                'FTLW', 'MAP', 'FJD', 'GRT', 'IREM', 'ITW', 'JUDG', 'LAPS', 'LSE99', 'LSE', 'LIAG', 
                'LIS', 'LPA', 'LPD', 'LPR', 'MSR', 'MGB', 'MADED', 'MSD', 'MCL', 'MCLD', 'MCLR', 
                'MNI', 'MGR', 'MSB', 'MSBD', 'MNID', 'MAGR', 'MOD', 'MTG', 'MSA', 'MTGNC', 'MDED', 
                'MCOR', 'MDECR', 'MDOD', 'MEAS', 'MLSE', 'MRELR', 'MTAG', 'MUN', 'NOTC', 'NSM', 
                'OPT', 'ORD', 'ODN', 'PMTG', 'POA', 'RTSL', 'RLS', 'RAGR', 'RCTR', 'RELJ', 'RLSE', 
                'RLB', 'RL', 'RELL', 'RLM', 'RORD', 'RELR', 'RLRW', 'RLR', 'REL', 'RPTCR', 'RAPT', 
                'RSL', 'RAG', 'RVAG', 'RVDSC', 'RVMD', 'RVPA', 'RVTR', 'RRFTL', 'ROW', 'SCAG', 
                'SEC', 'SHB', 'SDED', 'SHR', 'WETL', 'SSD', 'SWL', 'SAGR', 'SAGRNC', 'SOM', 'SUB', 
                'SPAG', 'APAG', 'SDR', 'SMTG', 'SORD', 'TSL', 'TAG', 'TOL', 'TRW', 'TERM', 'TLT', 
                'TWPC', 'TWPD', 'TRAG', 'CONT', 'CONTNC', 'VAC', 'WVR', 'WVL', 'WAR', 'WRS', 'WSJ', 
                'WLFS', 'WLF', 'WOE'
            ];
            const dateRange = await this.getDateRange('New Jersey', 'Burlington');
            ...
            await AbstractProducer.sendMessage('Burlington', 'New Jersey', countRecords, 'Civil & Lien');
            return true;
        }
        catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Burlington', 'New Jersey', countRecords, 'Civil & Lien', errorImage);
        }

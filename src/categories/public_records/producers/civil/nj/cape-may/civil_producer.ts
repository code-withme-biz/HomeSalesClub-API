...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'http://clerk.capemaycountynj.gov/publicsearch/'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        let dateRange = await this.getDateRange('New Jersey', 'cape-may');
        ...
        try {
            ...
            for (let i = days < 1 ? 1 : days ; i >= 0 ; i--) {
                ...
                while (keys.length > 0) {
                    ...
                    const resp = await axios.post('http://clerk.capemaycountynj.gov/publicsearch/api/search', formData);
                    if (resp.status == 200) {

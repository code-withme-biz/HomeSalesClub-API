...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPageLoaded: '//a[text()="Cumberland County Clerk"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const dateRange = await this.getDateRange('New Jersey', 'Cumberland');

            try {
                const [docBtnHandle] = await page.$x('//a[text()="Cumberland County Clerk"]');
                await docBtnHandle.click();
                await page.waitForNavigation();
            } catch (error) {
                console.log(error);
                await AbstractProducer.sendMessage('Cumberland', 'New Jersey', countRecords, 'Civil & Lien');
                return false;
            }

            await page.waitForXPath('//input[contains(@onclick, "doGuestLogin(true")]');
            const [guestHandle] = await page.$x('//input[contains(@onclick, "doGuestLogin(true")]');
            ...
            await fromDate?.type(this.getFormattedDate(dateRange.from), {delay: 150});
            await endDate?.click();
            await endDate?.type(this.getFormattedDate(dateRange.to), {delay: 150});

            let [unCheck]: any = await frame5?.$x('//ul[@id="instTree"]/li/div/span[3]');
            await unCheck?.click();
            let [deed]: any = await frame5?.$x('//div[@id="_easyui_tree_10"]/span[4]');
            await deed?.click();
            let [lien]: any = await frame5?.$x('//div[@id="_easyui_tree_11"]/span[4]');
            await lien?.click();
            let [lis]: any = await frame5?.$x('//div[@id="_easyui_tree_12"]/span[4]');
            await lis?.click();
            let [mortgage]: any = await frame5?.$x('//div[@id="_easyui_tree_15"]/span[4]')
                ...
                while (!isLast) {
                    ...
                    if (results) {
                        for (const result of results) {
                            ...
                            let namesHTML = await result.evaluate(el => el.children[6].children[0].children[0].innerHTML);
                            let names = namesHTML.split('<br>');
                            console.log(names);
                            let type = await result.evaluate(el => el.children[2].children[0].textContent?.trim());
                            let date = await result.evaluate(el => el.children[9].children[0].textContent?.trim());
            ...
            await AbstractProducer.sendMessage('Cumberland', 'New Jersey', countRecords, 'Civil & Lien', errorImage);
        }

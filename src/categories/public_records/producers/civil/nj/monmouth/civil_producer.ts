...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://oprs.co.monmouth.nj.us/oprs/clerk/ClerkHome.aspx?op=basic'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            let docTypeSelects = [
                'CONSENT', 'AIRCRAFT', 'LISPENAM', 'PARTAMEN', 'CLIENAM', 'AMTGALR', 
                'ASGNMTG', 'ASGNTSC', 'JUDGASSN', 'BILLSALE', 'BLDG', 'BLDGDIS', 
                'BLDGPLAN', 'BLDGREL', 'BLDGSPEC', 'BLDGSUB', 'MTGCANC', 'COMMSUIT', 
                'CLIEN', 'CLBONDC', 'CLBOND', 'CLIENDIS', 'CONTSALE', 'JUDGFINL', 
                'LEASE', 'DB_CONV', 'DEED', 'LISPENDI', 'MTGDIS', 'RFILEDIS', 'DISCLAIM', 
                'TRNAMEDI', 'MTGEXT', 'LISPENFO', 'CORP', 'INCORP', 'ILIEN', 'ILIENREL', 
                'ILIENW', 'JUDG', 'LEASEAGR', 'LISINRM', 'LISPEN', 'MTG', 'MTGAMEND', 
                'MTGALR', 'MTGMOD', 'MTGPOST', 'MTGSUBT', 'MTGSUB', 'MB_CONV', 'NJTXWVR', 
                'NOTSETCM', 'NOTSETC', 'NOTSETM', 'PART', 'POA', 'REDTSC', 'MTGREL', 
                'JUDGREL', 'POAREV', 'RFILE', 'ROADS', 'SHERBOND', 'NJPERMIT', 'STOP', 
                'TAXSALE', 'TSCCANC', 'TRNAME', 'UCCASGN', 'UCCAMEN', 'UCCCONT', 
                'UCCDF', 'UCC', 'UCCPR', 'UCCTERM', 'VACRDS'
            ];
            const dateRange = await this.getDateRange('New Jersey', 'Monmouth');
            ...
            await AbstractProducer.sendMessage('Monmouth', 'New Jersey', countRecords, 'Civil & Lien');
            return true;
        }
        catch (error) {
            console.log(error);
            const errorImage = await this.uploadImageOnS3(page);
            await AbstractProducer.sendMessage('Monmouth', 'New Jersey', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYALB/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Albany');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Albany', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = [
                `BLA`, `INC`, `CIV`, `CVL`, `CRI`, `DBA`, `D`, `FTL`,
                `J`, `L`, `LP`, `MAP`, `ML`, `MISC`, `M`, `NL`, `SA`,
                `SCA`, `SSL`, `JT`, `U`, `W`
            ];
            ...
            await AbstractProducer.sendMessage('Albany', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

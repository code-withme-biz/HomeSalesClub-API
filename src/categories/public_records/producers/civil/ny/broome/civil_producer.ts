...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYBRO/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Broome');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Broome', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = [
                `CIV`, `CORP`, `CRI`, `DBA`, `D`, `FTL`, `J`,
                `L`, `LP`, `MAP`, `MISC`, `M`, `SCA`, `U`
            ];
            ...
            await AbstractProducer.sendMessage('Broome', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

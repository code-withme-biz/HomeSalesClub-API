...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYCAY/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Cayuga');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Cayuga', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = [
                `CIV`,`CL`, `DBA`, `FTL`, `J`, `LP`, `MAP`,
                `MAT`, `MH`, `MR`, `RP`, `SA`, `SCA`, `W`, `U`
            ];
            ...
            await AbstractProducer.sendMessage('Cayuga', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

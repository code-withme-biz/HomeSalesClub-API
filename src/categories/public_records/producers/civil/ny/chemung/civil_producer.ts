...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Chemung');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
            
            // select chemung county
            await page.click('input[value="33"]');
            ...
            if (!result) {
                await AbstractProducer.sendMessage('Chemung', 'New York', countRecords, 'Civil & Lien');
                return false;
            }
            ...
            await AbstractProducer.sendMessage('Chemung', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

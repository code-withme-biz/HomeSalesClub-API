...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPageLoaded: '//a[text()="Monroe County Clerk"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const dateRange = await this.getDateRange('New York', 'Monroe');
            let fromDateVal = this.getFormattedDate(dateRange.from);
            let toDateVal = this.getFormattedDate(dateRange.to);

            try {
                const [docBtnHandle] = await page.$x('//a[text()="Monroe County Clerk"]');
            ...
            await page.waitForXPath(`//input[contains(@onclick, "doGuestLogin(false, 'StartPage')")]`);
            const [guestHandle] = await page.$x(`//input[contains(@onclick, "doGuestLogin(false, 'StartPage')")]`);
            ...
            if (text?.includes('No documents')) {
                ...
                while (!isLast) {
                    ...
                    if (results) {
                        ...
                        //     let namesHTML = await result.evaluate(el => el.children[0].children[0].children[0].children[0].children[4].children[0].children[0].innerHTML);
                        ...
                        //     let date = await result.evaluate(el => el.children[0].children[0].children[0].children[0].children[10].children[0].textContent?.trim());
                        ...
                        for (let i = 0; i < results.length; i++) {
                            let caseHandle = await frame10?.$x(`//div[@id="instList"]//div[@class="datagrid-view2"]/div[@class="datagrid-body"]/table/tbody/tr[${i + 1}]//tr/td[3]//a`);
                            if (!caseHandle) {
                                continue;
                            }
                            let caseID = await caseHandle[0].evaluate(el => el.textContent?.trim());
                            let nameHandle = await frame10?.$x(`//div[@id="instList"]//div[@class="datagrid-view2"]/div[@class="datagrid-body"]/table/tbody/tr[${i + 1}]//tr/td[5]//span`);
                            if (!nameHandle) {
                                continue;
                            }
                            let namesHTML = await nameHandle[0].evaluate(el => el.innerHTML);
                            let names = namesHTML.split('<br>');
                            let typeHandle = await frame10?.$x(`//div[@id="instList"]//div[@class="datagrid-view2"]/div[@class="datagrid-body"]/table/tbody/tr[${i + 1}]//tr/td[9]/div`);
                            if (!typeHandle) {
                                continue;
                            }
                            let type = await typeHandle[0].evaluate(el => el.textContent?.trim());
                            type = type?.replace(/[^a-zA-Z ]/g, "");
                            let dateHandle = await frame10?.$x(`//div[@id="instList"]//div[@class="datagrid-view2"]/div[@class="datagrid-body"]/table/tbody/tr[${i + 1}]//tr/td[11]/div`);
                            if (!dateHandle) {
                                continue;
                            }
            ...
            await AbstractProducer.sendMessage('Monroe', 'New York', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Monroe', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYMONT/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Montgomery');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Montgomery', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = [
                `CIV`, `COR`, `DBA`, `FTL`, `J`,
                `MAP`, `MF`, `NAT`, `O`, `ORD`, `RP`, `UCC`
            ...
            await AbstractProducer.sendMessage('Montgomery', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

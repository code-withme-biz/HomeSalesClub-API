...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://i2f.uslandrecords.com/NY/Nassau/D/Default.aspx?'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Nassau');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);

            // setting search type
            await page.select('select[name="SearchCriteriaName1$DDL_SearchName"]', 'Deeds/Mortgages Recorded Date Search');
                ...
                try {
                    ...
                    if (alert.length > 0) {
                        ...
                        while (!isLast) {
                            ...
                            const results = await page.$x('//div[@id="DocList1_ContentContainer1"]//table//tbody/tr[contains(@class, "DataGrid")]');
                            ...
                            for (let i = 0; i < results.length; i++) {
                                ...
                                let caseID;
                                while (true) {
                                    caseID = await results[i].evaluate(el => el.children[4].children[0].textContent?.trim());
                                    const detailHandle = await page.$x(`//div[@id="DocList1_ContentContainer1"]//table//tbody/tr[contains(@class, "DataGrid")][${i + 1}]/td[2]/a`);
                                    await detailHandle[0].click();
                                    try {
            ...
            await AbstractProducer.sendMessage('Nassau', 'New York', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Nassau', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

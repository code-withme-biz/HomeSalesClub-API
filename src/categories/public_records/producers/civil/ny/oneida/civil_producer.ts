...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYONE/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Oneida');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Oneida', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = [
                `CS`, `CIV`, `COR`, `CRI`, `DBA`, `DV`, `JUD`,
                `LN`, `LP`, `MAP`, `MC`, `MF`, `MR`, `O`, `RP`,
                `SH`, `W`, `UCC`
            ];
            ...
            await AbstractProducer.sendMessage('Oneida', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const dateRange = await this.getDateRange('New York', 'Schenectady');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
            
            // select Schenectady county
            await page.click('input[value="34"]');
            ...
            if (!result) {
                await AbstractProducer.sendMessage('Schenectady', 'New York', countRecords, 'Civil & Lien');
                return false;
            }
            ...
            await AbstractProducer.sendMessage('Schenectady', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

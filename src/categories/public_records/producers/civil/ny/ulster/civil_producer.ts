...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://www.searchiqs.com/NYULS/Login.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('New York', 'Ulster');
            ...
            try {
                ...
                console.log(error);
                await AbstractProducer.sendMessage('Ulster', 'New York', countRecords, 'Civil & Lien');
                return false;
            }

            let docTypeSelects = ['AMENDED FED TAX LIEN', 'AMENDED LIS PENDENS', 'CIVIL', 'DEED', 'FEDERAL TAX LIEN', 'FORECLOSURE', 'LIS PENDENS', 'MORTGAGE'];
            for (const docTypeSelect of docTypeSelects) {
                ...
                // setting doc type
                await page.select('select[id="ContentPlaceHolder1_cboDocType"]', docTypeSelect);
            ...
            await AbstractProducer.sendMessage('Ulster', 'New York', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Auglaize")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Auglaize');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Auglaize")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Auglaize', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Auglaize', 'Ohio', countRecords, 'Civil & Lien');
            return false;

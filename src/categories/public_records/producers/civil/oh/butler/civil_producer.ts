...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Butler")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Butler');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Butler")]');
            await Promise.all([
                county_handle.click(),
            ...
            while (nextPage) {
                ...
                await body_frame.waitForXPath('//iframe[@name="progressFrame"]', {visible: true});
                let [progress_frame]: any = await body_frame.$x('//iframe[@name="progressFrame"]');
                progress_frame = await progress_frame.contentFrame();

                const [loading_message] = await progress_frame.$x('//div[@id="message"][contains(text(), "Retrieving next page of results")]');
                if (loading_message) {
                    let retries = 0;
                    while (true) {
                        const [loading_message] = await progress_frame.$x('//div[@id="message"][contains(text(), "Retrieving next page of results")]');
            ...
            await AbstractProducer.sendMessage('Butler', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Butler', 'Ohio', countRecords, 'Civil & Lien');
            return false;

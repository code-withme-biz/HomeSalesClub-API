...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Champaign")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Champaign');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Champaign")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Champaign', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Champaign', 'Ohio', countRecords, 'Civil & Lien');
            return false;

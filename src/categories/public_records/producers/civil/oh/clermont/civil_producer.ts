...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Clermont")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Clermont');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Clermont")]');
            await Promise.all([
                county_handle.click(),
            ...
            const [login_as_public] = await page.$x('//input[contains(@value, "Login as Public")]');
            await Promise.all([
            ...
            while (nextPage) {
                ...
                for (const row of resultRows) {
                    let names = await result_list_frame.evaluate((el: any) => el.children[8].innerText.trim(), row);
                    console.log(names);
                    names = names.split('\n');
                    names = names.filter((name:string) => name.trim() !== '');
                    let recordDate = await result_list_frame.evaluate((el: any) => el.children[5].textContent.trim(), row);
                    let caseType = await result_list_frame.evaluate((el: any) => el.children[6].textContent.trim(), row);
            ...
            await AbstractProducer.sendMessage('Clermont', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Clermont', 'Ohio', countRecords, 'Civil & Lien');
            return false;

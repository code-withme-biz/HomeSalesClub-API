...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Coshocton")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Coshocton');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Coshocton")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Coshocton', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Coshocton', 'Ohio', countRecords, 'Civil & Lien');
            return false;

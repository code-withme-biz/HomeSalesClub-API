...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Crawford")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Crawford');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Crawford")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Crawford', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Crawford', 'Ohio', countRecords, 'Civil & Lien');
            return false;

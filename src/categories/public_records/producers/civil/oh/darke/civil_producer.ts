...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Darke")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Darke');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Darke")]');
            await Promise.all([
                county_handle.click(),
            ...
            await page.waitForXPath('//input[contains(@value, "Login as Guest")]');
            ...
            await AbstractProducer.sendMessage('Darke', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Darke', 'Ohio', countRecords, 'Civil & Lien');
            return false;

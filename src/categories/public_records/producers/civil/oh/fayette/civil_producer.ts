...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Fayette")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Fayette');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Fayette")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Fayette', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Fayette', 'Ohio', countRecords, 'Civil & Lien');
            return false;

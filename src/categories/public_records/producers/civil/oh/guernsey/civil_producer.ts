...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Guernsey")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Guernsey');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Guernsey")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Guernsey', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Guernsey', 'Ohio', countRecords, 'Civil & Lien');
            return false;

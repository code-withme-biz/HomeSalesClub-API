...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Hardin County Recorder")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {   
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Hardin');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Hardin County Recorder")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Hardin', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Hardin', 'Ohio', countRecords, 'Civil & Lien');
            return false;

...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Pickaway")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Pickaway');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Pickaway")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Pickaway', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Pickaway', 'Ohio', countRecords, 'Civil & Lien');
            return false;

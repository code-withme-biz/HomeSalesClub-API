...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Ross")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Ross');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Ross")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Ross', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Ross', 'Ohio', countRecords, 'Civil & Lien');
            return false;

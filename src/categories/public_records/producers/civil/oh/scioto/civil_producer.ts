...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://ohscioto.fidlar.com/OHScioto/AvaWeb/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        let countRecords = 0;
        try{
            const civilUrl: string = 'https://ohscioto.fidlar.com/OHScioto/AvaWeb/#!/search';
            let dateRange = await this.getDateRange('Ohio', 'Scioto');
            ...
            await AbstractProducer.sendMessage('Scioto', 'Ohio', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Scioto', 'Ohio', countRecords, 'Civil & Lien');

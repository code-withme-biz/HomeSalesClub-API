...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Tuscarawas")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Tuscarawas');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Tuscarawas")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Tuscarawas', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Tuscarawas', 'Ohio', countRecords, 'Civil & Lien');
            return false;

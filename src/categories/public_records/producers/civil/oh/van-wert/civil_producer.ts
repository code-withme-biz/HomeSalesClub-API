...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPAloaded: '//a[contains(text(), "Van Wert")]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            // get date range
            let dateRange = await this.getDateRange('Ohio', 'Van Wert');
            ...
            // choose county
            const [county_handle] = await page.$x('//a[contains(text(), "Van Wert")]');
            await Promise.all([
                county_handle.click(),
            ...
            await AbstractProducer.sendMessage('Van Wert', 'Ohio', countRecords, 'Civil & Lien');
        } catch (error) {
            console.log(error);
            await AbstractProducer.sendMessage('Van Wert', 'Ohio', countRecords, 'Civil & Lien');
            return false;

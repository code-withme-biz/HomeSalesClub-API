...
export default class CivilProducer extends AbstractProducer {
    ...
    xpaths = {
        isPageLoaded: '//a[text()="Berks County Recorder of Deeds"]'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Berks');
            let fromDateVal = this.getFormattedDate(dateRange.from);
            let toDateVal = this.getFormattedDate(dateRange.to);
			

            try {
                const [docBtnHandle] = await page.$x('//a[text()="Berks County Recorder of Deeds"]');
            ...
            await unCheck?.click();
            let [deed]: any = await frame5?.$x('//div[@id="_easyui_tree_19"]/span[4]');
            await deed?.click();
            let [mortgage]: any = await frame5?.$x('//div[@id="_easyui_tree_22"]/span[4]')
            ...
            await AbstractProducer.sendMessage('Berks', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Berks', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

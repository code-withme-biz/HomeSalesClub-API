...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Carbon');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
            
            // select Carbon county
            await page.click('input[value="21"]');
            ...
            if (!result) {
                await AbstractProducer.sendMessage('Carbon', 'Pennsylvania', countRecords, 'Civil & Lien');
                return false;
            }
            ...
            await AbstractProducer.sendMessage('Carbon', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

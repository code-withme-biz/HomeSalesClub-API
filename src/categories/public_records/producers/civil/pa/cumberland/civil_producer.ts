...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Cumberland');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
            
            // select Cumberland county
            await page.click('input[value="14"]');
            ...
            await AbstractProducer.sendMessage('Cumberland', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Cumberland', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

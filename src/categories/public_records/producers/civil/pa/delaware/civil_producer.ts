...
export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://delcorodonlineservices.co.delaware.pa.us/countyweb/loginDisplay.action?countyname=DelawarePAROD'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Delaware');
            let fromDateVal = this.getFormattedDate(dateRange.from);
            let toDateVal = this.getFormattedDate(dateRange.to);

            await page.waitForXPath('//input[contains(@onclick, "doGuestLogin()")]');
            const [guestHandle] = await page.$x('//input[contains(@onclick, "doGuestLogin()")]');
            const guestHandleResult = await this.waitForSuccess(async () => {
                await Promise.all([
                    guestHandle.click(),
                    page.waitForNavigation()
                ]);
            });
            if (!guestHandleResult) {
                return false;
            }
            ...
            await unCheck?.click();
            let [deed]: any = await frame5?.$x('//div[@id="_easyui_tree_59"]/span[4]');
            await deed?.click();
            let [mortgage]: any = await frame5?.$x('//div[@id="_easyui_tree_70"]/span[4]')
                ...
                while (!isLast) {
                    ...
                    if (results) {
                        for (const result of results) {
                            ...
                            let namesHTML = await result.evaluate(el => el.children[4].children[0].children[0].innerHTML);
                            let names = namesHTML.split('<br>');
                            let type = await result.evaluate(el => el.children[7].children[0].textContent?.trim());
                            let date = await result.evaluate(el => el.children[8].children[0].textContent?.trim());
            ...
            await AbstractProducer.sendMessage('Delaware', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Delaware', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Lehigh');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
			
            
            // select Lehigh county
            await page.click('input[value="22"]');
            ...
            await AbstractProducer.sendMessage('Lehigh', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Lehigh', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const dateRange = await this.getDateRange('Pennsylvania', 'Northumberland');
            let fromDate = this.getFormattedDate(dateRange.from);
            let toDate = this.getFormattedDate(dateRange.to);
            
            // select Northumberland county
            ...
            await AbstractProducer.sendMessage('Northumberland', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Northumberland', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

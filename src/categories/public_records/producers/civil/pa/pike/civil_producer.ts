    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {            
            ...
            const selectCountyResult = await this.waitForSuccess(async () => {
                await Promise.all([
                    page.select('select[name="countycode"]', 'pa103'),
            ...
            const selectDateResult = await this.waitForSuccess(async () => {
                await Promise.all([
                    page.select('select[name="searchType"]', 'searchByDateType'),
                    page.waitForXPath('//input[@name="docTypeVal"]', {visible: true})
                ])
            })
            ...
            // setting date range
            const dateRange = await this.getDateRange('Pennsylvania', 'Pike');
            ...
            // click search button
            const [searchButton] = await page.$x('//input[@id="inputbutton"]');
            ...
            while (!isLast) {
                ...
                if (results.length > 0) {
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        let namesHandles =  await page.$x('//font[contains(text(), "Grantee")]/parent::td/parent::tr/td[2]/font/a/font');
            ...
            await AbstractProducer.sendMessage('Pike', 'Pennsylvania', countRecords, 'Civil & Lien');
            await page.close();
            ...
            await AbstractProducer.sendMessage('Pike', 'Pennsylvania', countRecords, 'Civil & Lien', errorImage);
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://cotthosting.com/SCFlorenceExternal/LandRecords/protected/SrchDateRange.aspx'
    };
    ...
    async read(): Promise<boolean> {
        try {
            await this.browserPages.generalInfoPage?.waitForXPath('//*[contains(@id, "btnGuestLogin")]');
            return true;
        } catch (err) {
    ...
    async getData(page: puppeteer.Page, fillingDate: string) {
        ...
        let currentPage = 1
        try {
            await page.waitForXPath('//*[contains(@id, "cgvResults")]', {timeout: 30000});
            try {
                countPage = (await page.$x('//*[contains(text(), "Page:")]/parent::tr/td')).length;
            } catch (e) {
            }
            do {
                await page.waitForXPath('//*[contains(@id, "cgvResults")]');
                const rows = await page.$x('//*[contains(@id, "cgvResults")]/tbody/tr');
                for (let i = 0; i < rows.length; i++) {
                    let docType = (await rows[i].$eval('td:nth-child(6)', elem => elem.textContent))!.trim();
                    let namesElements = await page.$x(`//*[contains(@id, "cgvResults")]/tbody/tr[${i + 1}]/td[8]//tr`);
                ...
                const [nextPage] = await page.$x(`//tr[@class="GridView_Pager"]//a[text()="${currentPage + 1}"]`);
                if (!!nextPage) {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            await page.waitForXPath('//*[contains(@id, "btnGuestLogin")]')
            const [signGuestBtn] = await page.$x('//*[contains(@id, "btnGuestLogin")]')
            await signGuestBtn.click();
            
            try {
                await page.waitForXPath('//*[contains(@id, "btnEmergencyMessagesClose")]')
                const [acknowledgeBtn] = await page.$x('//*[contains(@id, "btnEmergencyMessagesClose")]')
                await acknowledgeBtn.click();
            } catch (e) {
                
            }
            
            //btnEmergencyMessagesClose
            let dateRange = await this.getDateRange('South Carolina', 'Polk');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('http://cotthosting.com/SCFlorenceExternal/LandRecords/protected/SrchDateRange.aspx', {waitUntil: 'load'});
            ...
            await AbstractProducer.sendMessage('Florence', 'South Carolina', countRecords, 'Civil');
            return false;
        }
        await AbstractProducer.sendMessage('Florence', 'South Carolina', countRecords, 'Civil');
        return true;

...
export default class CivilProducer extends AbstractProducer {
    ...
    async getData(page: puppeteer.Page, fillingDate: string) {
        ...
        try {
            ...
            for (let i = 0; i < linkArray.length; i++) {
                try {
                    await page.goto('https://www.uslandrecords.com' + linkArray[i], {waitUntil: "load"})
                    await page.waitForSelector('#searchResults')
                    const [docTypeElement] = await page.$x('//*[@id="detailsPanel"]//*[contains(text(),"Type")]/parent::tr[1]/following-sibling::tr[1]/td[1]/font'  )           //*[contains(text(),"Type")]/parent::tr[1]/following-sibling::tr[1]/td[1]/font');
                    let docType = (await page.evaluate((e: any) => e.innerText, docTypeElement)).replace('(', '').replace(')', '').trim();
                    const names = await page.$x('//*[@id="detailsPanel"]//*[contains(text(),"Grantee")]/parent::td[1]/following-sibling::td[1]/a')
                    for (let j = 0; j < names.length; j++) {
                        const name = (await names[j].$eval('font', elem => elem.textContent))!.trim()
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            await page.select('select[name="countycode"]', 'sc043')
            let dateRange = await this.getDateRange('South Carolina', 'Georgetown');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                ...
                try {
                    await page.waitForXPath('//*[@id="dateSearch"]/preceding-sibling::h3[1]')
                    const [dateSearchElement] = await page.$x('//*[@id="dateSearch"]/preceding-sibling::h3[1]');
                    await dateSearchElement.click();

                    const dateFormatedToSearch = dateSearch.toLocaleDateString('en-US', {
                        year: "numeric",
                        month: "2-digit",
                        day: "2-digit"
                    })
                    ...
                    await page.evaluate((date:string)=>{
                        ...
                        // @ts-ignore
                        document.querySelector('#todate').value = date;
                    },dateFormatedToSearch)
            ...
            await AbstractProducer.sendMessage('Georgetown', 'South Carolina', countRecords, 'Civil');
            return false;
        }
        await AbstractProducer.sendMessage('Georgetown', 'South Carolina', countRecords, 'Civil');
        return true;

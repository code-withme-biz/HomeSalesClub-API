...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            await page.select('select[name="countycode"]', 'sc055')

            let dateRange = await this.getDateRange('South Carolina', 'Kershaw');
            ...
            await AbstractProducer.sendMessage('Kershaw', 'South Carolina', countRecords, 'Civil');
            return false;
        }
        await AbstractProducer.sendMessage('Kershaw', 'South Carolina', countRecords, 'Civil');
        return true;

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://countyfusion2.kofiletech.us/countyweb/login.do?countyname=SumterSC'
    };
        ...
        try {
            do {
                ...
                for (let i = 0; i < rows.length; i++) {
                    const [docTypeElement] = await resultListFrame.$x(`//*[@class="datagrid-btable"]/tbody/tr[${i + 1}]//td[@field="6"]/div`);
                    const [namesElement] = await resultListFrame.$x(`//*[@class="datagrid-btable"]/tbody/tr[${i + 1}]//td[@field="10"]/div/span`);
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            await page.waitForXPath('//iframe[@name="bodyframe"]');
            let dateRange = await this.getDateRange('South Carolina', 'Sumter');
            ...
            await AbstractProducer.sendMessage('Sumter', 'South Carolina', countRecords, 'Civil & Lien');
            return false
        }
        await AbstractProducer.sendMessage('Sumter', 'South Carolina', countRecords, 'Civil & Lien');
        return true;

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://www.cc.co.bastrop.tx.us/RealEstate/SearchEntry.aspx'
    };
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Texas', 'Bastrop');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('http://www.cc.co.bastrop.tx.us/RealEstate/SearchEntry.aspx', {waitUntil: 'load'});
            ...
            await AbstractProducer.sendMessage('Bastrop', 'Texas', countRecords, 'Civil & Lien');
            return false;
        }
        await AbstractProducer.sendMessage('Bastrop', 'Texas', countRecords, 'Civil & Lien');
        return true;

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://countyfusion11.kofiletech.us/countyweb/login.do?countyname=BellTX'
    };
    ...
    async read(): Promise<boolean> {
        try {
            await this.browserPages.generalInfoPage?.waitForXPath('//*[@type="button" and contains(@value, "Search Records")]');
            return true;
        } catch (err) {
        ...
        try {
            do {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    const [namesElement] = await resultListFrame.$x(`//*[@class="datagrid-btable"]/tbody/tr[${i + 1}]//td[@field="11"]/div/span`);
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            const [clickCivilUnionBtn] = await page.$x('//*[@type="button" and contains(@value, "Search Records")]');
            await Promise.all([
            ...
            let dateRange = await this.getDateRange('Texas', 'Bell');
            ...
            await AbstractProducer.sendMessage('Bell', 'Texas', countRecords, 'Civil, Lien & Probate');
            return false;
        }
        await AbstractProducer.sendMessage('Bell', 'Texas', countRecords, 'Civil, Lien & Probate');
        return true;

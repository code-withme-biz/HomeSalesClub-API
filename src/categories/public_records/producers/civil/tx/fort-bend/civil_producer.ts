...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://ccweb.co.fort-bend.tx.us/RealEstate/SearchEntry.aspx'
    };
        ...
        try {
            do {
                ...
                for (let i = 0; i < rows.length; i++) {
                    await page.waitForXPath('//*[@mkr="dataTbl.hdn"]');
                    const [dateElement] = await page.$x(`//*[@mkr="dataTbl.hdn"]/tbody/tr[${i + 2}]/td[9]`);
                    ...
                    const [clickElement] = await page.$x(`//*[@mkr="dataTbl.hdn"]/tbody/tr[${i + 2}]/td[5]`);
                    ...
                    for (let j = 0; j < nameRows.length; j++) {
                        const [checkNomineeElement] = await page.$x(`//*[@id="Table1"]/tbody/tr[6]/td/table/tbody/tr[${j + 1}]/td[3]`);
                        const checkNominee = (await page.evaluate(e => e.textContent, checkNomineeElement)).trim();
                        if (checkNominee == 'NOMINEE') continue;
                        const [nameElement] = await page.$x(`//*[@id="Table1"]/tbody/tr[6]/td/table/tbody/tr[${j + 1}]/td[2]`);
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Texas', 'Fort Bend');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('http://ccweb.co.fort-bend.tx.us/RealEstate/SearchEntry.aspx', {waitUntil: 'load'});
                    ...
                    await page.keyboard.type(dateSearch.toLocaleDateString('en-US'), {delay: 100});
            ...
            await AbstractProducer.sendMessage('Fort Bend', 'Texas', countRecords, 'Civil & Lien');
            return false;
        }

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://judicial.co.guadalupe.tx.us/Search.aspx?ID=400'
    };
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                try {
                    ...
                    const [caseTypeElement] = await page.$x(`//table[last()]/tbody/tr[${i + 1}]/td[5]/div[1]`);
            ...
            for (let i = 0; i < linkAndTypeArray.length; i++) {
                await page.goto(`http://judicial.co.guadalupe.tx.us/${linkAndTypeArray[i].link}`, {waitUntil: 'load'});
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            ...
            let dateRange = await this.getDateRange('Texas', 'Guadalupe');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    ...
                    await page.goto('http://judicial.co.guadalupe.tx.us/Search.aspx?ID=400');
                    await page.waitForSelector('#DateFiledOnAfter');
                    await page.evaluate(() => {
                        // @ts-ignore
            ...
            await AbstractProducer.sendMessage('Guadalupe', 'Texas', countRecords, 'Civil');
            return false;
        }
        await AbstractProducer.sendMessage('Guadalupe', 'Texas', countRecords, 'Civil');
        return true;

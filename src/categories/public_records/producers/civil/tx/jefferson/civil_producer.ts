...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://jeffersontxclerk.manatron.com/RealEstate/SearchEntry.aspx'
    };
    ...
    async read(): Promise<boolean> {
        try {
            await this.browserPages.generalInfoPage?.waitForXPath('//a[@id="cph1_lnkAccept"]');
            return true;
        } catch (err) {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            let dateRange = await this.getDateRange('Texas', 'Jefferson');
            let date = dateRange.from;
            let today = dateRange.to;
            let days = Math.ceil((today.getTime() - date.getTime()) / (1000 * 3600 * 24)) - 1;
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    await page.goto('https://jeffersontxclerk.manatron.com/RealEstate/SearchEntry.aspx', {waitUntil: 'load'});
                    let [tosButton] = await page.$x('//a[@id="cph1_lnkAccept"]');
                    if (tosButton){
                        await Promise.all([
                            tosButton.click(),
            ...
            await AbstractProducer.sendMessage('Jefferson', 'Texas', countRecords, 'Civil & Lien');
            return false;
        }
        await AbstractProducer.sendMessage('Jefferson', 'Texas', countRecords, 'Civil & Lien');
        return true;

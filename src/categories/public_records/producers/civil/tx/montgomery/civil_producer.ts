...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://odyssey.mctx.org/County/Search.aspx?ID=200'
    };
    ...
    async getData(page: puppeteer.Page, fillingDate: string) {
        ...
        try {
            ...
            for (let i = 0; i < linkArray.length; i++) {
                await page.goto(`http://odyssey.mctx.org/County/${linkArray[i]}`, {waitUntil: 'load'})
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        let countRecords = 0;
        try {
            const [clickCivilSearch] = await page.$x('//a[contains(text(), "Civil & Probate Case Records")]');
            await clickCivilSearch.click();
            await page.waitForSelector('#DateFiledOnAfter');
            let dateRange = await this.getDateRange('Texas', 'Montgomery');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    ...
                    await page.goto('http://odyssey.mctx.org/County/Search.aspx?ID=200');
                    await page.waitForSelector('#DateFiledOnAfter');
                    await page.evaluate(() => {
                        // @ts-ignore
            ...
            await AbstractProducer.sendMessage('Montgomery', 'Texas', countRecords, 'Civil & Probate');
            return false;
        }
        await AbstractProducer.sendMessage('Montgomery', 'Texas', countRecords, 'Civil & Probate');
        return true;

...
const removeRowArray = [
    ...
    'ACADEMY', 'VENTURE', 'REVOCABLE', 'CONSTRUCTION', 'HOMETOWN', 'ORANGE', 'CALIFORNIA'
]
const removeRowRegex = new RegExp(`\\b(?:${removeRowArray.join('|')})\\b`, 'i')

export default class CivilProducer extends AbstractProducer {
    urls = {
        generalInfoPage: 'https://ccrecordse.tarrantcounty.com/RealEstate/SearchResults.aspx'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {          
            const dateRange = await this.getDateRange('Texas', 'Tarrant');
            ...
            await AbstractProducer.sendMessage('Tarrant', 'Texas', countRecords, 'Civil & Lien');
            return true;
        }
        catch (error) {

...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://eweb.washco.utah.gov:8080/recorder/eagleweb/docSearch.jsp'
    };
        ...
        try {
            ...
            do {
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    let nameHandle = await page.$x(`//*[@id="searchResultsTable"]/tbody/tr[${i}]/td[2]/a[contains(.,"From")]`)
                    if (nameHandle.length > 0) {
                        const [dateElement] = await page.$x(`//*[@id="searchResultsTable"]/tbody/tr[${i}]/td[2]/a`);
                        const recordDate = await dateElement.evaluate(el => el.textContent?.split(' ')[0].trim());
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            let dateRange = await this.getDateRange('Utah', 'Washington');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    ...
                    await page.goto('http://eweb.washco.utah.gov:8080/recorder/eagleweb/docSearch.jsp');
            ...
            await AbstractProducer.sendMessage('Washington', 'Utah', countRecords, 'Civil & Lien');
            return false
        }

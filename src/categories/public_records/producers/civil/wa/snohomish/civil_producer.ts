...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'https://www.snoco.org/RecordedDocuments/Home/Index'
    };
    ...
    async read(): Promise<boolean> {
        try {
            await this.browserPages.generalInfoPage?.waitForXPath('//a[@title="Record Date Search" and @aria-label="Record Date Search"]');
            return true;
        } catch (err) {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        try {
            try {
                const [clickSearch] = await page.$x('//a[@title="Record Date Search" and @aria-label="Record Date Search"]');
                await clickSearch.click();
                await page.waitForSelector('#idAcceptYes')
                await page.click('#idAcceptYes')
            } catch (e) {
            }
            let dateRange = await this.getDateRange('Washington', 'Snohomish');
            ...
            for (let i = days < 0 ? 1 : days; i >= 0; i--) {
                try {
                    ...
                    await page.goto('https://www.snoco.org/RecordedDocuments/search/index?theme=.blue&section=searchCriteriaRecordDate&quickSearchSelection=');
            ...
            await AbstractProducer.sendMessage('Snohomish', 'Washington', countRecords, 'Civil & Lien');
            return false
        }
        await AbstractProducer.sendMessage('Snohomish', 'Washington', countRecords, 'Civil & Lien');
        return true;

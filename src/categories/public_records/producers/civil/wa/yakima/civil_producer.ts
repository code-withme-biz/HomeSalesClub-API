...
export default class CivilProducer extends AbstractProducer {
    ...
    urls = {
        generalInfoPage: 'http://recording.yakimacounty.us/WAYakima/DirectSearch/#!/search'
    }
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        // 2000
        try{
            const civilUrl: string = 'http://recording.yakimacounty.us/WAYakima/DirectSearch/#!/search';
            let dateRange = await this.getDateRange('Washington', 'Yakima');
            ...
            await AbstractProducer.sendMessage('Yakima', 'Washington', countRecords, 'Civil & Lien');
            return true;
        } catch (error){
            console.log(error);
            await AbstractProducer.sendMessage('Yakima', 'Washington', countRecords, 'Civil & Lien');

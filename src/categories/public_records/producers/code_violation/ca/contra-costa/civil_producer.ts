...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
        [
            {url: 'https://aca-prod.accela.com/CONCORD/Cap/CapHome.aspx?module=Building&TabName=Building', handler: this.handleSource1}
        ];
    ...
    async getData1(page: puppeteer.Page, sourceId: number) {
        ...
        while (fromDate <= toDate) {
            try {
                await page.goto('https://aca-prod.accela.com/CONCORD/Cap/CapHome.aspx?module=Building&TabName=Building',{waitUntil:'load'})
                ...
                await page.waitForXPath('//table[contains(@id,"gdvPermitList")]', {timeout: 60000});
                ...
                do{
                    ...
                    for (let i = 0; i < totalRow!.length; i++) {
                        try {
                            ...
                            let address = await totalRow[i].$eval('td:nth-child(6) > div > span', elem => elem.textContent);

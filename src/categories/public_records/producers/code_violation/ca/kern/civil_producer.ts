...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://accela.co.kern.ca.us/CitizenAccess/Welcome.aspx', handler: this.handleSource }
    ];
    ...
    async handleSource(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        const isPageLoaded = await this.openPage(page, link, '//a[@title="Search Complaints"]');
        if (!isPageLoaded) {
            console.log()
            return counts;
        }
        const [searchCompliant] = await page.$x('//a[@title="Search Complaints"]');
        if (searchCompliant) {
            await Promise.all([
                searchCompliant.click(),
    ...
    async getData(page: puppeteer.Page, sourceId: number) {
        let counts = 0, pageNum = 1, url = 'https://accela.co.kern.ca.us';

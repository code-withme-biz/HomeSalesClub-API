...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        {url: 'https://citizen.countyofnapa.org/CitizenAccess/Cap/CapHome.aspx?module=Building&TabName=Building', handler: this.handleSource }
      ];
    ...
    async getData(page: puppeteer.Page, sourceId: number) {
        let counts = 0, pageNum = 1, url = 'https://citizen.countyofnapa.org';
        ...
        while (true) {            
            ...
            for (const row of rows) {
                ...
                let link = await row.evaluate(el => el.children[1].children[0].children[1].getAttribute('href'));

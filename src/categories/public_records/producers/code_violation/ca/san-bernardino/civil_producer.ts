...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        { url: 'https://ezop.sbcounty.gov/CitizenAccess/Cap/CapHome.aspx?module=Enforcement&TabName=Home', handler: this.handleSource1 },
        { url: '', installationID: 227, citizenService: true },
        { url: '', installationID: 366, citizenService: true }
      ];
    ...
    async handleSource1(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        let dateRange = {
            from: this.getFormattedDate(new Date(await this.getPrevCodeViolationId(sourceId, true, (new Date('1/1/2015')).getTime()))),
    ...
    async getData1(page: puppeteer.Page, sourceId: number) {
        let counts = 0, pageNum = 1, url = 'https://ezop.sbcounty.gov';

        while (true) {            
            const rowXpath = '//div[@class="ACA_Grid_OverFlow"]//tr[not(contains(@class, "ACA_TabRow_Header")) and contains(@class, "TabRow")]';
            const rows = await page.$x(rowXpath);
            for (const row of rows) {
                let fillingDate = await row.evaluate(el => el.children[10].textContent?.trim());

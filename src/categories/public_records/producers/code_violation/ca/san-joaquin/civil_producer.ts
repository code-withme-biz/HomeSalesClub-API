...
export default class CivilProducer extends AbstractProducer {
    sources =
        [
            { url: 'https://etrakit.cityoftracy.org/etrakit/Search/case.aspx', handler: this.handleSource1 },
        ];
    ...
    async getData1(page: puppeteer.Page, sourceId: number, codeViolationId: number, caseId: string) {
        ...
        for (const row of rows) {
            try {
                ...
                casetype = casetype.replace("CE ", "Code-Enforcement ");

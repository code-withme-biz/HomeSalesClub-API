...
export default class CivilProducer extends AbstractProducer {
    sources =
        [
            { url: 'https://morganhill.trakit.net/etrakit3/Search/case.aspx', handler: this.handleSource1 },
            { url: 'https://trakit.losaltosca.gov/etrakit/Search/case.aspx', handler: this.handleSource2 },
        ];
    ...
    async handleSource1(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        let startCaseCodes = [
            { code: 'ANML', flagStop: false },
            { code: 'BLDG', flagStop: false },
            { code: 'BUS', flagStop: false },
            { code: 'FIRE', flagStop: false },
            { code: 'NOIS', flagStop: false },
            { code: 'OTH', flagStop: false },
            { code: 'PROP', flagStop: false },
            { code: 'ROW', flagStop: false },
            { code: 'SIGN', flagStop: false },
            { code: 'VEH', flagStop: false },
            { code: 'WATR', flagStop: false },
            { code: 'ZONE', flagStop: false },
        ]
    ...
    async handleSource2(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        let startNum = await this.getPrevCodeViolationId(sourceId, false, 1, true);
        let startCaseCodes = [
            { code: 'CODE', flagStop: false },
            { code: 'BLDG', flagStop: false },
            { code: 'ENGR', flagStop: false },
            { code: 'PLNG', flagStop: false },
        ]
        for (let id = startNum; id < startNum + 200; id++) {
            // load page

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://data.hartford.gov/resource/86ax-cfey.json', handler: this.handleSource },
    ];
    ...
    async handleSource(page: puppeteer.Page, url: string, sourceId: number) {
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'reported', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    if (!record.location) continue;
                    const property_address = record.location;
                    const fillingdate = record.reported;
                    const codeViolationId = (new Date(fillingdate)).getTime();
                    const casetype = record.complaintviolation;

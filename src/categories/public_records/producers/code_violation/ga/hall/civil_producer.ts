...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://data.cityofgainesville.org/resource/y6su-758z.json?status=Opened', handler: this.handleSource },
    ];
    ...
    async handleSource(page: puppeteer.Page, url: string, sourceId: number) {
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'infraction', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    if (!record.location) continue;
                    const property_address = record.address;
                    const fillingdate = record.infraction;

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://data.cityofchicago.org/resource/22u3-xenr.json?violation_status=OPEN', handler: this.handleSource },
        { url: 'https://permits.palatine.il.us/EnerGov_Prod/SelfService/PalatineILProd#/search', handler: this.handleSource2 },
    ];
    ...
    async handleSource(page: puppeteer.Page, url: string, sourceId: number) {
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'violation_date', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    const property_address = record.address;
                    const fillingdate = record.violation_date;
                    const codeViolationId = (new Date(fillingdate)).getTime();
                    const casetype = 'Building Violations';
                    const res = {
                        property_address,
                        fillingdate,
                        casetype,

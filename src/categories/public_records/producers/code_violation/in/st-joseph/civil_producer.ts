...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        { url: 'https://aca-prod.accela.com/SOUTHBENDIN/Cap/CapHome.aspx?module=Enforcement', handler: this.handleSource1 },
        { url: 'https://hub.arcgis.com/datasets/SouthBend::code-enforcement-cases/data?orderBy=Record_Open_Date&orderByAsc=false', handler: this.handleSource2 }
      ];
    ...
    async handleSource2(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        let counts = 0;
        // load page
        const isPageLoaded = await this.openPage(page, link, '//*[text()="State ID #"]/ancestor::table[1]/tbody/tr');
        if (!isPageLoaded) {
        ...
        while (true) {
            const rows = await page.$x('//*[text()="State ID #"]/ancestor::table[1]/tbody/tr');
            let flag = false;
            for (const row of rows) {
                let fillingdate = await row.evaluate(el => el.children[8].textContent) || ''
                ...
                let casetype = await row.evaluate(el => el.children[7].textContent) || ''
                casetype = casetype.replace(/\s+|\n/gm, ' ').trim();

                let addr_num = await row.evaluate(el => el.children[1].textContent) || ''
                addr_num = addr_num.replace(/\s+|\n/gm, ' ').trim();
                let addr_dir = await row.evaluate(el => el.children[2].textContent) || ''
                addr_dir = addr_dir.replace(/\s+|\n/gm, ' ').trim();
                let addr_street = await row.evaluate(el => el.children[3].textContent) || ''
                addr_street = addr_street.replace(/\s+|\n/gm, ' ').trim();
                let addr_prefix = await row.evaluate(el => el.children[4].textContent) || ''
                addr_prefix = addr_prefix.replace(/\s+|\n/gm, ' ').trim();
                
                let property_address = `${addr_num} ${addr_dir} ${addr_street} ${addr_prefix}`

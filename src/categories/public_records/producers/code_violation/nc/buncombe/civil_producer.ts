...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://services.ashevillenc.gov/CitizenAccess/Cap/CapHome.aspx?module=Services&TabName=Home', handler: this.handleSource }
    ];
    ...
    async handleSource(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        const isPageLoaded = await this.openPage(page, link, '//div[contains(@id, "_btnNewSearch_container")]//a[@title="Search"]');
        if (!isPageLoaded) {
            console.log()
            return counts;
        }
    ...
    async getData(page: puppeteer.Page, sourceId: number) {
        let counts = 0, pageNum = 1, url = 'https://services.ashevillenc.gov';

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        { url: 'https://lms.parkeronline.org/eTRAKiT3/', handler: this.handleSource1 }
      ];
    username = "webdev";
    password = "Qwer!234";
    ...
    async setSearchCriteria1(page: puppeteer.Page, prefix: number) {
        ...
        let searchKey = `BC${year}-${prefix.toString().padStart(4, '0')}`;
    ...
    async getData1(page: puppeteer.Page, sourceId: number, pre: number) {
        ...
        for (const row of rows) {
            ...
            let owner_name: any = await row.evaluate(el => el.children[4].textContent?.trim());

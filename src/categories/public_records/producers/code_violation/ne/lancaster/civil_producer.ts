...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
        [
            { url: 'https://permits.lincoln.ne.gov/CitizenAccess/', handler: this.handleSource1 },
        ];
    ...
    async getData1(page: puppeteer.Page, sourceId: number) {
        ...
        await page.goto('https://permits.lincoln.ne.gov/CitizenAccess/');
        ...
        try {
            let valueForSelect = ['Building/Construction Complaint/City/NA', 'Building/Housing Complaint/City/NA', 'Building/Construction Complaint/County/NA', 'Building/Electrical/Complaint/NA', 'Building/Fire/Complaint/NA', 'Building/Plumbing/Complaint/NA'];
            let docTypeArr = ['City - Building Construction Complaint', 'City Housing Complaint', 'County - Building Construction Complaint', 'Electrical Complaints', 'Fire Complaints', 'Plumbing Complaints'];
            ...
            for (let j = 0; j < valueForSelect.length; j++) {
                ...
                let [btnPermit] = await iframe.$x('//a[contains(@title,"Building")]')
                ...
                await iframe.waitForXPath('//a[contains(.,"Search Applications")]', { visible: true, timeout: 200000 });
                let btnSearchPermit = await iframe.$x('//a[contains(.,"Search Applications")]')
                await btnSearchPermit[0].click();
                ...
                await this.sleep(5000);
                console.log(this.getFormattedDate(fromDate))
                await iframe.type('#ctl00_PlaceHolderMain_generalSearchForm_txtGSStartDate', this.getFormattedDate(fromDate), { delay: 200 });
                await iframe.type('#ctl00_PlaceHolderMain_generalSearchForm_txtGSEndDate', this.getFormattedDate(toDate), { delay: 200 });
                ...
                while (!flagStop) {
                    ...
                    for (let l = 0; l < totalRow!.length; l++) {
                        let index = l + 1;
                        let [addressXpath] = await iframe.$x('//table[@id="ctl00_PlaceHolderMain_dgvPermitList_gdvPermitList"]/tbody/tr[contains(@class,"ACA_TabRow_Odd") or contains(@class,"ACA_TabRow_Even")][' + index + ']/td[6]');

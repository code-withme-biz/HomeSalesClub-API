...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        { url: 'https://aca-prod.accela.com/lancaster/Cap/CapHome.aspx?module=Permits', handler: this.handleSource1 }
      ];
    ...
    async handleSource1(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        const isPageLoaded = await this.openPage(page, link, '//*[@id="ctl00_PlaceHolderMain_generalSearchForm_ddlGSPermitType"]');
        if (!isPageLoaded) {
    ...
    async setSearchCriteria1(page: puppeteer.Page, dateRange: any) {
        ...
        await page.select('#ctl00_PlaceHolderMain_generalSearchForm_ddlGSPermitType', 'Permits/Enforcement/Residential/Case');
        await page.waitForXPath('//div[@id="divGlobalLoading"]', {visible: true});
        await page.waitForXPath('//div[@id="divGlobalLoading"]', {hidden: true});

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://internal.chattadata.org/resource/qcrz-rvw7.json', handler: this.handleSource }
    ];
    ...
    async handleSource(page: puppeteer.Page, url: string, sourceId: number) {
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'date_entered', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    const property_address = record.address;
                    const fillingdate = record.date_entered;

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
      [
        { url: 'https://hub.arcgis.com/datasets/43f20b30904946e79ba53d95ea308e1b_0/data?geometry=-95.723%2C29.598%2C-95.407%2C29.650&orderBy=CaseDate&orderByAsc=false', handler: this.handleSource },
        { url: 'https://permits.missouricitytx.gov/energovprod/selfservice#/search', handler: this.handleSource2 },
      ];
    ...
    async handleSource(page: puppeteer.Page, link: string, sourceId: number) {
        ...
        while (true) {
            ...
            for (const row of rows) {
                let fillingdate = await row.evaluate(el => el.children[12].textContent) || ''
                ...
                let casetype = '';
                let property_address = await row.evaluate(el => el.children[9].textContent) || ''
                ...
                let mailing_address = await row.evaluate((el: any) => el.children[3].textContent) || '';
                mailing_address = mailing_address.replace(/\s+|\n/gm, ' ').trim();
                let mailing_city = await row.evaluate((el: any) => el.children[6].textContent) || '';
                mailing_city = mailing_city.replace(/\s+|\n/gm, ' ').trim();
                let mailing_zip = await row.evaluate((el: any) => el.children[8].textContent) || '';

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources =
        [
            {url: 'https://app.mygov.us/ce/citizen/check_project_status.php?citiesID=268', handler: this.handleSource1}
        ];
    ...
    async getData1(page: puppeteer.Page, sourceId: number) {
        let counts = 0;
        let complaintNumber = await this.getPrevCodeViolationId(sourceId, false, 5053);
        let nextFlag;
        let countSkip = 0
        do {
            try {
                nextFlag = true
                await page.goto(`https://app.mygov.us/ce/citizen/project_view.php?citiesID=268&projectsID=${complaintNumber}`, {waitUntil: 'load'})
                ...
                if (!!complaintPage) {
                    ...
                    countSkip++
                    if (countSkip > 30) nextFlag = false
                }
            } catch (e) {
                console.log(e)
                nextFlag = false

...
export default class CivilProducer extends AbstractProducer {
    ...
    sources = [
        { url: 'https://bellevue.data.socrata.com/resource/nme8-ens3.json?permitstatus=Open', handler: this.handleSource1, casetype: 'Code Compliance Enforcement Activity' },
        { url: 'https://data.seattle.gov/resource/ez4a-iug7.json?statuscurrent=Initiated', handler: this.handleSource2, casetype: 'Code Complaints and Violations'},
        { url: 'https://data.seattle.gov/resource/rsmq-5vwm.json?statuscurrent=Initiated', handler: this.handleSource2, casetype: 'Code Complaints and Violations Map' },
        { url: 'https://aca-prod.accela.com/KINGCO/Cap/CapHome.aspx?module=Enforce&TabName=Home', handler: this.handleSource3, casetype: '' }
    ];
    ...
    async handleSource1(page: puppeteer.Page, url: string, sourceId: number, casetype: string) {
        let dateRange = {
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'applieddate', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    const full_addr = JSON.parse(record.location_1.human_address);
                    const property_address = full_addr.address;
                    const fillingdate = record.applieddate;
    ...
    async handleSource2(page: puppeteer.Page, url: string, sourceId: number, casetype: string) {
        let dateRange = {
            from: new Date(await this.getPrevCodeViolationId(sourceId, true)),
            to: new Date()
        };
        ...
        while (true) {
            const response = await this.getCodeViolationData(url, limit, offset, 'opendate', dateRange.from, dateRange.to);
            if (response.success) {
                for (const record of response.data) {
                    const property_address = record.originaladdress1;
    ...
    async getData1(page: puppeteer.Page, sourceId: number) {
        ...
        const sortHandle1 = await page.$x('//span[text()="Opened Date"]/parent::a');
        await sortHandle1[0].click();
        await page.waitForXPath('//div[@id="divGlobalLoading"]', {visible: true});
        await page.waitForXPath('//div[@id="divGlobalLoading"]', {hidden: true});

        const sortHandle2 = await page.$x('//span[text()="Opened Date"]/parent::a');

...
export default class CivilParser extends AbstractParser {
  ...
  public getHeaders(fileName: string, type: string): string[] {
    if (fileName === "CASES.EXP" && type == "civil") {
      return this.getCasesHeaders();
    } else if (fileName === "CASETYPE.EXP" && type == "civil") {
      return this.getCaseTypesHeaders();
    } else if (fileName === "PARTIES.EXP" && type == "civil") {
      return this.getPartiesHeaders();
    } else if (
      /Indebtedness_[0-9]+\.txt/g.test(fileName) &&
      type == "indebtedness"
    ) {
      return this.getIndebtednessHeaders();
    } else if (fileName === "CASE.EXP" && type == "family") {
      return this.getFamilyCaseHeaders();
    } else if (fileName === "PARTIES.EXP" && type == "family") {
      return [
        "",
        "caseID",
        "caseNumber",
    ...
    } else if (/PRMOEST[0-9]+\.txt/g.test(fileName)) {
      return [
        "caseNumber",
        "fillingDate",
        "description",
  ...
  public async parse(caseGroup: [{ [key: string]: any }]): Promise<boolean> {
    ...
    for (const group of caseGroup) {
      ...
      const pls: any[] = items.filter((item: { [key: string]: any }) => {
        return item["partyType"] === "PN" || "Petitioner";
      });

      const defs: any[] = items.filter((item: { [key: string]: any }) => {
        return item["partyType"] === "DN" || "Decedent";
      });
  ...
  public async parseFamily(
    ...
    for (const group of caseGroup) {
      const items = group[1];
      const pls: any[] = items.filter((item: { [key: string]: any }) => {
        return item["partyType"] === "PE";
      });

      const defs: any[] = items.filter((item: { [key: string]: any }) => {
        return item["partyType"] === "RE";
      });
      await this.process(defs, pls);
    }
    return true;

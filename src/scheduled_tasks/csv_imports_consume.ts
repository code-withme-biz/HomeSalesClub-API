...
const setDate: string = process.argv[4];
...
( async () => {
    ...
    let condition: any = { source: 'csv-imports' };
    if (state && county) {
      condition = {
        ...condition,
    ...
    let data = await db.models.PublicRecordProducer.aggregate([
        { $match: condition },
    ...
    if(data.length < 1){
      ...
      data = await db.models.PublicRecordProducer.aggregate([
        { $match: condition },
        { $lookup: { from: 'county_priorities', localField: 'countyPriorityId', foreignField: '_id', as: 'county_priorities' }},
        { $unwind: "$county_priorities" },
        { $sort: {'county_priorities.priority': 1}}
    ...
    if(data.length > 0){
      // let wrongCounty = false;
      // for(const stateCsv in countiesWithCsv){
      //   if(stateCsv != data[0].county){
      //     wrongCounty = true;
      //     break;
      //   }
      //   for(const countyCsv of countiesWithCsv[state]){
      //     if(countyCsv != data[0].state){
      //       wrongCounty = true;
      //       break;
      //     }
      //   }
      ...
      try{
        ...
        const lastDate = publicRecordProducer.csvFetchLastDate;
        let today = new Date();
        let todayString: any = getFormattedDate(today);
        // todayString = '2021-05-18';
        if(setDate){
          console.log(">>>>> fetch csv imports from s3 for:", county, state, "<<<<<")
          await processCsvImport(state, county, setDate);
          console.log(">>>>> csv import end <<<<<");
        } else {
          if(lastDate != todayString){
            console.log(">>>>> fetch csv imports from s3 for:", county, state, "<<<<<")
            if(await processCsvImport(state, county, todayString)){
              console.log('Done csv imports');
              // // set the last date of fetched csv s3
              // let prc: IPublicRecordProducer = await db.models.PublicRecordProducer.findOne({ _id: data[0]['_id'] });
              // prc.csvFetchLastDate = todayString;
              // await prc.save();
            ...
            console.log("csv import already fetched from s3 today!");
          }
        }
      } catch(e){

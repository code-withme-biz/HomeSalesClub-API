...
( async () => {
    ...
    const removeAuctionCounty = async () => {
        await db.models.PublicRecordProducer.deleteMany({ source: 'auctioncounty' }).exec();
    }

    const createAuctionCounty = async () => {
        let index = 0;
    ...
    console.log('Starting Migration for auctioncounty');
    try {
        await removeAuctionCounty();
        await createAuctionCounty();
    } catch (error) {
        console.log(error);
    }
    console.log('Done');
